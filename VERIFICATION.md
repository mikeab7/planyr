...
## 🔲 Needs verification
### V74 — Optimistic concurrency: a stale save is rejected with a "reload" prompt (B297) ⏳ — needs the migration + a signed-in two-session check
- **Added** 2026-06-20 · **Cadence** once (acceptance) · **Last checked** — · **Next check** after the owner runs the migration, then a signed-in run.
- **⚠ Activate first (owner, one-time):** run `src/workspaces/site-planner/db/optimistic_concurrency.sql` in the Supabase SQL editor (adds the `version` column to `sites` + `doc_reviews`). Until then the guard is dormant **by design** — saving behaves exactly as today (the merge-safe fallback).
- **Steps (signed in, two browser tabs on the SAME project):** in tab A move Building 1; let it save ("Synced ✓"). In tab B (opened before A's save) move Building 1 differently and save. **Expect:** tab B's save is **rejected** with a loud blue banner "this project was changed in another session — reload before saving" + a **Reload** button (NOT a silent overwrite); Reload → B gets A's change and B's edit merges in (nothing lost). Repeat with the column ABSENT (pre-migration) → saving behaves exactly as before (no banner, no block).
...
### V73 — Multi-tab warning: same project open in another tab raises a banner (B298) ✅ (self-verified headless — fully done)
...
### V69 — Production error telemetry → Supabase `client_errors` (B279) ✅ (capture pipeline self-verified headless; ⏳ run the SQL once + one signed-in cloud-insert check)
...
- **⏳ Needs (a) the one-time SQL, then (b) a signed-in cloud-insert check:** run `src/shared/telemetry/client_errors.sql` once in the Supabase SQL editor (creates the INSERT-only table + RLS), then on planyr.io confirm a real captured error actually lands a row in `public.client_errors` — the DB write is the one path the sandbox can't exercise logged-out (the sink is a no-op without an anon key). Also confirm an **anonymous/logged-out** error still inserts (`user_id` null): logging that doesn't depend on a working session is the whole point. (Claude cohort's job, never Michael's.)
