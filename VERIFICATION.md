...
- **Cost/perf:** the Tesseract worker only spins up if a no-text page is actually present (a normal vector set pays nothing); it's a separate ~16 KB JS chunk + the WASM/model load on first use from CDN, then browser-cached. lint 0 · full suite green · build green; DocReview + pdf lazy chunks intact.
