...
## 🔲 Needs verification
### V90 — Doc Review: scanned (image-only) drawings are read by OCR into the auto-assembly pipeline (B343) ✅ (self-verified headless, incl. the LIVE Tesseract engine — fully done, browser-only)
...
- **Cost/perf:** the Tesseract worker only spins up if a no-text page is actually present (a normal vector set pays nothing); it's a separate ~16 KB JS chunk + the WASM/model load on first use from CDN, then browser-cached. lint 0 · 853 tests · build green; DocReview + pdf lazy chunks intact.
