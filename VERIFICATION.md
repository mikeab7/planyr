...
- **✅ Self-verified 2026-06-20 (15/15 checks, 0 page errors):** **B249** — fresh session shows **"Snap off"** and no global `localStorage:planarfit:snap`; toggling writes `sessionStorage` only. **B248** — with snap ON, dragging the building flush against the parking then away left the parking untouched (Δ=0px — no implicit bond). **B247** — Shift-click both → **Group** → the "⊞ Group" box renders **with no resize handles** (a group just stays together; it never scales as a whole — owner clarification) → dragging one member moved **both** (108px each) → **Ungroup** removed the box → members then moved independently again. **B250** — a 2-plan site → ✕ armed an inline "Delete …?" confirm (no browser dialog) → Delete removed the **current** plan and switched to its sibling (it did NOT resurrect) → the lone remaining plan has no ✕. lint **0 errors** · **537 tests** · build green.
