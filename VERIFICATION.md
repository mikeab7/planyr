...
## 🔲 Needs verification
### V74 — Doc Review stitch + markup safety guards: degenerate-align reject, unaligned-sheet flag/warn, ≥3-pt Area/Perimeter (B299 / B300 / B301) ✅ (self-verified headless — fully done, no signed-in check needed)
...
- **Steps:** Document Review (**Markup**). **B301:** open a PDF → **Area** → click **2** points + Enter or double-click ⇒ nothing commits (takeoff stays "No measurements yet"); click **3** points + Enter ⇒ one area commits. **Then Stitch sheets ▸**, open a multi-page PDF, add two sheets. **B300:** the 2nd sheet shows an amber **"Not aligned — click Align"** overlay + a panel chip; drawing a Distance/Area over it raises **"…isn't aligned yet — Align it first…"**. **B299:** click **Align** on the 2nd sheet and click the two moving-sheet points on ~the same spot ⇒ banner **"Those two points are too close together…"** and the sheet **does not move**; a real Align (distinct points) then succeeds and clears the flag.
- **✅ Self-verified 2026-06-20** (`ui-audit/verify-b299-b301.mjs`, headless **chromium-1228**, logged-out, screenshot `ui-audit/screens/b299-b301.png`): **14/14 checks** — 2-pt Area commits nothing / 3-pt commits one; 2nd sheet flagged + measure-over-it warns; degenerate Align banner fires and the sheet's `<g transform>` is **unchanged** (not flung); valid Align clears the flag; **0 page JS errors**. (Same chromium-1228 requirement main's V72 documents — 1194 lacks `Map.prototype.getOrInsertComputed` and throws mid-render.)
- **Deterministic logic** — `solveM` + the `alignBaselinesDegenerate` guard, `sheetContains` / `measureOverUnaligned`, and `canCommitMeasure` (now gating **both** Enter/`finishDraft` and double-click/`onDbl`, so a 2-point area can't slip in either way) — is covered by **18 unit tests** (`test/stitchGeom.test.js` 8, `test/takeoff.test.js` +2 for B301), all green. lint 0 · build green.
