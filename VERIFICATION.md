...
### V43 — Header/UI polish: legible module tabs, slimmer top header, no page-scroll (scale bar visible), parcel active checkbox (B167–B170) ✅ (self-verified) / ⏳ (signed-in)
- **Added** 2026-06-19 · **Cadence** once (acceptance) · **Last checked** 2026-06-19 ✅ (headless Chromium on the production build, logged-out) · **Next check** —
- **Steps:** (1) Look at the top of the app — **Site / Schedule / Markup** tabs: the inactive two should read as clearly clickable (legible muted text + crisp icons), not grayed-out/disabled; hovering one previews its module color; the active tab is colored + underlined. (2) The **top header row** should be visibly slimmer (~35px vs the old 44). (3) Open a saved site in the planner on a **short window** — the **map scale bar (bottom-right)** must be visible with **no page scrolling**, and both header rows stay in view at the same time. (4) **Site → Parcel** panel: each parcel row has a **checkbox** (checked = active); new/existing parcels start checked; unchecking dims the row + marks "· inactive" and drops it from yield/coverage/detention (B100 math).
- **Expect:** tabs legible (not disabled-looking); header thinner; scale bar always visible / no page scroll; per-row active checkbox reflecting + toggling parcel active state.
- **✅ Self-verified 2026-06-19 (headless Chromium, `npm run build` + `vite preview`, logged-out, seeded 2-parcel site):** at a deliberately short **1280×560** viewport `documentElement.scrollHeight − innerHeight = 0` (no page scroll) and the scale bar sits at the canvas bottom-right with both header rows shown; tab colors computed = active `rgb(29,158,117)` (teal accent) / inactive `rgb(184,178,164)` (legible muted, not 30%-opacity white); Parcel panel rows each render a checkbox — parcel 1 checked, parcel 2 unchecked + dimmed "· inactive". lint 0 errors · 257 tests · build green · `SitePlannerApp` lazy chunk intact.
- **✅ Re-verified after the furniture-overlay follow-up (owner re-report — scale + north not fully shown):** scale bar + north arrow now DOM overlays anchored to the visible canvas corners. At **1600×800 and 1600×540** the "FEET" (scale) and "N" (north) labels both `getBoundingClientRect` fully inside the viewport (overflow 0); bottom-right corner shot shows the scale bar clear below the +/−/⤢ zoom stack; bottom-left shot shows the north arrow **beside** (not under) the green "Scaled · county GIS" calibration pill.
- **⏳ Still worth a SIGNED-IN glance:** the parcel `active` flag persists via the same Site Model `data` path as B100 (already proven), so confirm a toggled-off parcel stays off across reload when signed in — low-risk confirmation, not a new mechanism.
