...
- **✅ 2026-06-21 (B356) — fixed a real `parseSheetScale` miss:** an architectural scale was read as **null whenever a number printed just before it** (e.g. a date next to the scale, `…SHEET A-201  10/24/2025  1/8"=1'-0"`) — the mixed-number branch swallowed the date digits + fraction, failed the sanity range, and never tried the real fraction. Fixed + unit-locked (`test/sheetScale.test.js`). Pure-logic (no browser needed); folds into the B356 corpus run for a live look on the owner's real arch sheets.
