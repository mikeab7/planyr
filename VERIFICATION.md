...
- **Why ⏳ (now narrowed to one prod round-trip).** After the UA fix deploys to production, confirm on `planyr.io` directly: a real image request returns **200 `image/png`** (not 302) and a follow-up `…&meta=1` flips to **`cached:true`** with a `ts` — proving fetch+store+serve through Drive end-to-end. (Sandbox curl CAN reach planyr.io for these endpoints even though browser navigation is blocked.) Then an owner glance in-app: layers paint, "as of …" age shows. Fail-open + client fallback mean worst case is today's behavior, never a broken layer. Cadence: once after the fix ships. Last checked: UA root-cause proven live on preview; prod round-trip pending the fix deploy.
