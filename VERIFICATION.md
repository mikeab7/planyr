...
## 🔲 Needs verification
### V133 — B384: "Add by address" in the planner's ＋ Add parcel menu ⏳ (UI wired + geocoder unit-tested; needs ONE live geocode→GIS round-trip)
- **Added** 2026-06-24 · **Cadence** once (feature acceptance) · headless UI **5/5** (`ui-audit/verify-b384-add-by-address.mjs`) · **Next check** — the live round-trip below, on planyr.io (or any network-enabled browser). **Claude cohort / a human, never Michael.**
- **What changed.** A third add method, **📍 Add by address**, in the planner's ＋ Add parcel menu: type a street address → it geocodes (shared `lib/geocode.js`, the same Esri-first pipeline the map's search uses — never a fork, B383) → projects the hit into the plan's feet frame → runs the existing `quickAddAt` (identify-and-add) so the lot, its appraisal card, and jurisdiction all populate.
- **✅ Already proven without the network.** 7 unit tests for the geocoder (`test/geocode.test.js`); headless UI (logged-out) confirms the field renders in the menu, Find disables on empty / enables on text, typing keeps the menu open, no page errors. lint 0 · 1442 tests · build green.
- **⏳ The live confirm owed (network-gated — the sandbox can't reliably reach geocode.arcgis.com OR the county parcel service, same class as the map's existing address search).** On a network-enabled browser: open a **georeferenced** plan → ＋ Add parcel → 📍 Add by address → type a real address inside the plan's county (e.g. a Katy/Harris address) → **Find**. **Expect:** the card reads "Finding address…", then the lot at that address is **added to the plan** (parcel appears + selected, appraisal card shows). A bad address → an honest "Couldn't find…" message, not a crash. A non-georeferenced plan shows the disabled explainer instead of the field.
