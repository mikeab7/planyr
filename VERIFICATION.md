...
### V45 — Site Analysis: constraint queries resolve + click-to-show-on-map (B184 / B185) ✅ (self-verified, headless, with live GIS data)
...
- **🔑 Allowlist note (so a fresh session can self-verify headless):** as of 2026-06-19 the GIS hosts are on the env egress allowlist — `services.arcgis.com` (county + ETJ) and `feature.geographic.texas.gov` (city) confirmed reachable (HTTP 200) from-sandbox; **`harcags.harcresearch.org` (MUD) needs a NEW session** to pick up the allowlist change (the session that shipped this still got 403 "Host not in allowlist"). So a fresh Claude session **can** drive the headless app and confirm all four layers' tiles actually paint — see the handoff below.
- **✅ Self-verified 2026-06-19 (headless, `npm run build` + `vite preview`, logged-out):** the **Jurisdictions** group and all four toggles render in the Layers panel; toggling **County boundaries** on works with **zero JS errors** (`gis-verify/jurisdictions-overlay-verify.mjs`). Tile PAINT still needs the fresh-session check above.
- **📋 Handoff for the next session:** in a NEW session (so `harcags.harcresearch.org` is reachable), run `npm ci && npm run build && npx vite preview --port 4173`, then a Playwright script that boots the app, opens the **Layers → Jurisdictions** group, toggles each layer, and **records the network responses for the tile/export requests** (county/ETJ → `services.arcgis.com`; city → `feature.geographic.texas.gov`; MUD → `harcags.harcresearch.org/.../export`). PASS = each returns HTTP 200 with a non-empty image/feature payload and the layer's status dot goes "loaded". Extend `gis-verify/jurisdictions-overlay-verify.mjs` (it already renders the panel + toggles) with a `page.on("response")` collector filtered to those hosts. Then flip this item ✅ (or ❌ with the failing host).
...
- **Self-verified (logged-out) — ✅ PASS, no errors from this code:** booted the preview, switched to the **Markup** module, and confirmed the new **🗂 Files** pill renders in **Row 1** (next to the project name, NOT a fourth tab) and opens the **Project Files** drawer. Logged-out it correctly shows the **"Sign in to browse your project files"** gate. The only console errors were the pre-existing FBCAD GIS CORS noise from the Site Planner probe — unrelated to this change. Engine is covered by **33 unit tests** (`fileFacts` 15, `placeOnMap` 10, `verifyPlacement` 8); full suite **290 tests** + lint 0 + build green; doc-review lazy chunk split intact.
