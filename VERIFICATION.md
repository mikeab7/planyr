...
## 🔲 Needs verification
### V75 — Parcel click-vs-drag + "Select parcels" toggle (B300 / B301) ✅ (fully self-verified headless — no signed-in check needed)
...
- **✅ Self-verified 2026-06-20 (`ui-audit/verify-b300-b301.mjs`, 7/7 checks, 0 page JS errors):** **B300** a deselected→**click selects** the parcel (stroke→#c2410c / width 3); a **95px drag pans and does NOT select** (stroke stays #5b6650 / width 2; the parcel's on-screen box shifted +95px — it panned). **B301** the toolbar toggle defaults **ON** (`aria-pressed=true`); flipping it **OFF** makes a click no longer select; flipping back **ON** restores click-to-select; the **OFF state persists across a full page reload** (saved per project in the Site Model `settings`). Screenshots `ui-audit/screens/b300-click-selected.png`, `b300-drag-panned.png`. lint 0 · 690 tests · build green; `SitePlannerApp` lazy chunk intact.
