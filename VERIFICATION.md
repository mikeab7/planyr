...
## 🔲 Needs verification
### V128 — B438: GIS imagery service worker (host-scoped) ✅ build + lint + 1412 tests + headless 5/5 (`verify-b438-gis-sw.mjs`); ⏳ live production confirm after ship
- **What changed.** `public/gis-sw.js` caches cross-origin ArcGIS imagery/tiles (county/agency layers + basemap) with stale-while-revalidate so saved sites paint instantly + survive a server outage; host-scoped (same-origin app assets passed straight through → no stale-bundle risk); versioned cache, oldest-first trim. `lib/gisSwRules.js` (pure matcher/trim, 10 unit tests) + `lib/registerGisSw.js` wired in `main.jsx` (fail-safe, secure-context-only).
- **Verified (sandbox).** `ui-audit/verify-b438-gis-sw.mjs` 5/5: SW registers + activates + controls the page; caches the cross-origin basemap tiles (37 cached, mocked at the network layer since the sandbox can't reach gov hosts); NO app/same-origin asset leaks; app shell still renders. lint 0 · 1412 tests.
- **Production (post-ship, 2026-06-24).** ✅ `https://planyr.io/gis-sw.js` is **served live** (HTTP 200, `application/javascript`, our B438 worker — confirmed via curl). The in-**browser** registration check on planyr.io can't run from the sandbox (its Chromium network policy only allows the GIS tile hosts, not planyr.io → `ERR_CONNECTION_CLOSED`), so the live register/activate/control + real gov-host opaque caching is an **owner glance** (the logic itself is already proven by the 5/5 headless test). Owner spot-check: open a site with layers on, reload → layers paint instantly from cache; throttle/offline → last-good still shows.
- **Why ⏳.** The real gov-host opaque-response caching + the instant-paint/outage-survival win can only be confirmed against **production** (the sandbox can't reach those hosts, and its browser can't reach planyr.io). Screening-only (no precise per-layer age badge yet — a follow-up that needs SW→app plumbing since the SW caches invisibly). Cadence: once after ship. Last checked: SW file confirmed live; in-browser live check pending owner glance.
