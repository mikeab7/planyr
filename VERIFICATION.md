...
## 🔲 Needs verification
### V137 — B472: new-site data-loss mitigation (save-verify + "Save now") + logged-out path proven ✅ headless 10/10 (`ui-audit/verify-new-site-save.mjs`) + lint 0 + 1528 tests + build green + `verify-readonly-takeover.mjs` 7/7 (no regression); ⏳ ONE live signed-in occurrence to pin the root cause
- **What changed.** Owner repro (signed in): a brand-new site, single tab, placed items → gone after reload. The headless repro proves the pure logged-out path SAVES + survives reload, so the loss is signed-in/cloud-specific. Shipped the cause-agnostic mitigation: the autosave reads back every on-device write and fires a LOUD red banner + `save-verify-failed` telemetry if it didn't persist; a new **Save now** menu action force-saves, verifies, and shows "Saved ✓ N items".
- **Verified (sandbox, logged-out).** `verify-new-site-save.mjs`: create via "Start blank" → place 2 markups → store has them → reload → still present + planner re-opens them (10/10). No regression in the read-only takeover harness.
- **Why ⏳ (Cowork / owner, signed-in).** The actual loss is signed-in-only (sandbox CORS-blocks sign-in). To PIN it: from a live failure, capture `window.pfTelemetry.recent()` + the `public.client_errors` rows + whether the new site's record is in `planarfit:sites:cloud:<uid>` vs the legacy store. That separates never-saved / wrong-store / orphaned-resume / cloud-reject. The mitigation means a recurrence is now LOUD + traceable rather than silent.
...
### V131 — B446/B447/B448: Document Review open-feedback + switch-determinism + mid-upload backdrop ✅ logged-out headless 6/6 + 5 unit tests + lint 0 + 1434 tests + build; ⏳ one signed-in switch + mid-upload click-through
...
### V129 — B445: GIS imagery cache, server-side (Drive-backed) ✅ VERIFIED LIVE on planyr.io (backend end-to-end); ⏳ only an in-app visual glance remains  *(B# renumbered from B439 → B445 on 2026-06-24 to clear a collision with the breadcrumb rename/delete B439/B440)*
