...
## 🔲 Needs verification
### V73 — Doc Review Stitcher + Markup takeoff-correctness guards (B297–B299) ✅ (self-verified headless — fully done, no signed-in check needed)
- **Added** 2026-06-20 · **Cadence** once (acceptance) · **Last checked** 2026-06-20 ✅ (headless **Chromium-1228** on the built app, `vite preview`, logged-out, generated 2-page Letter PDF) · **Next check** — none (pure Stitch/Markup-canvas geometry; opening a local PDF + placing/aligning/measuring needs no auth or cloud path). ⚠ Code-complete on branch `claude/awesome-feynman-i7wypf`; verified against the **branch build** — pending merge to `main` to go live.
- **✅ Self-verified 2026-06-20 (`ui-audit/verify-stitch-guards.mjs`, 5/5 checks, 0 page errors):** **B298** an un-aligned second sheet is flagged **NEEDS ALIGN**, and a **Distance** drawn over it is **blocked** (0 committed lines + the "isn't aligned" banner) so a mixed scale can't silently corrupt the shared takeoff. **B297** a **degenerate Align** (two coincident clicks on the moving sheet) is **rejected** with the "too close together" banner and the sheet's transform is **left untouched** — scale **1.000 → 1.000**, NOT flung to a runaway ×N (the bug had no undo).
- **✅ Regression — single-sheet viewer unaffected (`ui-audit/verify-docreview-viewer.mjs`, 13/13, 0 page errors):** the B299 `finishDraft`/`onDbl` min-vertex change + the `takeoff.js` `canCommitMeasure` extraction don't disturb the B288–B296 viewer behaviours (count double-click still finishes with exactly 3 dots). **B299** itself (≥3-point area/perimeter) is a pure-function guard covered by `test/takeoff.test.js`; **B297/B298** geometry is covered by `test/stitchGeom.test.js`. lint **0 errors** · **644 tests** · build green (Doc Review lazy chunk intact).
