...
## 🔲 Needs verification
### V153 — Claude↔Scheduler connector end-to-end (B527) ✅ unit 28/28 + lint/build green; ⏳ live deploy + Claude-connected round-trip + claude.ai-web OAuth owed
- **What changed (2026-06-27, branch `claude/claude-app-integration-d1s30x`).** New MCP connector (`server/scheduler/` + same-origin endpoint `functions/api/mcp/[[path]].js`) so the Claude Michael already pays for can read the live Scheduler and **propose** changes (pending rows in `planar_suggestions` → the Scheduler's Review panel → one-click Approve/Dismiss; suggest-only). Tools `get_schedule` / `propose_task_change` / `propose_new_task`; one-time RLS `public/sequence/db/suggestions_rls.sql`.
- **✅ Verified here (no browser/deploy needed).** `test/scheduler.test.js` **28/28** — builders enforce the `SGQ_ALLOWED_PATCH` allow-list + value validation; `getSchedule` compaction + error mapping; `insertSuggestion` posts the right row and maps RLS 401/403 to a run-the-SQL hint; `handleMcp` covers initialize / tools/list / tools/call (all three tools, incl. isError surfacing) / notifications. `npm test` **1669**, lint 0 errors, build green.
- **⏳ Why pending (can't be exercised from the sandbox — needs the live edge + a connected Claude).**
  1. **Owner provisions** (see `OWNER-TODO.md`): run the SQL, set `SCHEDULER_SUPABASE_URL` / `SCHEDULER_SUPABASE_ANON_KEY` / `SCHEDULER_CONNECTOR_TOKEN` in Cloudflare Pages, deploy.
  2. **Connected round-trip:** from Claude Desktop (`mcp-remote` + bearer header) or a claude.ai connector, "read my schedule" → `get_schedule` returns the real projects; "push task <id> out N days" → a **pending** row appears in the Scheduler's Review panel; Approve applies it and dates cascade.
  3. **OAuth for the claude.ai-web path:** the bearer token is the interim gate; the native custom-connector flow expects OAuth — wire a `workers-oauth-provider` (or equivalent) wrapper so Projects/chats on claude.ai web can connect. (Desktop via `mcp-remote` works on the bearer token alone.)
- Cadence: once after the owner provisions.
