...
- **✅ B326 self-verified (`ui-audit/verify-markup-rail.mjs`, 11/11):** all 10 tools live in the right rail (none left in the header), the rail sits right of the canvas + left of the Takeoff, **Select** is active by default on the Markup accent **#EF9F27**, clicking a tool activates it + the hint strip follows, the rail's zoom controls work, the Takeoff panel collapses + re-opens. Screenshot `ui-audit/screens/b326-markup-rail.png`.
- **✅ No regression:** the full B288–B296 viewer suite (`ui-audit/verify-docreview-viewer.mjs`, 13/13) still green under the new transform model. lint 0 · 794 tests · build green; `DocReview` + `SitePlannerApp` lazy chunks intact.
- **Note:** touchscreen two-finger pinch isn't wired yet (matches the Site map, which also has none) — filed as **B327** (in flight this session); desktop trackpad pinch (ctrl+wheel) routes through the wheel zoom path and works.
