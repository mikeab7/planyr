...
### V9 — Attach & mark up a drawing on a parcel (B67) ⏳ (attach UI confirmed; full flow pending a manual file pick)
- **Added** 2026-06-16 · **Cadence** once (feature acceptance) · **Last checked** 2026-06-20 (partial, Cowork) · **Next check** 2026-06-20
- **◑ Partial 2026-06-20 (Cowork — signed-in planyr.io).** Drew a Boundary → parcel ("Parcel 1", 6.24 ac); the Parcel panel then correctly shows **ATTACHED DRAWINGS → "＋ Attach a drawing (PDF / JPG)"**. Could **not** complete the attach via tooling: setting the hidden file `<input>` programmatically (both a PDF and a JPG) did **not** register — no "Pick a sheet" dialog, `parcelDrawings` stayed empty — even though the *same* technique works for the V4 overlay input. So this parcel-drawing handler appears to require a *trusted* file-pick (native dialog) rather than a synthetic input change. The attach **UI** is present/correct; the attach→sheet-picker→markup→reopen/reload persistence flow still needs a real manual file drop to confirm. (No bug implied — just not driveable headlessly via the synthetic input.)
