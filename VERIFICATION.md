...
### V45 — Project Files drawer: saved views, document classes, drop-to-file, needs-filing (B180) ⏳ (app-wide open ✅ self-verified; signed-in browse needs a session)
- **Added** 2026-06-19 · **Cadence** once (feature acceptance) · **Last checked** 2026-06-19 (app-wide open ✅, headless logged-out) · **Next check** SIGNED-IN on planyr.io (sandbox proxy blocks auth, so the file list / views can't render here)
- **Steps (on https://planyr.io, SIGNED IN, with at least a few filed PDFs):** From **any** workspace (Site / Schedule / Markup) click the **Files** button in the top row (next to the project name). (1) Confirm the **saved-view chips** show with counts — All files, Map-ready (spatial), All surveys, Title commitments, Civil set, Reference docs, On the map, Needs filing. (2) Click **All surveys** with the project filter set to **All projects** → surveys from *every* project list (cross-project); set the **project filter** to one project → the same chip now lists only that project's matching files where the view is project-scoped. (3) In **All files**, expand a project → discipline → files; each file shows a **document-class dot** (teal = spatial, gray = reference, amber = title commitment) and an **On map** pill if it's been calibrated. (4) **Drop a PDF** on the top drop zone (or a project/discipline) → it files; with no project it lands under **Needs filing**. (5) In **Needs filing**, pick a project + discipline on a row and click **File** → it leaves the holding area. (6) On a spatial file click **⊕ Map** → the placement-readiness panel shows which cascade rung would run and why each higher rung isn't ready yet. (7) Click a file → it opens in the Markup viewer (switching workspace if needed).
- **Expect:** Folders behave as *queries* (the same file appears under every view it matches; nothing is a hand-moved tree). Cross-project vs per-project toggles purely via the project filter. Filing is private-by-default (only your files). No console errors.
- **✅ Self-verified 2026-06-19 (headless, `npm run build` + `vite preview`, logged-out):** the **Files** button renders in Row 1 of **both** the Site Planner and the Markup workspace; clicking it opens the **Project Files** drawer (header + the signed-out prompt + the "saved views (queries)" footer) with **zero JS errors** (`ui-audit/verify-files-drawer.mjs`). The view-model itself (tagging, saved-view queries, per/cross-project filtering, needs-filing, Filed/On-map) is covered by **14 unit tests** (`test/fileIndex.test.js`). Only the signed-in *browse* (which needs real `doc_reviews` rows) is unconfirmed here — it rides the existing B14 Supabase read path (already proven), so this is a low-risk confirmation, not a new mechanism.
...
- **Note:** Browser verification not possible in this session — `sequence/index.html` loads React/Babel/Supabase from CDN, which is blocked (403) in the sandbox. Build is green. Code changes are in `public/sequence/index.html` (keyboard handler + GridView rendering). Logic verified by static analysis. Verify in a real browser on next run or in production.
...
### V44 — Site Analysis tool: presence-first screen of the active parcels, honest unknowns (B147 v1) ✅
