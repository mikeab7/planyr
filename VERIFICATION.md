...
>
> - **V140 ⏳** — B475/B476 Markup switcher cold-cache divergence not driven this session (the B475 fix is signed-in cold-cache-only — needs a fresh tab/device with an empty project cache; B476 is messaging-only).
...
### V140 — B475/B476: Markup switcher cache divergence + honest at-risk wording ✅ lint 0 · 1571 tests · build green; ⏳ ONE signed-in cold-cache spot-check (Cowork)
...
### V131 — B446/B447/B448: Document Review open-feedback + switch-determinism + mid-upload backdrop ✅ logged-out headless 6/6 + 5 unit tests + lint 0 + 1434 tests + build; **B446 open-feedback ✅ VERIFIED LIVE 2026-06-25 (Cowork)**; ⏳ B447 switch-determinism + B448 mid-upload still owed (need a 2-review project + a droppable PDF)
...
- **Why ⏳.** The **switch-determinism** (B447) and **keyless-mid-upload** (B448) paths need TWO saved cloud reviews + auth, which the logged-out sandbox can't drive (Supabase auth is CORS-blocked). **Signed-in live check (planyr.io):** open file A, make an edit, switch to B, switch back to A → A's backdrop + markups + calibration + view + project breadcrumb all return intact (the edit is not lost). Then: drop a fresh PDF and IMMEDIATELY switch to another file and back before the upload finishes → the dropped backdrop still shows (never the re-drop banner / a blank canvas). Cadence: once after ship. Last checked: **2026-06-25** — **B446 ✅ VERIFIED LIVE (Cowork, signed-in)**: clicking a Library entry (8 South's Geotech report) raised the **"Opening &lt;name&gt;…" overlay** then rasterized to a clean render — the open is never silent. **B447 switch-determinism + B448 keyless-mid-upload still ⏳**: couldn't drive this session — the available test project had only ONE saved review (a clean A↔B switch-back needs two reviews in one project), and the mid-upload drop needs a local PDF the Cowork browser session can't supply. Re-run when a 2-review project + a droppable PDF are on hand. (Sandbox logged-out paths remain green.)
### V130 — B439/B440: rename + delete projects from the breadcrumb switcher ✅ Site path headless 7/7 + 4 unit tests + lint 0 + 1416 tests + build; ✅ **Schedule** module signed-in click-through VERIFIED LIVE 2026-06-25 (Cowork)
- **Collision RESOLVED (2026-06-24):** a concurrent session had also minted B439 for the server-side GIS imagery cache; that GIS item was renumbered **B439→B445** (it took the next free B#) so this rename/delete feature keeps the canonically-filed **B439/B440** (PR #327). No remaining duplicate.
...
- **Why ⏳.** The **Schedule** half drives the embedded `/sequence/` app, which can't boot in the offline sandbox (its CDN libs + its own Supabase backend are blocked — the same wall `verify-schedule-picker.mjs` hits here), so the bridge can't be exercised headless. On planyr.io it boots normally. **Signed-in live check:** open the **Schedule** module → project switcher → right-click (or hover ⋯) a project → **Rename** it (the name updates in the crumb + the scheduler) and **Delete** a project (it disappears; deleting the open one lands you on the Dashboard). Cadence: once after ship. Last checked: **2026-06-25 ✅ VERIFIED LIVE (Cowork, signed-in)** — in the Schedule switcher: hovering a project row reveals the ⋯ kebab; on a throwaway project, **Rename** (inline edit) updated the name in BOTH the breadcrumb crumb AND the embedded scheduler's project list; **Delete** showed the inline "this can't be undone" confirm, removed the project, and — it being the currently-open one — dropped the app to the **Dashboard** (All-projects task report). The owner's "i cant change names or delete in the schedule module" report is fixed live. (Sandbox Site path remains green.)
...
### V129 — B445: GIS imagery cache, server-side (Drive-backed) ✅ VERIFIED LIVE on planyr.io — backend end-to-end AND in-app render (2026-06-25); fully passed  *(B# renumbered from B439 → B445 on 2026-06-24 to clear a collision with the breadcrumb rename/delete B439/B440)*
...
- **Why still ⏳ (narrowed to a visual glance).** The backend is proven; the one thing curl can't show is the **in-app render** — that esri-leaflet, pointed at the proxy, paints the layer and the panel shows the "as of …" age. Owner glance on `planyr.io`: turn on FEMA/wetlands, confirm the layer paints + the age label appears; (optional) block the agency host in devtools → the last-good copy still shows. The client path has a unit-tested one-shot fallback to the direct agency URL, so worst case is today's behavior, never a broken layer. Cadence: once. Last checked: **2026-06-25 — in-app render VERIFIED LIVE**.
- **✅ IN-APP RENDER VERIFIED LIVE (2026-06-25, Cowork, signed-in browser).** On planyr.io, toggled **FEMA flood zones** on and zoomed to street level → the NFHL raster **paints** over the basemap (teal flood polygons along the Houston Ship Channel). Network confirms esri-leaflet's export requests route through the **same-origin proxy** `…/api/gis-cache/svc/<b64>/export?…` (the b64 decodes to `https://hazards.fema.gov/arcgis/rest/services/public/NFHL/MapServer`) returning **HTTP 200 `image/png`** — **not** a 302 fail-open, and with **no** direct `hazards.fema.gov` request — plus a `&meta=1` sidecar at 200 that drives the panel **age badge** ("just now" / "as of …"). The whole B445 chain renders in-app, closing the one thing curl couldn't show. Fully passed → archivable to VERIFICATION-DONE.md.
...
### V117 — Signed-in: files OPEN from the Library browser now that Drive is live, and each failure names its precise cause (B405 + B207) ✅ core "files OPEN" path VERIFIED LIVE 2026-06-25 (Cowork, signed-in); oversize-banner + Drive-row steps not re-exercised this run
- **Added** 2026-06-23 · **Cadence** once (bug-fix + deploy acceptance) · **Last checked** **2026-06-25 ✅** — opened **8 South** → Review → clicked the existing **8 South B2 & B3 - Final Geotech Report** entry: the B446 **"Opening …" overlay** fired, then the **68-sheet PDF rendered** (cover page + sheet thumbnails + tool rail + Takeoff panel), with **no banner and no blank screen**. Files OPEN from the Library signed-in (the original "opening any file fails" root cause is cleared). · **Next check** — the optional **oversize banner** (step 3) + **Drive-row re-confirm** (step 4), on planyr.io. **Claude cohort's job, never Michael's.**
