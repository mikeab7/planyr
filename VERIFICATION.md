...
## 🔲 Needs verification
### V84 — Contrast regression fixed (B321) + theme picker moved to the Settings gear (B322) ✅ (self-verified headless BOTH themes — fully done, no signed-in check needed)
...
- **✅ B321 self-verified (`ui-audit/verify-b321-contrast.mjs`, `ui-audit/contrast-audit.mjs` + `test/contrast.test.js`):** the live computed text/background of the **user-name (Sign-in) pill**, the **Dashboard/Map breadcrumb** (active + idle), the **project crumb**, and the **module tabs** all clear **WCAG AA in both themes** (pill 16.66 light / 15.11 dark; crumbs 16.66 / 15.11; tabs 5.3–6.9 / 8–10.3) — the cream/white-on-light-chrome regression is gone. The programmatic token-pair audit reports every defined pair ≥ its floor in both themes (7 documented exceptions). Screenshots `ui-audit/screens/b321-chrome-{light,dark}.png` + `theme-markup-{light,dark}.png` (Markup header: title, Files, "Saved/Not-saved" badge, toolbar all legible). lint 0 · **783 tests** · build green.
- **✅ B322 self-verified (`ui-audit/verify-b322-settings.mjs`, logged-out):** the open segmented theme control is gone from the header; a **Settings gear** opens a popover with Light/Dark/System; choosing **Dark** flips `data-theme="dark"` live + persists to `localStorage`, back to **Light** flips it back — all while signed out. Screenshot `ui-audit/screens/b322-settings-popover.png`.
