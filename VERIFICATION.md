...
## 🔲 Needs verification
### V67 — Account names (profiles table) + identity-pill dropdown (B271 / B272) ✅ (signed-in UI self-verified headless via seeded session; ⏳ one live cloud round-trip)
...
- **Harness:** `ui-audit/verify-b271-b272.mjs`. supabase-js reads its session from `localStorage` with **no network**, so seeding a well-formed far-future session drives the **real signed-in UI** despite the sandbox's logged-out-only auth proxy.
- **✅ Self-verified headless (12/12, 0 JS errors):** the Row-1 pill shows the user's name ("Mike Abbott"); clicking it opens the account dropdown with **Profile / Settings / Sign out** + the account email; **Profile** opens the modal pre-filled (First/Last/Org) with a **Save profile** action; **Settings** shows **Change password**. Logged-out, the pill is **"Sign in"** and opens the modal → Sign up shows the **First/Last name** fields. Screenshots `ui-audit/screens/b272-account-dropdown.png`, `b271-profile-modal.png`, `b272-settings-tab.png`, `b271-signup-form.png` (gitignored).
- **Deterministic logic** — the never-blank display chain (`displayNameFor`/`firstNameFor`/`initialFor`) — is covered by **9 unit tests** (`test/profileDisplay.test.js`), all green.
