...
### V51 — Building feature-edit buttons hide when the on-screen footprint is too small; show only on the selected/hovered building (B221 + B222) ✅
...
- **Steps:** seeded one **400′×300′ cross-dock building** in a parcel, resumed into the planner. **B221:** selected it, then swept the wheel zoom from large to tiny, and at each step measured the building footprint rect's on-screen px **and** counted the drawn feature-add buttons (the +/− circles with an "Add …"/"Remove this" `<title>`), comparing to the count predicted by the per-axis rule. **B222:** with nothing selected, moved the cursor off the building, onto it, and off again, counting buttons each time. Script `ui-audit/verify-b221-b222.mjs`.
- **Result ✅ (all checks passed):**
  - **B221 size-gate (per-axis):** **8** buttons while the footprint was large (494×370 … 127×95px); **2** (the long-side sidewalk buttons only) at **81×60px** — height < 72 ≤ width, so the cramped short-end + corner buttons correctly drop while the long-side ones persist (the overlap/"also check" case, handled without a collapse menu); **0** once tiny (≤ 64×48px) — no spill, no cluster. The drawn count matched the rule predicted from the measured px at **every** non-ambiguous step. Screens: `b221-zoomed-in.png`, `b221-partial-longside.png`, `b221-zoomed-out.png`.
  - **B222 selected-or-hovered:** nothing-selected & not-hovering = **0** buttons; hovering the building = **8** (revealed by hover alone — `b222-hover.png` shows the add-buttons with NO selection grips); moving off = **0**. So the buttons appear on the ONE active building only, gated by the same size rule.
