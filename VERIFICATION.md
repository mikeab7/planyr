...
- **Why ⏳.** The real win — layers painting from the Drive copy + surviving an agency outage + the age badge — only exercises against **production** (deployed Function + live Drive creds), which the sandbox can't reach (planyr.io blocked → `ERR_CONNECTION_CLOSED`; no Functions under `vite preview`). Owner glance: open a site with layers on → reload → layers paint; (optional) block the agency host in devtools → last-good copy still shows with an "as of …" age. Fail-open + the client fallback mean the worst case is exactly today's behavior (no caching), never a broken layer. Cadence: once after ship. Last checked: logic proven by 27 unit tests; in-browser live check pending owner glance.
