...
## 🔲 Needs verification
### V71 — Doc Review single-sheet zoom / pan / navigation + drawing-correctness + markup editing (B283–B291) ✅ (self-verified headless — fully done, no signed-in check needed)
...
- **✅ Self-verified 2026-06-20 (`ui-audit/verify-b283-b291.mjs`, 13/13 checks, 0 page errors):** **B283** wheel/Ctrl/pinch zooms in (canvas 1054→1212px) **cursor-anchored** (point under the cursor drifted **0.4px**). **B285** the **+** button zooms holding the **viewport centre** fixed (drifted 0.6px). **B284** the **Pan** tool drag scrolls the viewport by the exact drag delta (Δ = 110,70 for a 110,70 drag). **B287** switching Sheet 1→2 **keeps** the zoom (1454px both, no snap to fit-width). **B290** **Fit page** fits the whole sheet (602×780 inside an 1078×804 viewport) where plain **Fit** (width) overflows height (h=1364). **B286 (HIGH)** Count: 3 clicks + double-click finishes with **3** dots, not 5 — the double-click's two phantom pointerdowns are stripped. **B288** a placed Rect **drags** to a new position (x 181→301 for a +120px drag); a Text note is **created** through an inline `<input>` (no `window.prompt`) and **re-edited** on double-click (HELLO→WORLD). lint **0 errors** · **563 tests** · build green.
- **B289 (drop a PDF onto the open document)** is the one item not in the headless harness — a synthetic HTML5 file-drop with a real `File` is awkward to dispatch — but it's a two-line reuse of the already-proven `openFile` path (the same path the file input + the empty-state drop use), so it's covered by code + those existing checks. **B291 (one-decimal linear measures)** is a pure-function change covered by a unit test (`test/takeoff.test.js`).
