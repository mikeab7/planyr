...
### V55 — Print now downloads a Planyr-composed PDF (no browser print dialog): white bg, exact page size, no injected chrome (B228) ✅
...
- **Result ✅ (all checks passed):**
  ...
  - **+10 unit tests** (`test/imagePdf.test.js`: byte-accurate xref, MediaBox in points, DCTDecode embed, title escaping). lint **0** · **446 tests** · build green; the existing live print harness (`verify-print-live.mjs`) still passes with the renamed menu item; `SitePlannerApp` lazy chunk intact.
