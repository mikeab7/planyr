...
### V96 — Invisible site-name fixed + whole-app low-contrast sweep (B360) ✅ (self-verified headless BOTH themes — no signed-in check needed)
...
- **⛔ 2026-06-21 (B360) — the real-sheet accuracy run is BLOCKED on a Drive re-auth, not a browser.** The owner's training corpus (michael@planyr.io › Planyr › Training Files) is unreadable: the Claude Code Drive connector is still on the wrong account (5 empty reads incl. `list_recent_files` + `owner='me'`). A **scoring harness is built and waiting** — `node ui-audit/score-filing.mjs` scores `readTitleBlockText`+`matchProjectInText`+`parseSheetScale` against each filename's ground truth; drop the extracted text into `ui-audit/corpus/<name>.txt` once the connector is re-authed. Until then this stays ⏳ on accuracy (logic is green: 1066 unit tests).
...
- **✅ 2026-06-21 (B360) — fixed a real `parseSheetScale` miss:** an architectural scale was read as **null whenever a number printed just before it** (e.g. a date next to the scale, `…SHEET A-201  10/24/2025  1/8"=1'-0"`) — the mixed-number branch swallowed the date digits + fraction, failed the sanity range, and never tried the real fraction. Fixed + unit-locked (`test/sheetScale.test.js`). Pure-logic (no browser needed); folds into the B360 corpus run for a live look on the owner's real arch sheets.
