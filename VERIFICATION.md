...
## 🔲 Needs verification
### V129 — B439/B440: rename + delete projects from the breadcrumb switcher ✅ Site path headless 7/7 + 4 unit tests + lint 0 + 1416 tests + build; ⏳ one signed-in **Schedule** module click-through
...
- **Why ⏳.** The real gov-host opaque-response caching + the instant-paint/outage-survival win can only be confirmed against **production** (the sandbox can't reach those hosts). After ship: open a site with layers on, reload → layers paint from cache; toggle a layer with the network throttled/offline → last-good still shows. Screening-only (no precise age badge yet — follow-up). Cadence: once after ship. Last checked: sandbox green.
