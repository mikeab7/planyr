...
## 🔲 Needs verification
### V84 — Markup canvas pan/zoom parity with the Site map (shared engine, B320) + tools moved to a right-side rail (B321) ✅ (fully self-verified headless — no signed-in check needed)
...
- **✅ B321 self-verified (`ui-audit/verify-b321.mjs`, 11/11):** all 10 tools live in the right rail (none left in the header), the rail sits right of the canvas + left of the Takeoff, **Select** is active by default on the Markup accent **#EF9F27**, clicking a tool activates it + the hint strip follows, the rail's zoom controls work, the Takeoff panel collapses + re-opens. Screenshot `ui-audit/screens/b321-markup-rail.png`.
...
- **Note:** touchscreen two-finger pinch isn't wired yet (matches the Site map, which also has none) — filed as **B322** (in flight this session); desktop trackpad pinch (ctrl+wheel) routes through the wheel zoom path and works.
