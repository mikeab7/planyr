 ...
 *      history, contacts, automation, settings). B562 — the floppy "Save" button was REMOVED
...
const parentCtl = () => page.evaluate(() => {
  ...
  return {
    ...
    // B562 — save status is now the shared Row-1 cloud badge, not a floppy in the lifted toolbar.
...
try {
  ...
  ok("cloud sync badge renders in the shell header (Row-1) — replaces the floppy Save button (B562)", c.cloudBadge);
  ok("the old floppy 'Save status' button is GONE from the lifted toolbar (B562)", c.floppyGone);
