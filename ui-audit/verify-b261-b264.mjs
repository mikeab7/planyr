/* Self-verification for B261–B264 (the "deliberate Group" tranche):
 *   B261 — explicit Group tool: select ≥2 → Group → they move/select/copy as one unit;
 *          double-click a member drills in to edit it; Group/Ungroup commands.
 *   B262 — snap is pure positional ALIGNMENT only; dragging never bonds elements.
 *   B263 — snap defaults OFF each session (sessionStorage), not globally sticky.
 *   B264 — per-plan delete (inline confirm), never the last plan.
...
/* ============================ PHASE 1 — Group / no-bond / snap ============================ */
{
  ...
  await assertMeasurable(page, "verify-b261-b264");
  const errors = [];
  ...
  // --- B263: snap defaults OFF this session + old global key retired ---
  ...
  log(/Snap off/.test(snapInit.btn) && !snapInit.ls, `B263 snap defaults OFF, no global localStorage key (btn="${snapInit.btn}", ls=${snapInit.ls})`);
  ...
  log(/on/.test(snapOn.btn) && snapOn.ss === "1", `B263 toggling snap persists to sessionStorage only (btn="${snapOn.btn}", ss=${snapOn.ss})`);

  // --- B262: a plain drag flush onto a neighbour must NOT bond them ---
  await fit();
  ...
  log(parkMoved < 8, `B262 flush drag did NOT bond — parking stayed put while building moved away (parking Δ=${parkMoved.toFixed(1)}px)`);

  // --- B261: Group two elements → move as one unit → ungroup ---
  await fit();
  ...
  log(!!grpBtn, `B261 Group command available with 2 selected (clicked "${grpBtn}")`);
  log(await hasGroupBox(), "B261 group bounding-box (⊞ Group) renders after grouping");
  ...
  log(boxRects === 1, `B261 group box has NO resize handles (just the outline; rects=${boxRects}) — a group never scales as a whole`);
  ...
  log(bMoved > 30 && pMoved > 30, `B261 dragging one member moved the WHOLE group (building Δ=${bMoved.toFixed(0)}px, parking Δ=${pMoved.toFixed(0)}px)`);
  ...
  log(!!unBtn && !(await hasGroupBox()), `B261 Ungroup removes the group box (clicked "${unBtn}")`);
  await page.keyboard.press("Escape"); await page.waitForTimeout(150);
  ...
  log(Math.hypot(up1.x - up0.x, up1.y - up0.y) < 8, "B261 after Ungroup, members move independently again");

  await page.screenshot({ path: OUT + "b261-groups.png" });
  console.log(errors.length ? `PHASE1 page errors:\n${errors.slice(0, 6).join("\n")}` : "(phase1: no page errors)");
  if (errors.length) fail++;
  await ctx.close();
}

/* ============================ PHASE 2 — per-plan delete (B264) ============================ */
{
  const ctx = await browser.newContext({ viewport: { width: 1440, height: 900 }, deviceScaleFactor: 1, ignoreHTTPSErrors: true });
  ...
  log(rows0 >= 2, `B264 two plans listed in the Plan menu (${rows0})`);
  ...
  log(armed && hasConfirm, `B264 ✕ shows an inline "Delete …?" confirm (no browser dialog)`);
  ...
  log(rows1 === rows0 - 1, `B264 a single plan was deleted (rows ${rows0} → ${rows1}); the app stayed alive`);
  ...
  log(!lastHasX, "B264 the last remaining plan has no delete affordance (can't delete the only plan)");

  await page.screenshot({ path: OUT + "b264-plan-delete.png" });
