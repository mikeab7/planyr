/* Self-verification for B315 — Undo (Ctrl+Z) after moving a building.
...
await assertMeasurable(page, "verify-b315");
...
await page.screenshot({ path: OUT + "b315-undo.png" });
console.log(errors.length ? `page errors:\n${errors.slice(0, 8).join("\n")}` : "(no page errors)");
if (errors.length) fail++;
await ctx.close();
await browser.close();
console.log(fail === 0 ? "\n✓ ALL B315 CHECKS PASSED" : `\n✗ ${fail} CHECK(S) FAILED`);
