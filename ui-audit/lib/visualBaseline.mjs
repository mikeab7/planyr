...
export function buildStatusMarkdown({ manifest, noiseFloor, addedCiTimeNote }) {
  ...
  const lines = [
    ...
    "5. **Read the triage comment the workflow posts on the PR before trusting the result.** Moving " +
      "the render to CI does not remove the human judgment call a local `--approve` always relied on " +
      "— it relocates it. Every run posts (or, with no open PR yet, writes to the job summary) the " +
      "FULL list of every surface/theme/viewport it touched, changed or not, each with its diff " +
      "magnitude against the prior committed baseline. A changed baseline on a surface your PR's own " +
      "diff never touches is not noise — it's evidence of a ripple wider than intended, and it's a " +
      "stop-and-report, not something to wave through because the workflow ran clean.",
    "",
