/* Verify the Document-Review editing batch B303–B307 against the REAL built app.
 *   B303 undo/redo (Ctrl-Z / Ctrl-Shift-Z / Ctrl-Y) over markups + calibration
 *   B304 manual Calibrate validates input (rejects "1/8" / "1:240"; accepts feet)
 *   B305 toolbar buttons don't wrap labels mid-word (white-space:nowrap, 1 line)
 *   B306 sheet paging — Prev/Next buttons + ← / → keyboard
 *   B307 measurement label sits at the path midpoint, not the first vertex
 *
 * Run:  npm run build && npx vite preview --port 4173   (one shell)
 *       node ui-audit/make-sample-pdf.mjs                (creates /tmp/samples/sample.pdf)
 *       node ui-audit/verify-b303-b307.mjs               (another shell)
 */
...
await assertMeasurable(page, "verify-b303-b307");
const pageErrors = [];
...
/* ---------- B305 — toolbar labels don't wrap ---------- */
...
check("B305 toolbar buttons are single-line (nowrap, ≤34px)", tbOk, JSON.stringify(tbBtns));
...
check("B306 Next button advances the sheet", start === "1 / 4" && afterNext === "2 / 4", `${start} → ${afterNext}`);
check("B306 → / ← keys page the sheets", afterArrowR === "3 / 4" && afterArrowL === "2 / 4", `→ ${afterArrowR}, ← ${afterArrowL}`);
...
check("B303 undo disabled before any edit", undoDisabledAtStart === true, `disabled=${undoDisabledAtStart}`);
check("B303 draw → undo removes it", afterDraw === 1 && afterUndo === 0, `draw=${afterDraw}, undo=${afterUndo}`);
check("B303 redo (Ctrl-Shift-Z and Ctrl-Y) restores it", afterRedoShift === 1 && afterRedoY === 1, `shiftZ=${afterRedoShift}, ctrlY=${afterRedoY}`);
// leave the canvas clean (undo the rect) for the label test
await page.keyboard.press("Control+z");
await page.waitForTimeout(250);
check("B303 final undo leaves a clean sheet", (await rectCount()) === 0);

/* ---------- B304 — manual Calibrate validation ---------- */
...
check("B304 inline Calibrate box opens (no window.prompt)", inputAppeared === 1);
check("B304 rejects '1/8' with a message, stays uncalibrated", rejected >= 1 && stillUncal, `msg=${rejected}, uncal=${stillUncal}`);
check("B304 rejects '1:240' as a scale ratio", ratioMsg >= 1);
check("B304 accepts '100' → sheet calibrated, box closes", nowCalibrated && inputGone === 0, `cal=${nowCalibrated}, gone=${inputGone}`);

/* ---------- B307 — distance label at the midpoint, not pts[0] ---------- */
...
check("B307 distance label sits near the midpoint (~450), not pts[0] (~250)", midOk, `labelX=${labelX}`);

await page.screenshot({ path: new URL("./screens/b303-b307.png", import.meta.url).pathname });
