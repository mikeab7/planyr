/* Self-verification for B230–B234 (Site map finder).
 *   B232 — shared status tokens on pins + chips + list markers
 *   B233 — left-rail rework: chips-as-filters, type-to-filter, collapsible groups
 *   B234 — per-layer source-vintage stamp
 *   B230/B231 — address search recenters + selects parcel + info (UI wiring; the
...
const OUT = new URL("./screens/b230-b234/", import.meta.url).pathname;
mkdirSync(OUT, { recursive: true });
const EXEC = process.env.PW_CHROME || "/opt/pw-browsers/chromium-1194/chrome-linux/chrome";

// Token colors we expect to see (B232).
...
async function run() {
  ...
  // ---- B232: pins paint in the status-token colors ----
  ...
  ok("B232 active pin = green #639922", allCols.includes(TOK.active.toLowerCase()));
  ok("B232 pursuit pin = blue #378ADD (hollow stroke)", allCols.includes(TOK.pursuit.toLowerCase()));
  ok("B232 onhold pin = amber #BA7517", allCols.includes(TOK.onhold.toLowerCase()));
  ok("B232 complete pin = gray #888780", allCols.includes(TOK.complete.toLowerCase()));
  ok("B232 dead pin = red #E24B4A", allCols.includes(TOK.dead.toLowerCase()));
  // module accent colors must NOT appear as a pin (the bug B232 closes)
  ok("B232 module amber #EF9F27 NOT used as a pin", !allCols.includes("#ef9f27"));
  ok("B232 module teal #1D9E75 NOT used as a pin", !allCols.includes("#1d9e75"));

  // ---- B233: type-to-filter exists; collapsible groups; chips filter ----
  const nameInput = page.locator('input[placeholder="Filter by name…"]');
  ok("B233 type-to-filter input present", (await nameInput.count()) === 1);
  ...
  ok("B233 active row visible by default", await rowVisible("Katy Active One"));
  ...
  ok("B233 Complete group collapsed by default (row hidden)", !completeRowVisibleBefore);
  ...
  ok("B233 expanding Complete reveals its rows", completeRowVisibleAfter);
  ...
  ok("B233 type-to-filter hides non-matches", !katyAfterFilter);
  ok("B233 type-to-filter surfaces a match even in a collapsed group", tomballAfterFilter);
  ...
  ok("B233 chip filter narrows the map pins", pinsAfter > 0 && pinsAfter < pinsBefore, `${pinsBefore} → ${pinsAfter}`);
  ...
  // ---- B234: layer source-vintage stamp ----
  ...
  ok("B234 vintage 'as of:' stamp appears when a layer is on", asOfCount >= 1, `${asOfCount} stamps`);
  const asOfText = asOfCount ? await page.getByText(/as of:/i).first().innerText() : "";
  ok("B234 vintage stamp shows the FIRM-panel vintage (honest, not a fake date)", /FIRM panel/i.test(asOfText), asOfText);
  await page.screenshot({ path: OUT + "layer-vintage.png" });

  // ---- B230/B231: address search wiring ----
  const search = page.locator('input[placeholder*="Find a site"]');
  ok("B230 address search input present", (await search.count()) === 1);
  const goBtn = page.getByRole("button", { name: /^Go$/ });
  ok("B230 Go button present", (await goBtn.count()) >= 1);
  ...
  ok("B230/B231 Go produced an honest result (info card or error), not a silent no-op", (infoCard + errToast) >= 1,
