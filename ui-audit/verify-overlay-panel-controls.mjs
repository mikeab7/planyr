/* Verify the overlay-panel control refinements (B560–B564), logged-out, on the built app.
 *
 *  B560 (NEW-1): the hide / lock / remove header buttons render at IDENTICAL sizes (one shared
 *    square icon-button style + inline SVG icons — no more emoji-vs-SVG metric mismatch).
 *  B561 (NEW-2): a numeric percent input sits beside the opacity slider, two-way bound + clamped 10–100%.
 *  B562 (NEW-3): the scale control is a Bluebeam-style preset dropdown; picking "Custom…" reveals
 *    editable [page][unit] = [real][unit] fields; 0.5" = 60' resolves to 1"=120' (impossible before).
 *  B563 (NEW-4): the Width row, the Reset-rotation button, and the obsolete drop-hint text are gone.
 *  B564 (NEW-5): the filename has its OWN full-width row and WRAPS (no mid-name ellipsis truncation).
 *
...
// ---- B560: the three header icon buttons render at identical sizes ----
...
check("B560 — all three header buttons present (hide/lock/remove)", haveAll, JSON.stringify(boxes));
if (haveAll) {
  ...
  check("B560 — each header button is square (w≈h)", square);
  check("B560 — all three header buttons are the SAME size", sameW && sameH);
}

// ---- B564: filename on its own full-width row, wrapping (taller than a single line) ----
...
check("B564 — filename row found", !!fileRow, JSON.stringify(fileRow));
if (fileRow) {
  check("B564 — filename takes ~the full panel width", fileRow.w >= fileRow.cardW - 24, `name=${fileRow.w}px card=${fileRow.cardW}px`);
  check("B564 — filename sits ABOVE the action buttons (own row)", fileRow.bottom <= fileRow.hideTop + 2, `nameBottom=${fileRow.bottom} hideTop=${fileRow.hideTop}`);
  check("B564 — long name WRAPS to multiple lines (not single-line truncation)", fileRow.h >= 34, `height=${fileRow.h}px`);
}

// ---- B561: opacity numeric input, two-way bound + clamped ----
const pct = page.locator('[data-testid="overlay-opacity-pct"]');
check("B561 — opacity percent input present", await pct.count() === 1);
check("B561 — shows 85% for stored opacity 0.85", (await pct.inputValue()) === "85", `value=${await pct.inputValue()}`);
await pct.fill("40"); await page.waitForTimeout(250);
check("B561 — typing 40 sets the overlay to ~0.40 opacity (live)", Math.abs((await overlayImgOpacity()) - 0.4) < 0.02, `opacity=${await overlayImgOpacity()}`);
await pct.fill("5"); await pct.blur(); await page.waitForTimeout(250);
check("B561 — below-range 5 clamps to 10% (opacity 0.1)", Math.abs((await overlayImgOpacity()) - 0.1) < 0.02, `opacity=${await overlayImgOpacity()}`);
check("B561 — field re-displays the clamped 10 after blur", (await pct.inputValue()) === "10", `value=${await pct.inputValue()}`);
// restore a sane opacity for the screenshot
await pct.fill("85"); await pct.blur(); await page.waitForTimeout(150);

// ---- B562: scale preset dropdown + Custom paired fields ----
const presetSel = page.locator('[data-testid="overlay-scale-preset"]');
check("B562 — scale preset dropdown present", await presetSel.count() === 1);
check("B562 — round-trips to eng-30 (1\"=30') for the seeded size", (await presetSel.inputValue()) === "eng-30", `value=${await presetSel.inputValue()}`);
// pick an architectural preset → 1/4" = 1' → 4 ft/in
await presetSel.selectOption("arch-1-4"); await page.waitForTimeout(250);
check("B562 — selecting 1/4\"=1'-0\" applies 1\"=4'", (await panelText()).includes("1″=4′"), "(arch preset)");
...
check("B562 — picking Custom… reveals the paired page=real fields", await custom.count() === 1);
...
check("B562 — 0.5\" = 60' resolves to 1\"=120' (the impossible-before scale)", afterText.includes("1″=120′"), afterText.includes("1″=120′") ? "" : "no 1″=120′ in panel");
// width feet = imgW(2592) * ftPerPx(120/72=1.6667) = 4320
check("B562 — sheet now reads ~4320' wide at that scale", afterText.includes("4320′ wide"), afterText.includes("4320′ wide") ? "" : "no 4320′ wide");
// a fraction works too: 1/2 → same 120
await pageInput.fill("1/2"); await page.waitForTimeout(120);
await realInput.fill("60"); await realInput.press("Enter"); await page.waitForTimeout(300);
check("B562 — fraction '1/2' in the page field also resolves to 1\"=120'", (await panelText()).includes("1″=120′"));
await page.screenshot({ path: OUT + "overlay-panel-scale-custom.png" });

// ---- B563: removed cruft is gone ----
