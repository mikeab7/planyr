/* Verify the overlay-panel control refinements (B566–B570), logged-out, on the built app.
 *
 *  B566 (NEW-1): the hide / lock / remove header buttons render at IDENTICAL sizes (one shared
 *    square icon-button style + inline SVG icons — no more emoji-vs-SVG metric mismatch).
 *  B567 (NEW-2): a numeric percent input sits beside the opacity slider, two-way bound + clamped 10–100%.
 *  B568 (NEW-3): the scale control is a Bluebeam-style preset dropdown; picking "Custom…" reveals
 *    editable [page][unit] = [real][unit] fields; 0.5" = 60' resolves to 1"=120' (impossible before).
 *  B569 (NEW-4): the Width row, the Reset-rotation button, and the obsolete drop-hint text are gone.
 *  B570 (NEW-5): the filename has its OWN full-width row and WRAPS (no mid-name ellipsis truncation).
 *
...
// ---- B566: the three header icon buttons render at identical sizes ----
...
check("B566 — all three header buttons present (hide/lock/remove)", haveAll, JSON.stringify(boxes));
if (haveAll) {
  ...
  check("B566 — each header button is square (w≈h)", square);
  check("B566 — all three header buttons are the SAME size", sameW && sameH);
}

// ---- B570: filename on its own full-width row, wrapping (taller than a single line) ----
...
check("B570 — filename row found", !!fileRow, JSON.stringify(fileRow));
if (fileRow) {
  check("B570 — filename takes ~the full panel width", fileRow.w >= fileRow.cardW - 24, `name=${fileRow.w}px card=${fileRow.cardW}px`);
  check("B570 — filename sits ABOVE the action buttons (own row)", fileRow.bottom <= fileRow.hideTop + 2, `nameBottom=${fileRow.bottom} hideTop=${fileRow.hideTop}`);
  check("B570 — long name WRAPS to multiple lines (not single-line truncation)", fileRow.h >= 34, `height=${fileRow.h}px`);
}

// ---- B567: opacity numeric input, two-way bound + clamped ----
const pct = page.locator('[data-testid="overlay-opacity-pct"]');
check("B567 — opacity percent input present", await pct.count() === 1);
check("B567 — shows 85% for stored opacity 0.85", (await pct.inputValue()) === "85", `value=${await pct.inputValue()}`);
await pct.fill("40"); await page.waitForTimeout(250);
check("B567 — typing 40 sets the overlay to ~0.40 opacity (live)", Math.abs((await overlayImgOpacity()) - 0.4) < 0.02, `opacity=${await overlayImgOpacity()}`);
await pct.fill("5"); await pct.blur(); await page.waitForTimeout(250);
check("B567 — below-range 5 clamps to 10% (opacity 0.1)", Math.abs((await overlayImgOpacity()) - 0.1) < 0.02, `opacity=${await overlayImgOpacity()}`);
check("B567 — field re-displays the clamped 10 after blur", (await pct.inputValue()) === "10", `value=${await pct.inputValue()}`);
// restore a sane opacity for the screenshot
await pct.fill("85"); await pct.blur(); await page.waitForTimeout(150);

// ---- B568: scale preset dropdown + Custom paired fields ----
const presetSel = page.locator('[data-testid="overlay-scale-preset"]');
check("B568 — scale preset dropdown present", await presetSel.count() === 1);
check("B568 — round-trips to eng-30 (1\"=30') for the seeded size", (await presetSel.inputValue()) === "eng-30", `value=${await presetSel.inputValue()}`);
// pick an architectural preset → 1/4" = 1' → 4 ft/in
await presetSel.selectOption("arch-1-4"); await page.waitForTimeout(250);
check("B568 — selecting 1/4\"=1'-0\" applies 1\"=4'", (await panelText()).includes("1″=4′"), "(arch preset)");
...
check("B568 — picking Custom… reveals the paired page=real fields", await custom.count() === 1);
...
check("B568 — 0.5\" = 60' resolves to 1\"=120' (the impossible-before scale)", afterText.includes("1″=120′"), afterText.includes("1″=120′") ? "" : "no 1″=120′ in panel");
// width feet = imgW(2592) * ftPerPx(120/72=1.6667) = 4320
check("B568 — sheet now reads ~4320' wide at that scale", afterText.includes("4320′ wide"), afterText.includes("4320′ wide") ? "" : "no 4320′ wide");
// a fraction works too: 1/2 → same 120
await pageInput.fill("1/2"); await page.waitForTimeout(120);
await realInput.fill("60"); await realInput.press("Enter"); await page.waitForTimeout(300);
check("B568 — fraction '1/2' in the page field also resolves to 1\"=120'", (await panelText()).includes("1″=120′"));
await page.screenshot({ path: OUT + "overlay-panel-scale-custom.png" });

// ---- B569: removed cruft is gone ----
...
check("B569 — the Width row is gone for a SHEET/PDF overlay (scale picker owns sizing)", !cardText.includes("Width"), "(no Width control)");
check("B569 — the 'Reset rotation' button is gone", !cardText.includes("Reset rotation"));
check("B569 — the 'Drag it to move' drop-hint tail is gone", !body.includes("Drag it to move"));
check("B569 — the 'Sizing to the drawing scale comes next' parenthetical is gone", !body.includes("Sizing to the drawing scale comes next"));
check("B569 — the 'Scale to the drawing — sizes the sheet' line is gone", !cardText.includes("Scale to the drawing"));
check("B569 — core drop hint kept", body.includes("White paper is knocked out"));
...
// ---- B569 fix (review finding): an IMAGE overlay (sheet:null) KEEPS its numeric Width control ----
...
// numeric size entry. So images must retain Width + the ±10% nudge.
{
  ...
  check("B569 fix — image overlay (no sheet) KEEPS the numeric Width control", ct.includes("Width"));
  check("B569 fix — image overlay shows NO scale preset picker (no physical inch)", await p.locator('[data-testid="overlay-scale-preset"]').count() === 0);
  check("B569 fix — image overlay still has the opacity percent field", await p.locator('[data-testid="overlay-opacity-pct"]').count() === 1);
  await p.screenshot({ path: OUT + "overlay-panel-image.png" });
  await c.close();
}

// ---- B568 fix (review finding): an idle focus→blur on the custom REAL field must NOT quantize ----
...
// scale exactly where it was (per-field dirty guard) — pre-fix this re-committed "3.3" and drifted ~0.6%.
{
  ...
  check("B568 fix — non-round metric scale renders ~118' wide before the idle blur", before === "118", `before=${before}`);
  ...
  check("B568 fix — idle focus+blur on the custom real field does NOT quantize the scale", before === after, `before=${before} after=${after}`);
  await c.close();
}
