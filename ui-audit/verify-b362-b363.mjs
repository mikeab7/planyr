/* LIVE verify for B362 + B363 (bump-out resize persistence + bonded-child rotation repair).
 *
 ...
 * Resumes into the planner (so loadSite → createSiteModel runs the B363 repair on the render
 ...
 *   B363 — every drifted child's rot snapped to the host's 0° (re-anchored), the correct child
 *          is untouched, and the assembly is no longer ~1° off the building;
 *   B362 — the user-sized bump-out KEPT along/proj 80×70 across the load/save round-trip (it
...
(async () => {
  ...
  await assertMeasurable(page, "verify-b362-b363");
  const errs = [];
  const isSeedNoise = (t) =>
  ...
  try {
    ...
    // B363 — every drifted child snapped to the host's 0°
    const drifted = ["SW", "TC", "BL", "BR"];
    const rots = drifted.map((eid) => norm(by(eid).rot));
    const allStraight = rots.every((r) => Math.min(r, 360 - r) < 0.01);
    ok("B363-children-resynced", allStraight, `drifted children rots → [${rots.map((r) => r.toFixed(3)).join(", ")}] (want all ≈ 0°)`);

    // the correctly-bonded child kept its angle (and wasn't shoved around)
    const okChild = by("OK");
    ok("B363-correct-child-untouched", norm(okChild.rot) === 0 && Math.abs(okChild.cx - (-152.5)) < 0.01 && Math.abs(okChild.cy) < 0.01,
      `correct child rot=${norm(okChild.rot)} cx=${okChild.cx.toFixed(2)} cy=${okChild.cy.toFixed(2)} (want 0 / -152.5 / 0)`);

    // B362 — the user-sized bump KEPT its 80×70, default bump stayed 55×60
    const br = by("BR"), bl = by("BL");
    ok("B362-user-size-persists", br.dogEar.along === 80 && br.dogEar.proj === 70,
      `right bump dogEar={along:${br.dogEar.along}, proj:${br.dogEar.proj}} (want 80/70, NOT reset to 55/60)`);
    ok("B362-default-bump-unchanged", bl.dogEar.along == null && bl.dogEar.proj == null,
