/* Verify B196 (inactive parcel inherits active state) + B197 (edge-run setbacks) +
 * B198 (fanned, single-per-side dimensions).
 *
 ...
 *
 * Checks:
 *  B196 — only the ACTIVE parcel draws a setback line + acreage chip; an easement
 *         anchored to the inactive parcel is hidden, a free easement still shows.
 *  B197 — selecting the parcel shows ONE setback pill per SIDE (4 runs, not 6 edges);
 *         "Per segment" toggles to one pill per edge (6).
 *  B198 — the run length dimension + the setback pill are fanned (different positions),
...
// Inactive parcel offset to the east — would draw a setback + chip if not for B196.
...
// ── B196: active-state inheritance (no parcel selected) ──
...
let c = await counts();
ok("B196 — exactly ONE acreage chip (inactive parcel's chip hidden)", c.chips === 1, `chips=${c.chips}`);
ok("B196 — exactly ONE setback line (inactive parcel draws none)", c.setbackLines === 1, `setbackLines=${c.setbackLines}`);
ok("B196 — easement on the inactive parcel hidden; the free one shows (1)", c.easements === 1, `easementPolys=${c.easements}`);

// ── B197/B198: select the active parcel → per-SIDE setback pills (runs, not edges) ──
...
c = await counts();
ok("B197 — ONE setback pill per SIDE: 4 runs (S, E-run, N, W), not 6 edges", c.sbPills === 4, `pills=${c.sbPills}`);

// Toggle to per-segment → one pill per edge (6).
await page.locator('button:has-text("Per segment")').click();
await page.waitForTimeout(500);
c = await counts();
ok("B197 — 'Per segment' override exposes one pill per edge (6)", c.sbPills === 6, `pills=${c.sbPills}`);
...
await page.waitForTimeout(400);
c = await counts();
ok("B197 — toggling back to 'By side' returns to 4 pills", c.sbPills === 4, `pills=${c.sbPills}`);

// ── B198: the run-length dim (outboard) and setback pill (inboard) are fanned apart ──
...
ok("B198 — boundary dim and setback pill are fanned apart (not stacked)", fan.minGap >= 12 && fan.dims >= 4, JSON.stringify(fan));
