...
async function openAndCheck(browser, { county, fips, countyName, wse100, wse100Status }) {
  ...
  await assertMeasurable(page, "verify-b807-zonea-draft100");
  ...
  // B824 — the card merged into Yield → Stormwater: the detail text now lives HERE
  // (expanded above); Site Analysis carries only a screening link row.
...
async function run() {
  ...
  console.log("· Fort Bend unstudied Zone A / FBCDD 100YR = 96.0 (priced + DRAFT labels + FFE basis + PDF parity)");
  {
    ...
    // B823 — the loud note is ONE line now; the "read from … rasters" teaching copy rides its ⓘ title.
