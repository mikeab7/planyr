...
|---|---|---|---|---|
| Surfaces | `--surface-page` / `--surface-raised` / `--surface-overlay` / `--surface-base` | `#F3F5F8` / `#FFFFFF` / `rgba(255,255,255,.94)` / `#F3F5F8` | `#14161B` / `#1D2027` / `rgba(24,27,33,.93)` / `#14161B` | `-overlay` is the frosted floating-panel-over-the-map surface. `-base` is the opaque field/input surface (an alias of `-page`) — use it for a text input/select sitting inside a raised panel; it was referenced by several components before being defined (B986096-HARDENING-25), which rendered those inputs transparent. |
