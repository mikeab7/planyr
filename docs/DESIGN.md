...
|---|---|---|---|---|
| Surfaces | `--surface-page` / `--surface-raised` / `--surface-overlay` / `--surface-base` | `#E7EBF0` / `#FFFFFF` / `rgba(255,255,255,.94)` / `#E7EBF0` | `#0A0C0F` / `#1D2027` / `rgba(24,27,33,.93)` / `#0A0C0F` | `-overlay` is the frosted floating-panel-over-the-map surface. `-base` is the opaque field/input surface (an alias of `-page`) — use it for a text input/select sitting inside a raised panel; it was referenced by several components before being defined (B986096-HARDENING-25), which rendered those inputs transparent. `-page` was moved darker (Model Round 4) so a raised card reads against it without a shadow doing all the work — see that token's own header comment in index.css. |
...
| Semantic text | `--success-text` / `--danger-text` / `--info-text` | `#12722F` / `#B3361B` / `#1D4ED8` | `#4FBF7B` / `#F2706F` / `#6FB4F0` | Colored labels on themed panels. A hardcoded hex reads fine on a light card and fails AA on a dark one — this is exactly the class of bug the CI guard exists to catch. `--success-text` was darkened one notch (Model Round 4) to keep clearing AA on `--surface-page` after that token moved darker. |
