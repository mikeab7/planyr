...
const NOTES_ITEMS = [
  ...
  item({ id: "notes.zoom-in", label: "Zoom in", combo: ["mod", "+"], note: "Or Ctrl/⌘ + scroll wheel — zooms at the pointer.",
    evidence: [{ file: "src/workspaces/notes/lib/notesViewport.js", pattern: 'key === "=" || key === "+" || key === "Add"' }] }),
  item({ id: "notes.zoom-out", label: "Zoom out", combo: ["mod", "-"],
    evidence: [{ file: "src/workspaces/notes/lib/notesViewport.js", pattern: 'key === "-" || key === "_" || key === "Subtract"' }] }),
  item({ id: "notes.zoom-reset", label: "Reset zoom to 100%", combo: ["mod", "0"],
    evidence: [{ file: "src/workspaces/notes/lib/notesViewport.js", pattern: 'key === "0") return { kind: "reset" }' }] }),
  item({ id: "notes.zoom-fit", label: "Fit the whole page on screen", combo: ["mod", "9"],
    evidence: [{ file: "src/workspaces/notes/lib/notesViewport.js", pattern: 'key === "9") return { kind: "fit" }' }] }),
