...
function RangeControl({ value, min, max, step, onChange }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
      <input type="range" value={value ?? 0} min={min ?? 0} max={max ?? 1} step={step ?? 0.05}
...
function EnumControl({ value, options, onChange }) {
  return (
          ...
          <div key={key} style={{ padding: "6px 12px", borderBottom: `1px solid ${B}` }}>
            {type === "bool" ? (
              ...
              <>
                ...
                {type === "color"  && <ColorControl value={value} onChange={emit} />}
                {type === "number" && <NumberControl value={value} min={min} max={max} onChange={emit} />}
                {type === "range"  && <RangeControl value={value} min={min} max={max} step={step} onChange={emit} />}
                {type === "enum"   && <EnumControl value={value} options={options} onChange={emit} />}
              </>
            )}
          </div>
        );
