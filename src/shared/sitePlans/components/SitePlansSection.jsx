...
export default function SitePlansSection({
  ...
  // B1134753 — SERIALIZE placement commits per overlay id, and track each overlay's version
  // OUTSIDE React state/render timing. Before this, TWO commits fired close together (a
  // corner-scale released, then immediately a rotate) could both read `existing.version` off
  // `overlaysRef.current` before EITHER's async round trip had updated it — both went out
  // carrying the SAME expected version, the first to land won, and the second came back a
  // false "someone else changed this site plan" even though it was the very same user's own
  // very next gesture. (Measured on the owner's real row: version 18 after four minutes of
  // ordinary corner/rotate nudging — far more commits than four minutes of deliberate releases
  // should produce.) `pendingCommitRef` chains same-id commits so each one's write only starts
  // once the PRIOR one has fully settled; `versionRef` records the version each settle actually
  // produced, read synchronously within that same chain link — never through `overlaysRef`,
  // whose update via `setOverlays` is scheduled, not synchronous, so relying on it here would
  // just move the same race one layer down.
  const pendingCommitRef = useRef(new Map()); // overlay id -> promise chain tail
  const versionRef = useRef(new Map()); // overlay id -> last known-good version
  ...
  const doCommitPlacement = async (id, placement) => {
    const existing = overlaysRef.current.find((o) => o.id === id);
    if (!existing) return;
    const knownVersion = versionRef.current.has(id) ? versionRef.current.get(id) : existing.version;
    const next = { ...existing, ...placement, version: knownVersion };
    ...
    const { version: newVersion, conflict, error } = await commitOverlayPlacementWithComps(id, next, compPositions, knownVersion);
    if (error) {
      console.error("[sitePlanOverlays] placement commit failed:", error);
      setPanelError(friendlySaveError(error));
      const fresh = await reload(); // the optimistic move didn't actually save — pull the real, current position back
      const row = fresh && fresh.find((o) => o.id === id);
      if (row) versionRef.current.set(id, row.version); else versionRef.current.delete(id);
    } else if (conflict) {
      ...
      if (Number.isFinite(newVersion)) {
        versionRef.current.set(id, newVersion);
  ...
  // The one entry point every caller uses (the map's onCommitPlacement, and the panel's own
  // typed-rotation field below) — chains onto whatever commit for this SAME overlay is already
  // in flight, so two commits for the same overlay never race each other's version guard.
  // Commits for DIFFERENT overlays are unaffected (a separate chain per id).
  const commitPlacement = (id, placement) => {
    const prior = pendingCommitRef.current.get(id) || Promise.resolve();
    const run = prior.then(() => doCommitPlacement(id, placement)).catch((e) => console.error("[sitePlanOverlays] queued placement commit failed:", e));
    pendingCommitRef.current.set(id, run);
    return run;
  };
