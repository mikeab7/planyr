...
export default function Stitcher({ onReview, loadReq = null, onConsumeLoad, onOpenReview, signedIn = false }) {
  ...
  const [calInput, setCalInput] = useState(null); // inline Calibrate entry { pts:[world], x, y (screen px), value } (B284)
  const [busy, setBusy] = useState(false);
  const [err, setErr] = useState("");
  const drag = useRef(null);

  /* ---- undo / redo (B283) — measures + the composite calibration ---- */
  ...
  // world midpoint maps to screen px via the current pan/zoom. (B284)
  ...
  // Validate + apply the composite calibration; reject ratios/junk with a message (B284).
          ...
          <svg ref={svgRef} width="100%" height="100%" style={{ display: "block", cursor: align ? "crosshair" : tool === "pan" ? "grab" : "crosshair", touchAction: "none" }}
            ...
            <g transform={G}>
              ...
              {measures.map((m) => {
                ...
                const c = centroidOf(m.pts); // area-weighted centroid, clamped inside concave shapes (B287)
