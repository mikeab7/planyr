...
export default function Stitcher({ onReview, loadReq = null, onConsumeLoad, onOpenReview, signedIn = false }) {
  ...
  const addSheet = async (pdf, pageNum) => {
    ...
    try {
      ...
      setPlaced((arr) => {
        ...
        // Align before its measurements can be trusted against the shared scale (B301).
  ...
  const onDown = (e) => {
    ...
    if (align) {
      ...
      if (align.step === 0) setAlign({ ...align, step: 1, A1: w });
        ...
        // B300: reject a near-coincident baseline (two clicks on ~the same point, on either
    ...
    // B301: a measurement/calibration point on a not-yet-aligned sheet would be scored with
  ...
  const loadStitch = async (rec) => {
    ...
    try {
      ...
      for (const s of st.placed || []) {
        ...
        // Back-compat: reviews saved before B301 carry no `aligned` flag — treat them as
  ...
  const unalignedCount = placed.reduce((n, s, i) => n + (i > 0 && s.aligned !== true ? 1 : 0), 0); // B301
      ...
      <div style={{ flex: 1, display: "flex", minHeight: 0 }}>
          ...
          <svg ref={svgRef} width="100%" height="100%" style={{ display: "block", cursor: align ? "crosshair" : tool === "pan" ? "grab" : "crosshair", touchAction: "none" }}
            ...
            <g transform={G}>
              {placed.map((s, i) => {
                ...
                const needsAlign = i > 0 && s.aligned !== true && !aligning; // B301
        ...
        <div style={{ flex: "none", width: 220, background: "#fff", borderLeft: `1px solid ${PAL.line}`, overflowY: "auto", padding: 12, fontFamily: "system-ui, sans-serif" }}>
          ...
          {placed.map((s, i) => {
            ...
            const needsAlign = i > 0 && s.aligned !== true; // B301
            return (
