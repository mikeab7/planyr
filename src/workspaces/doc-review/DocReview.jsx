...
// Rail icons for the Markup tools + zoom controls (B326). 16×16, stroke = currentColor so a
...
export default function DocReview({ shellModule, onShellSwitch, authControl, onGoDashboard, onNewProject, docIntent = null } = {}) {
  ...
  // Viewport transform (B325): ONE shared pan/zoom model with the Site map. `view` is
  ...
  const [spaceHeld, setSpaceHeld] = useState(false); // hold-Space = temporary pan in any tool (B289/B325)
  ...
  const [loadNonce, setLoadNonce] = useState(0);    // bump to force a fresh fit on open / reset / load (B325)
  const viewRef = useRef(view); viewRef.current = view; // live view for the once-bound wheel handler
  const panRef = useRef(null);        // active pan drag { sx, sy, tx0, ty0 } (B325)
  ...
  const [takeoffOpen, setTakeoffOpen] = useState(true); // right-side Takeoff panel collapse (B326)
  ...
  const openFile = async (file) => {
    ...
    try {
      ...
      setPage(1);
      setView(null); setPageBase(null); setRenderScale(0); setLoadNonce((n) => n + 1); // fit the new backdrop (B325)
  ...
  // the PDF at full resolution when it stops. Panning (tx/ty only) never re-rasters. (B325)
  useEffect(() => {
  ...
  /* ---- zoom/pan viewport — the shared engine (B325) ---- */
  ...
  // Fit the current sheet to the viewport (Fit = width, Fit page = the whole sheet). (B295/B325)
  const fitNow = (mode) => {
    setFitMode(mode);
  ...
  // trackpad pinch all arrive here as wheel events and all zoom toward the cursor. (B325)
  ...
  const fetchSourceBytes = async (src, tok) => {
    ...
    setPdfDoc(pdf);
    setNumPages(pdf.numPages); setView(null); setPageBase(null); setRenderScale(0); setLoadNonce((n) => n + 1); // refit on load (B325)
  };
  ...
  const onDown = (e) => {
    ...
    // empty → pan; Select-on-object → select/move; a drawing tool → draw, never pan. (B325)
  ...
  const onMove = (e) => {
    if (panRef.current) { // panning: move the sheet with the drag, free in any direction (B325)
  ...
  const zoom = (f) => doZoom(f, null, null); // ± buttons zoom about the viewport centre (B290/B325)
  ...
  // Right-side tool rail (B326): the drawing/measure tools + zoom controls, Bluebeam-style.
  const railItems = [
    ...
    <div style={{ height: "100%", display: "flex", flexDirection: "column", background: PAL.paper, position: "relative" }}>
      ...
      <AppHeader
        ...
        toolbarContent={
          <>
            ...
            {/* Drawing/measure tools + zoom controls now live in the right-side tool rail (B326).
      ...
      {!pdfRef.current ? (
        ...
        <div style={{ flex: 1, display: "flex", minHeight: 0 }}>
          ...
          {/* canvas + overlay — a transform viewport (B325). The sheet is a page-sized box
          ...
          {/* tool rail (B326) — drawing/measure tools + zoom, flush to the canvas */}
          <ToolRail items={railItems} accent={MODULE_ACCENT["doc-review"]} data-testid="markup-rail" />

          {/* takeoff — collapsible (B326); a thin re-open tab when hidden */}
          {takeoffOpen ? (
