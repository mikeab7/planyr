...
--
-- No RLS change: the existing owner/team-admin UPDATE policies already govern who may stamp or
-- clear the column, and the DELETE policy still governs the final purge. The client degrades
-- gracefully if this migration hasn't run (missing-column -> the old immediate hard delete).
--
