...
export function buildAdjacency(sheets) {
  ...
  for (const a of sheets) {
    for (const ml of a.matchLines || []) {
      ...
      // B341 — CONTRADICTION guard. If B also names A but on a side that is NOT that opposite
...
// B341 — sheets in one real plan set are the SAME plot size, so a seam-to-seam fit should place a
...
export function autoPlaceGroup(sheets = []) {
  ...
  while (queue.length) {
    ...
    for (const link of adj.get(a.id) || []) {
      ...
      if (!scaleInBand(Mb)) continue; // B341 — size mismatch ⇒ would rescale wrongly; leave for manual Align
