...
// Per-tab `version` tokens for the optimistic-concurrency guard (B312), populated by
...
export async function upsertReview(record) {
  ...
  // Optimistic concurrency (B312), guarded by the version we last synced. TWO independent
...
export async function loadReview(id) {
  ...
  if (data.version != null) reviewVersions[id] = data.version; // remember it for the next save's CAS guard (B312)
...
export async function deleteReview(id) {
  ...
  delete reviewVersions[id]; // stop tracking a removed review's version (B312)
  const uid = await currentUid();
