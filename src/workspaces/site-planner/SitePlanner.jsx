...
// B221: the building feature-add buttons (+/− dock, sidewalk, parking, bump-out) are
...
export default function SitePlanner({ active = true, siteId = null, overlays, setOverlays, cloud = null, layerStatus = {}, setLayerStatus, onBackToMap, sites = [], onOpenSite, onNewSite, onNewPlanSameParcel, onDuplicateSite, onRenameSite, onRenamePlan, onSiteDropped, onSiteSaved, shellModule, onShellSwitch, authControl } = {}) {
  ...
  const [hoverElId, setHoverElId] = useState(null); // B222: building under the cursor (select mode, nothing selected) → preview its feature-add buttons
  ...
  const onMove = (e) => {
    ...
    if (!d) {
      // B222: when nothing is selected, preview the hovered building's feature-add
  ...
  // B221 + B222: the feature-add buttons render for exactly ONE building — the selected
  ...
  const sideAddNodes = (() => {
    ...
    const wpx = Math.abs(el.w) * view.ppf, hpx = Math.abs(el.h) * view.ppf; // B221: rendered footprint, px
    ...
    return (
      <g>
        {sides.map(([name, nx, ny]) => {
          // B221: an inset +/− needs its wall's PERPENDICULAR on-screen size to clear the
        ...
        {/* dog-ear bump-outs at each corner of every dock side — need room on BOTH axes (B221) */}
  ...
  const courtAddNodes = (() => {
    ...
    if (Math.min(Math.abs(el.w), Math.abs(el.h)) * view.ppf < FEAT_BTN_MIN_PX) return null; // B221: hide before it clusters
