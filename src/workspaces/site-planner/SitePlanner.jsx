...
// B300 — parcel click-vs-drag: a press on a (locked) parcel pans the canvas; only a brief,
...
const DEFAULT_SETTINGS = {
  ...
  parcelSelect: true,   // B301: ON = click a parcel to select it (drag still pans); OFF = pure browse/measure, a click never selects. Persisted per project.
...
export default function SitePlanner({ active = true, siteId = null, overlays, setOverlays, cloud = null, layerStatus = {}, setLayerStatus, onBackToMap, sites = [], onOpenSite, onNewSite, onNewPlanSameParcel, onDuplicateSite, onDeletePlan, onRenameSite, onRenamePlan, onSiteDropped, onSiteSaved, shellModule, onShellSwitch, onOpenReviewInDocReview, authControl } = {}) {
  ...
  const onUp = (e) => {
    ...
    // B300: a press that began on a LOCKED parcel pans the canvas; if the pointer barely moved
  ...
  const startMoveParcel = (e, id) => {
    ...
    // B301: "Select parcels" OFF → parcels are click-through for pure browse/measure. Don't
    ...
    // B300: a press on a LOCKED parcel starts a PAN, not a select — selecting on pointer-down
