...
export function slimForCloud(model) {
  ...
  // B692 — the sharing pointer never rides the stored jsonb: the `team_id` COLUMN is the single
...
// B692 — the column payload an ordinary content push sends (pure; exported for tests). The
...
export function keepaliveCloudPush(uid, model) {
  ...
  // (B692 — the keepalive is always an update; a stale teamId here could silently unshare).
