...
// compare-and-swap that REJECTS a stale write instead of silently clobbering (B274).
...
export async function cloudUpsert(uid, model) {
  ...
  // Optimistic concurrency (B274): a conditional write guarded by the version we last synced.
...
// so the next save can compare-and-swap against it (B274).
