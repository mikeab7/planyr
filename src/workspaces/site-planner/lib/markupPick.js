// markupPick.js — pure hit-testing + z-stack cycling for Site Planner markups (B915 / B916).
//
...
//
//   • B915 / NEW-1 — the ONE fill-aware rule the DOM render also follows: a CLOSED markup grabs by
...
//   • B916 / NEW-2 — repeat-click / Alt-click must CYCLE down through every markup under the pointer
...
// A markup's clickable geometry + whether its BODY captures (the B915 rule in ONE place, so the
// DOM render and this picker never diverge): { ring, path, closed, filled, area } | null.
//   - polygon/rect/ellipse — closed; `filled` follows fillOpacity > 0 (B915).
...
// null. Honours the B915 rule: an unfilled closed shape hits on its stroke only, never its interior.
export function markupUnderPoint(m, p, tol) {
  ...
  if (g.closed) {
    ...
    if (g.filled) {
      ...
      return { area: g.area }; // B915: unfilled → stroke + tolerance only
    }
    return null;
  }
  const path = g.path;
