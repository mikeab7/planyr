/* Deed / metes-and-bounds PARSER — pure, client-side (no API).
 *
 * Split out of `metesAndBounds.js` (which keeps the polyline offset/buffer primitives) so it can
 * be DEFERRED: parsing a legal description only ever happens once the user is already in the deed
 * workflow — dropping a .docx or pasting a description — and that workflow already loads its file
 * reader on demand (`shared/files/docxText.js`, B1141). Nothing on the boot path needs it, and the
 * planner's site route has no bundle headroom to spare, so it is reached through
 * `deedParseLazy.js` rather than a static import. **Do not "tidy" it back into a static import.**
 * The geometry primitives stayed behind because roads, easements and the KMZ export DO need them
