...
export function createExportSheet(ctx) {
  ...
  const exportKmz = (extrude = false) => {
    ...
    try {
      ...
      // Dimension lines off — clutter in a 3D walkthrough. Dock doors ALWAYS export (owner
      // decision) as one run per dock side, so there is no flag for them here to get wrong.
   ...
   *
   * NEW-2 — `includeMetricsBand` is the print menu's "Stats band" toggle (default true, so an
   * existing sheet renders unchanged unless the owner turns it off). Off: `printSheetLayout`
