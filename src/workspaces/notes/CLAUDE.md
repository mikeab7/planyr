...
- **`lib/notesBlankPaper.js` — IS THIS PRESS ON BLANK PAPER, OR ON SOMEBODY'S WRITING? (B1393 ×4, 2026-09-18.)** PURE. `pressPastLineEnd` takes a DOM `Range`'s **per-rendered-line** rectangles and answers whether a press landed past the end of the writing on **its own row**, with the slack being the line's OWN height; `isBlankDoublePress` reconstructs the press pair rather than trusting native `dblclick` alone (traps 31/32). **⛔ IT EXISTS BECAUSE `pressIsBesideLine` (in `components/NoteEditor.jsx`) TESTS ONLY THE VERTICAL AXIS** — correct for B1368's margin case, and the reason a double-click ten inches right of a five-inch line put the caret on the end of that line and typed into his sentence, four rounds running. **⛔ IT DELIBERATELY DOES NOT CHANGE THE SINGLE CLICK** (that would reopen B1368): only the second press of a pair, only past the row's writing, only on the right-hand side. Read `docs/NOTES-CARRY-FORWARD.md` §5 family 19 and trap 33 before touching either — trap 33 is why four correct fixes missed this. Guard: **verify-notes-in-sheet-placement** under `ui-audit/` (rendered position vs. click coordinates, real `page.mouse.dblclick()`, known-good arms that VOID the run).
