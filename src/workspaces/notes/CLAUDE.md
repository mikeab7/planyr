...
- **`lib/notesBlankPaper.js` — IS THIS THE SECOND PRESS OF A GENUINE DOUBLE-CLICK ON BLANK PAPER? (B1393 lineage, superseded by NEW-1, 2026-09-22.)** PURE. `isBlankDoublePress` reconstructs the press pair rather than trusting native `dblclick` alone (carry-forward traps 31/32) — this survives because the sandbox that runs every placement harness genuinely cannot always raise a native double-click from two separate down/up pairs. **⛔ `pressIsBesideLine`/`pressPastLineEnd` (the flow-text "beside a line" mechanism B1393's five rounds built, formerly in `components/NoteEditor.jsx`) ARE GONE, DELETED RATHER THAN LEFT UNUSED** — the owner removed flow text from the page entirely ("I just want the double-click thing"), so there is no line left for a press to be beside. Read `docs/NOTES-CARRY-FORWARD.md` §5 family 19 for the retired mechanism's own record before assuming it still exists. Guard: **verify-notes-in-sheet-placement** under `ui-audit/` (rendered position vs. click coordinates, real `page.mouse.dblclick()`, known-good arms that VOID the run) — rewritten for the boxes-only model in the same commit that retired the mechanism.
- **`lib/notesFlowMigration.js` — AN OLD PAGE'S FLOW BODY → ONE BOX, ON READ, NEVER ON WRITE (NEW-1, 2026-09-22).** PURE. `migrateFlowBody` bundles every top-level child that is not a `noteAnchor`/`noteSketch` (and is not the one trailing structural paragraph — see `notesAnchorNode.js`'s own header) into a single new box anchored at the sheet's top-left, in document order, leaving every existing anchor/sketch exactly where it is. Runs once, in `NoteEditor.jsx`'s `initialDoc` computation, before `useEditor` — never inside a transaction, so `hasUserInputRef` (B1662464) still gates the first save on a real edit. A boxes-only document is a no-op, same object reference.
- **`lib/notesReadingOrder.js` — TOP-LEVEL BOXES/SKETCHES, TOP TO BOTTOM THEN LEFT TO RIGHT (NEW-1, 2026-09-22).** PURE, zero dependencies. `sortTopLevelForReading` reorders a document's top-level array by each box's own `x`/`y` — used by the Markdown exporter (`notesMarkdown.js`) and the print/HTML exporter (`notesDocHtml.js`) so the exported SOURCE order matches how a person reads the page, even though a printed box still paints at its own absolute CSS position (PDF-PARITY, unchanged). A single-box document — every migrated flow-body page — sorts to itself unchanged, which is what keeps a migration's Markdown export byte-identical to its pre-migration export.
