...
- **`src/workspaces/notes/lib/notesBlankPaper.js`** — TODO — describe
  - _exports_: `BLANK_DBLTAP_MS`, `BLANK_DBLTAP_PX`, `isBlankDoublePress`, `pressPastLineEnd`
...
- **`src/workspaces/notes/lib/notesSketchEditor.js`** — Sketch mode's INTERACTIVE half — double-click empty canvas to make a box, type in the box, drag a box to move it, drag from its dot onto another box to draw an arrow. Behind a cached dynamic import so a note with no sketch never downloads it.
  - _exports_: `attachSketchEditor`, `default`, `isSketchDoubleTap`, `SKETCH_DBLTAP_MS`, `SKETCH_DBLTAP_PX`
...
- **`src/workspaces/site-planner/lib/overlayCrop.js`** — Pure non-destructive crop geometry for a placed site-plan overlay (B712595): clamp/normalize a crop rect to the sparse "no key = full image" convention, the SVG clipPath screen rect, and the panel's feet-based edge-trim fields
  - _exports_: `clampCropRect`, `clampPolyPoints`, `clipPathValueForCrop`, `cropClipRectScreen`, `cropFromTrimFeet`, `cropKind`, `cropTrimFeet`, `effectiveCropRect`, `hasCrop`, `isFullCrop`, `isUsablePoly`, `isValidCropShape`, `MIN_CROP_PX`, `MIN_POLY_BBOX_PX`, `MIN_POLY_VERTICES`, `normalizeCrop`, `normalizeCropShape`, `normalizePolyCrop`, `polygonAreaPx`, `polyPointsToRect`, `rectToPolyPoints`
...
- **`src/workspaces/site-planner/lib/roadNetwork.js`** — Dissolves connected road strips + curb-return wedges into ONE pavement region per cluster (clipper union, orientation-normalised, morphologically closed), and trims curb stripes at junctions. The topology replacement for the old per-junction cover patches.
  - _exports_: `clipPolylineOutside`, `clusterIds`, `collapseRingSpikes`, `dissolveRings`, `polygonOutlineCutSegments`, `rectOutlineCutSegments`, `regionPathD`, `RING_MICRO_AREA_FLOOR_SQFT`, `RING_SPIKE_DRIFT_FT`, `RING_SPIKE_LEN_FT`, `RING_SPIKE_PASS_DRIFT_FT`, `RING_SPIKE_TURN_DEG`, `ringDrift`, `ringSpikes`, `ringTurnAngles`, `ROAD_JOIN_MITER_LIMIT`, `roadSurfaceRing`
