...
- **`src/app/buildSkew.js`** — Notices when this tab is running an older deploy than the one the server is serving, and says so without reloading anything itself
  - _exports_: `fetchServedBuild`, `fetchServedMeta`, `installBuildSkewWatch`, `isBuildSkewed`, `isLoadedBuildUnsafe`, `LOADED_BUILD`, `shouldEscalate`, `shouldOfferReload`, `SKEW_FIRST_CHECK_MS`, `SKEW_POLL_MS`, `VERSION_URL`
...
- **`src/app/recoverableError.js`** — Which render crashes the error boundary may clear by remounting instead of showing a dead end (B1189): classifies React's nested-update circuit breaker (dev text + minified #185) and decides recover-vs-show under a bounded, time-windowed retry budget. — `UPDATE_DEPTH_CODE`, `isUpdateDepthError`, `isRecoverableRenderError`, `MAX_AUTO_RECOVERIES`, `RECOVERY_WINDOW_MS`, `planRecovery`
  - _exports_: `crashSubtree`, `isRecoverableRenderError`, `isUpdateDepthError`, `MAX_AUTO_RECOVERIES`, `planRecovery`, `RECOVERY_WINDOW_MS`, `UPDATE_DEPTH_CODE`
- **`src/app/renderLoopProbe.js`** — TODO — describe
  - _exports_: `classifyDepChange`, `createLoopProbe`, `DEP_IDENTITY_ONLY`, `DEP_PRESENCE`, `DEP_UNCHANGED`, `DEP_VALUE`, `LOOP_MAX_SITES`, `LOOP_SUSPECT_RUNS`, `LOOP_WINDOW_MS`, `loopReport`, `loopSummary`, `noteEffectRun`, `resetLoopProbe`
...
- **`src/app/ShortcutsPage.jsx`** — TODO — describe
  - _exports_: `default (ShortcutsPage)`
- **`src/app/unsafeBuilds.js`** — TODO — describe
  - _exports_: `UNSAFE_BUILDS`
...
- **`src/workspaces/dashboard/components/DashboardCards.jsx`** — the six default Dashboard card renderers (pure presentational, token-only).
  - _exports_: `CardSkeleton`, `GoingQuietCard`, `JumpBackInCard`, `JumpBackInCountControl`, `PipelineCard`, `ScheduleHealthCard`
...
- **`src/workspaces/dashboard/lib/dashboardLayout.js`** — pure model for the arrangeable card grid: catalog, default layout, add/remove/reorder/resize.
  - _exports_: `addCard`, `appendNewCatalogCards`, `applyGridChange`, `availableToAdd`, `CARD_DEFS`, `CARD_KEYS`, `DEFAULT_LAYOUT`, `DEFAULT_ORDER`, `dismissCard`, `GRID_COLS`, `JUMP_BACK_IN_COUNT_DEFAULT`, `JUMP_BACK_IN_COUNT_MAX`, `JUMP_BACK_IN_COUNT_MIN`, `narrowOrder`, `newCatalogCards`, `normalizeDismissed`, `normalizeJumpBackInCount`, `normalizeLayout`, `removeCard`, `resetLayout`, `toRglItem`, `undismissCard`
...
- **`src/workspaces/dashboard/lib/dashboardPipeline.js`** — pure grouping/derivation over `sites` rows for the Pipeline, Pursuits-by-activity, Going-quiet, and Jump-back-in cards.
  - _exports_: `goingQuiet`, `groupProjectsByGroupId`, `pipelineCounts`, `recentProjects`
...
- **`src/workspaces/dashboard/lib/topoMotion.js`** — TODO — describe
  - _exports_: `DEPTH_RATE`, `easeToward`, `FOLLOW_RADIUS2`, `FOLLOW_STRENGTH`, `SETTLE_POS`, `SETTLE_STRENGTH`
...
- **`src/workspaces/notes/components/NoteEditor.jsx`** — One note page (title · toolbar · document) and the module's ONLY editor-engine import — the lazy boundary. Snapshots the document as plain JSON at edit time so the flush never queries a torn-down instance.
  - _exports_: `default`, `noteTitleFontPx`, `PASTE_MODE_META`, `placeMenu`
...
- **`src/workspaces/notes/components/TemplateManager.jsx`** — TODO — describe
  - _exports_: `default (TemplateManager)`
...
- **`src/workspaces/notes/lib/notesArrows.js`** — TODO — describe
  - _exports_: `addArrow`, `cascadeRemoveArrows`, `edgePoint`, `normalizeArrows`, `removeArrow`
...
- **`src/workspaces/notes/lib/notesBlankPaper.js`** — TODO — describe
  - _exports_: `BLANK_DBLTAP_MS`, `BLANK_DBLTAP_PX`, `isBlankDoublePress`
...
- **`src/workspaces/notes/lib/notesBlockMerge.js`** — TODO — describe
  - _exports_: `mergeNoteDocs`
...
- **`src/workspaces/notes/lib/notesExtensions.js`** — The ONE declaration of what a note may contain — the editor extension set (incl. images, the empty-page placeholder and search marking), the empty-document constant, and the per-page configured variant.
  - _exports_: `EMPTY_DOC`, `HEADING_LEVELS`, `NOTE_EXTENSIONS`, `noteExtensions`
...
- **`src/workspaces/notes/lib/notesFlowMigration.js`** — TODO — describe
  - _exports_: `MIGRATED_BOX_WIDTH`, `migrateFlowBody`, `migrateSketchesToBoxes`
...
- **`src/workspaces/notes/lib/notesFormatPalette.js`** — The text colours, highlights, fonts and point sizes the formatting controls offer. The module's ONLY literal colours, and content rather than chrome: shared by the toolbar and the right-click mini-toolbar so the two cannot disagree about what a colour is.
  - _exports_: `DEFAULT_SIZE`, `FONTS`, `HIGHLIGHT_COLORS`, `SIZE_MAX`, `SIZE_MIN`, `SIZES`, `TEXT_COLORS`
- **`src/workspaces/notes/lib/notesImageDb.js`** — The raw IndexedDB tier under the note-image store — the ONE file that touches `indexedDB`. Every call resolves rather than rejects, and reports whether the bytes actually landed.
  - _exports_: `idbDeleteImages`, `idbDeleteMergeBases`, `idbDeleteVersions`, `idbGetImage`, `idbGetMergeBase`, `idbGetVersion`, `idbListImageMeta`, `idbListVersions`, `idbPutImage`, `idbPutMergeBase`, `idbPutVersion`, `notesIdbAvailable`
...
- **`src/workspaces/notes/lib/notesIndentLevel.js`** — a list item's own indent LEVEL, decided once: reader, ceiling and markup. Pure and engine-free so the Markdown exporter (static path) can read it.
  - _exports_: `INDENT_STEP_EM`, `INDENTABLE`, `indentAttrs`, `indentCssRules`, `listMarkerCssRules`, `MAX_INDENT`, `MAX_MARKER_DEPTH`, `parseIndent`, `readIndent`
- **`src/workspaces/notes/lib/notesKeys.js`** — the device storage KEY FORMAT and the scope rule, written down once — a dependency-free leaf so the one module allowed to read these keys from outside the Notes route cannot drift from the store.
  - _exports_: `ACTIVE_PAGE_KEY_BASE`, `IGNORED_DUPES_KEY_BASE`, `LOCAL_SCOPE`, `PAGE_KEY_BASE`, `scopeFor`, `SYNC_KEY_BASE`, `TEMPLATES_KEY_BASE`, `TREE_KEY_BASE`
- **`src/workspaces/notes/lib/notesKeyScope.js`** — who owns a keypress: while there is a live caret in editable text, every global binding in Notes is inert. One shared predicate, asserted as a source property.
  - _exports_: `bindingShouldDecline`, `caretOwnsTheKey`, `FIELD_SELECTOR`, `formFieldOwnsTheKey`, `keysBelongToTheCaret`, `readCaretScope`, `UNGATED_KEYS`
...
- **`src/workspaces/notes/lib/notesMarkdown.js`** — PURE Markdown export of a note's document model (GFM tables/task lists, HTML fallback for what Markdown cannot spell, plus a lossiness report) and `docToText` for body search.
  - _exports_: `assetIdsInDoc`, `attachmentIdsInDoc`, `attachmentIdsInDocs`, `docToMarkdown`, `docToText`, `escapeText`, `imageIdsInDoc`, `imageIdsInDocs`, `lossyNote`, `MD_INLINE_ATTACHMENT_MAX`, `MD_MAX_HEADING`, `NOTE_MD_HANDLED`, `pageToMarkdown`, `remapAssetIds`, `safeFileName`
- **`src/workspaces/notes/lib/notesMarquee.js`** — PURE: one gesture on empty page, two meanings, decided by distance — place below the slop, marquee above it. Also the band's rectangle, which boxes it caught, shift-toggling, arrow nudges, and moving a whole set by ONE clamped delta so the arrangement cannot deform.
  - _exports_: `applyMarquee`, `boxesInMarquee`, `DRAG_SLOP`, `dragDistance`, `gestureOutcome`, `latchGesture`, `marqueeRect`, `moveSelection`, `NUDGE_STEP`, `NUDGE_STEP_FAST`, `nudgeDelta`, `panTarget`, `rectsOverlap`, `toggleSelection`
...
- **`src/workspaces/notes/lib/notesPageHeight.js`** — TODO — describe
  - _exports_: `clampHeight`, `dragHeightFromDelta`, `PAGE_HEIGHT_MAX`, `PAGE_HEIGHT_MIN`, `pageHeightLabel`, `resolvePinnedBaseHeight`, `scrollToReach`, `topEdgeCompensation`
- **`src/workspaces/notes/lib/notesPageWidth.js`** — TODO — describe
  - _exports_: `dragWidthFromDelta`, `FULL_WIDTH_FLOOR`, `FULL_WIDTH_GUTTER`, `leftEdgeDrag`, `normalizePageMargin`, `PAGE_COL_MIN`, `PAGE_WIDTH_MAX`, `PAGE_WIDTH_MIN`, `PAGE_WIDTH_PRESETS`, `pageWidthLabel`, `pageWidthPresetId`, `resolvePinnedBaseWidth`, `resolvePresetPx`, `rightEdgeDrag`, `sheetWidthFor`
...
- **`src/workspaces/notes/lib/notesPrint.js`** — The Notes print / Save-as-PDF sheet — a pure HTML document builder whose paper CSS mirrors the screen's editor CSS, plus the hidden-iframe driver that opens the print dialogue.
  - _exports_: `buildPrintDocument`, `pageAnchorExtentLeftPx`, `pageAnchorExtentPx`, `pageAnchorExtentTopPx`, `pageHeightPinExtentPx`, `pageMarginLeftPx`, `pageTableExtentPx`, `pageWidthPinExtentPx`, `printHtmlDocument`
...
- **`src/workspaces/notes/lib/notesReadingOrder.js`** — TODO — describe
  - _exports_: `sortTopLevelForReading`
...
- **`src/workspaces/notes/lib/notesSketchModel.js`** — PURE sketch decisions: THE CANVAS OWNS EVERYTHING — a box carries its own text and its own position, arrows are an explicit list, `removeBox` is the ONE place a box is destroyed (and where the arrow cascade runs), plus the layout and the migration off the superseded outline shape.
  - _exports_: `BODY_LINE_H`, `BODY_WRAP`, `BOX_MIN_H`, `BOX_W`, `boxSize`, `edgePoint`, `LABEL_WRAP`, `layoutSketch`, `LINE_H`, `MARGIN`, `MIN_CANVAS_H`, `MIN_CANVAS_W`, `normalizeSketch`, `PAD_Y`, `SLACK_X`, `SLACK_Y`, `wrapText`
...
- **`src/workspaces/notes/lib/notesSpacing.js`** — how far apart the lines are: a BLOCK property (paragraph/heading), written into the markup so it saves, syncs and prints with no second stylesheet
  - _exports_: `BLOCK_SPACES`, `blockFontSize`, `COMFORTABLE_LINE`, `DEFAULT_DENSITY`, `DENSITIES`, `densityFor`, `densityStyle`, `fontSizePx`, `LINE_SPACINGS`, `SINGLE`, `SPACE_AFTER_DEFAULT`, `SPACE_BEFORE_DEFAULT`, `SPACING_LINE_OPTIONS`, `SPACING_PRESETS`, `spacingFromElement`, `spacingGlyphFor`, `spacingLabel`, `spacingStyle`
- **`src/workspaces/notes/lib/notesStore.js`** — The ONE storage seam for Notes — per-account scoped keys, tree and page bodies kept separate, image bytes in IndexedDB behind enforced ceilings, `purgePages` as the one place bytes are destroyed, every failure broadcast (LOUD-FAILURE). Cloud sync would be a change here and nowhere else.
  - _exports_: `clearNotesStorageError`, `collectBinFacts`, `copyNoteAssets`, `createPage`, `deleteNoteImages`, `deletePages`, `deletePageVersions`, `duplicatePageTree`, `ignoreDuplicate`, `knownBinnedPages`, `lastNotesStorageError`, `listStoredPageIds`, `LOCAL_SCOPE`, `markPagesBinned`, `markPagesRestored`, `MAX_FILE_BYTES`, `MAX_IMAGE_BYTES`, `MAX_NOTEBOOK_IMAGE_BYTES`, `noteImageUsage`, `notesConflictFor`, `notesConflictKeptLine`, `notesConflictLine`, `notesConflicts`, `notesScope`, `notesScopeLabel`, `notesStorageLine`, `notesSyncState`, `onNotesConflict`, `onNotesPagesChanged`, `onNotesStorageError`, `onNotesSyncState`, `PAGE_KEY_BASE`, `pageKey`, `purgePages`, `putNoteFile`, `putNoteImage`, `readActivePageId`, `readIgnoredDuplicates`, `readNoteFile`, `readNoteFiles`, `readNoteImage`, `readNoteImages`, `readNoteTemplates`, `readNoteView`, `readPage`, `readPageVersion`, `readPageVersions`, `readTreeRaw`, `refreshNotesSync`, `registerOpenNoteDoc`, `reportImageProblem`, `resolveNotesConflict`, `restorePageVersion`, `searchNotes`, `setNotesScope`, `snapshotPage`, `startNotesSync`, `stopNotesSync`, `sweepEmptyAnchors`, `sweepImagesOfMissingPages`, `sweepOrphans`, `SYNC_KEY_BASE`, `syncKey`, `TREE_KEY_BASE`, `treeKey`, `writeActivePageId`, `writeNoteTemplates`, `writeNoteView`, `writePage`, `writeTree`
...
- **`src/workspaces/notes/lib/notesTableColumns.js`** — TODO — describe
  - _exports_: `applyTableColumnNormalization`, `default`, `normalizeTableColumnWidths`, `NoteTableColumns`, `NoteTableView`, `tableColumnWidths`
...
- **`src/workspaces/notes/lib/notesTableWidth.js`** — TODO — describe
  - _exports_: `isTableTouched`, `normalizedColumnWidths`, `TABLE_COL_MIN_WIDTH`, `TABLE_DEFAULT_COL_WIDTH`, `tableTotalWidth`
- **`src/workspaces/notes/lib/notesTemplates.js`** — Extensible page-creation templates (plain data, no editor-engine import) written straight through the storage seam so a template never has to mount the editor.
  - _exports_: `commitTemplateLabel`, `createTemplateRecord`, `deleteTemplateRecord`, `displayTemplateLabel`, `duplicateTemplateRecord`, `renameTemplateRecord`, `SEED_TEMPLATES`, `seedTemplateRecords`, `templateById`, `templateFromDoc`, `writeTemplateBody`
...
- **`src/workspaces/notes/lib/notesViewport.js`** — TODO — describe
  - _exports_: `clampViewZoom`, `fitView`, `frameView`, `normalizeView`, `panBy`, `parseView`, `serializeView`, `stepZoom`, `toViewport`, `toWorkspace`, `VIEW_KEY_BASE`, `VIEW_ZOOM_DEFAULT`, `VIEW_ZOOM_MAX`, `VIEW_ZOOM_MIN`, `VIEW_ZOOM_STEPS`, `viewKey`, `zoomAbout`, `zoomForWheel`, `zoomKeyIntent`, `zoomLabel`
...
- **`src/shared/comps/lib/compSheetColumns.js`** — pure column model for the comp-entry spreadsheet: per-cell get/set, polymorphic fields, display formatting, fill-down/paste-spill
  - _exports_: `absorbPasteIntoSheet`, `applyCellEdit`, `BASIS_OPTIONS`, `cellPlaceholder`, `cellState`, `columnIndex`, `computeFlexWidths`, `fillDownColumn`, `formatNumberDisplay`, `frozenLeftOffsets`, `groupLabelIsRedundant`, `GROUPS`, `isUnfilledRow`, `matchOption`, `NOTES_COLUMN`, `optionsForColumn`, `PERIOD_OPTIONS`, `sanitizeNumericInput`, `saveButtonLabel`, `SHEET_COLUMNS`, `spillPaste`, `TYPE_OPTIONS`, `UNIT_OPTIONS`, `visibleColumnIndices`, `widthFor`
...
- **`src/shared/comps/lib/kmlExport.js`** — TODO — describe
  - _exports_: `balloonHtml`, `buildKml`, `buildKmz`, `crc32`, `KMZ_MIME`, `kmzFilename`, `pointFeature`, `polygonFeature`, `ringWithCentroidPin`, `siteRecordFeatures`, `xmlEscape`, `zipStore`
- **`src/shared/comps/lib/kmlImport.js`** — pure, hand-rolled KML placemark parsing (B849233): Point/Polygon geometry (a polygon's area-weighted centroid, not a vertex average), plus best-effort description extraction reusing compParse.js's prose parser
  - _exports_: `kmlDescriptionToText`, `kmlToDraftRows`, `kmzToDraftRows`, `kmzToKmlText`, `parseKmlPlacemarks`, `placemarkToDraftRow`, `polygonCentroid`
...
- **`src/shared/gis/parcelSnapshotBuild.js`** — Pure county parcel snapshot transforms: strip to UI-read fields and quantize polygon coordinates into a compact gzippable GeoJSON FeatureCollection
  - _exports_: `buildSnapshotFC`, `esriRingsToGeoJsonGeometry`, `KEEP_FIELDS`, `leanFeature`, `leanProps`, `quantizeGeometry`
...
- **`src/shared/keyboard/platform.js`** — TODO — describe
  - _exports_: `comboAriaLabel`, `formatCombo`, `isApplePlatform`
- **`src/shared/keyboard/shortcutsData.js`** — TODO — describe
  - _exports_: `ALL_SHORTCUT_ITEMS`, `SHORTCUT_SECTIONS`
- **`src/shared/keyboard/shortcutsPageBus.js`** — TODO — describe
  - _exports_: `closeShortcutsPage`, `isShortcutsPageOpen`, `openShortcutsPage`, `subscribeShortcutsPage`
...
- **`src/shared/projects/projectModel.js`** — Pure project-model helpers: collapse site records into one project per site-group, name-match suggest, dropdown filter, and relative-time formatting for the breadcrumb switcher
  - _exports_: `applyFrozenOrder`, `DELETED_RETENTION_DAYS`, `distanceFeetBetween`, `filterProjects`, `findProjectAtOrigin`, `groupProjects`, `hasSavedProjectRecord`, `markProjectFreshlyMinted`, `normalizeProjectName`, `projectGateStatus`, `relTime`, `reorderWithCurrentAndPinned`, `resolveControlledId`, `resolveCurrentName`, `SAME_GROUND_FT`, `shortenDisplayName`, `suggestNameMatch`, `unionProjectLists`, `wasProjectFreshlyMinted`, `withCurrentProject`
...
- **`src/shared/theme/familyInk.js`** — the default ink of each drawn family + the rule that keeps them perceptually apart (a measurement and a markup shared #c2410c, so the number vanished into the shape)
  - _exports_: `CALLOUT_DEFAULT_FILL`, `ELEMENT_DEFAULT_PAINT`, `FAMILY_DEFAULT_INK`, `INK_DISTINCT_MIN_DE`, `MEASURE_INK`, `parseHex`
...
- **`src/shared/ui/breadcrumbFit.js`** — TODO — describe
  - _exports_: `CRUMB_SEP_W`, `crumbNeedsCompact`
...
- **`src/workspaces/site-planner/components/icons.jsx`** — Small stroke icons (pin / empty-circle / warn-triangle) shared by the planner's panel components, replacing the 📍 emoji that ignored its row's theme colour; route-local on purpose so the bytes stay off every other route's chunk. — `PinIcon`, `EmptyCircleIcon`, `WarnTriangleIcon`
  - _exports_: `CloseXIcon`, `CompIcon`, `DuplicateIcon`, `EmptyCircleIcon`, `HistoryIcon`, `LayersIcon`, `NoteIcon`, `PadlockIcon`, `PeopleIcon`, `PinIcon`, `PlusIcon`, `RedoIcon`, `SaveIcon`, `SitePlanIcon`, `StorageIcon`, `UndoIcon`, `WarnTriangleIcon`, `ZoomFitIcon`
...
- **`src/workspaces/site-planner/components/OverlayCropDialog.jsx`** — TODO — describe
  - _exports_: `default (OverlayCropDialog)`
...
- **`src/workspaces/site-planner/lib/appraisal.js`** — Pure CAD-attribute curation: regex-maps raw county/TxGIO parcel columns to labelled owner/value/acreage/use rows for both panels
  - _exports_: `acreageKey`, `ADDR_LINE_RE`, `APPR_FIELDS`, `apprAll`, `apprRows`, `apprVal`, `findAttr`, `isPlaceholderValue`, `MAILING_KEY_RE`, `mailingAddressValues`, `OWNER_KEY_RE`, `ownerKey`, `ownerName`, `PARCEL_CARD_PRIMARY_LABELS`, `PARCEL_PANEL_PRIMARY_LABELS`, `parcelCardRows`, `parcelPanelRows`, `prettyKey`, `siteNameFromParcel`, `SITUS_FIELD`, `SITUS_LADDER`, `situsAddress`, `situsKey`, `splitCuratedRows`, `tidyAddressLabel`
...
- **`src/workspaces/site-planner/lib/arcgis.js`** — Esri ArcGIS REST client: bounded parcel identify (query+identify fallback, multi-county eager race) and lon/lat↔State-Plane-feet conversion
  - _exports_: `aerialPlacement`, `aerialTileGrid`, `BACKUP_GRACE_MS`, `deepenZoomFor`, `featureToParcel`, `feetExtentToBbox`, `feetToLatLng`, `geoJsonToEsriFeature`, `getLayerInfo`, `humanizeError`, `identifyAtPoint`, `identifyParcelAcross`, `identifyParcelDetailed`, `identifyParcelEager`, `isPaginationUnsupportedError`, `isQueryCapabilityError`, `largestRingLngLat`, `listLayers`, `lngLatFeatureToParcel`, `lngLatRingToFeet`, `lngLatToGlobalPixel`, `outerRingsLngLat`, `overlayExportPlacement`, `PARCEL_FETCH_TIMEOUT_MS`, `ParcelFetchError`, `pickAerialTileZoom`, `queryAtPoint`, `queryFeatures`, `resolveGisUrl`, `resolveLayerUrl`
...
- **`src/workspaces/site-planner/lib/bootFramingDeadline.js`** — TODO — describe
  - _exports_: `BOOT_FRAMING_CEILING_MS`, `bootFramingDeadlineAt`, `bootFramingRemainingMs`, `resetBootFramingDeadlines`
...
- **`src/workspaces/site-planner/lib/buildingGrid.js`** — Pure structural column-grid + dock-door layout: uniform in-band bays, pinned speed bays, doors avoiding column lines
  - _exports_: `computeBuildingGrid`, `divideSpan`, `GRID_DEFAULTS`, `GRID_MIN_PPF`, `gridLinesVisible`, `placeDockDoors`, `resolveGridSettings`
- **`src/workspaces/site-planner/lib/buildingProps.js`** — Pure tiered building-property rules: sf-driven clear-height + slab-thickness defaults with per-building manual overrides
  - _exports_: `addTier`, `autoClearHeight`, `autoSlab`, `DEFAULT_BUILDING_RULES`, `effectiveBuildingProps`, `evalTier`, `fmtClearHeight`, `fmtSlab`, `maxFiniteUpTo`, `moveTier`, `normalizeRules`, `removeTier`
...
- **`src/workspaces/site-planner/lib/calloutStyle.js`** — TODO — describe
  - _exports_: `CALLOUT_LINE`, `CALLOUT_STD_KEYS`, `calloutStyle`
...
- **`src/workspaces/site-planner/lib/cityScopes.js`** — TODO — describe
  - _exports_: `CITY_SCOPE_KEYS`, `cityScopeAnswer`
...
- **`src/workspaces/site-planner/lib/counties.js`** — County parcel/GIS registry: CAD endpoints, TxGIO statewide fallback, jurisdiction utility layers, click-routing bboxes, tax-unit resolver
  - _exports_: `candidateCountiesForPoint`, `COUNTIES`, `COUNTIES_MAP`, `countyBboxIntersectsView`, `countyForView`, `countyIdentity`, `countyKeyForName`, `countyKeysForState`, `countyPolygonsReady`, `detectField`, `FEET_WKID`, `IDENTIFY_ONLY_LAYER_URLS`, `isIdentifyOnlyLayerUrl`, `isStatewideLayerUrl`, `JURISDICTION_LAYERS`, `loadCountyPolygons`, `noParcelSourceNote`, `resolveTaxRates`, `sharedLayerUrlConflicts`, `SNAPSHOT_COUNTIES`, `stateForCountyKey`, `STATEWIDE_KEYS`, `STATEWIDE_LAYER_URLS`, `STATEWIDE_PARCEL_LAYER`, `statewideFallbackFor`, `TAX_RATE_SOURCES`, `trimLayerUrl`
...
- **`src/workspaces/site-planner/lib/dockZones.js`** — Building-anchored dock-zone stack geometry: outward court/trailer/buffer chain, catalog layers, dock-side axes, stranded-zone pruning
  - _exports_: `ALONG_ANCHOR`, `alongAnchorFromDrag`, `alongLenIsChainEcho`, `alongOffsetFor`, `anchoredAlongSpan`, `boxExtentAlong`, `catalogDepthDefault`, `compassLabelForBearing`, `DOCK_AXES`, `DOCK_ZONES`, `dockAxisEstablished`, `dockAxisOf`, `dockSideBearing`, `dockSideCompassLabel`, `dockSidesFor`, `establishDockAxisPatch`, `footprintAxes`, `footprintDepth`, `footprintLength`, `healDockAxes`, `layoutStack`, `layoutZone`, `layoutZoneByKind`, `MAX_DOCK_ZONES`, `normalizeAlongAnchor`, `pruneStrandedZones`, `resizedAlongLen`, `resizedZoneAlongFit`, `resizedZoneAlongLen`, `rotateDockAxisPatch`, `strandedZoneIds`, `usableCourtSpan`, `withDockAxis`, `ZONE_CATALOG`, `zoneAlongExtent`, `zoneAlongPlacement`, `zoneAlongSpan`, `zoneDepthDefault`, `zoneDepthDefaults`, `zoneDepthExtent`
...
- **`src/workspaces/site-planner/lib/exportSheet.js`** — the on-demand export path: PDF/PNG/KMZ sheet composition, the B839 aerial tile Stitcher, and GIS raster/vector layer capture. Loaded via dynamic import() from SitePlanner (B1042) so it never rides the boot bundle; reads planner state through a per-call `ctx` object
  - _exports_: `createExportSheet`, `pageSizeForFit`, `sheetLayoutBoxesIn`, `sheetPlanAspect`
...
- **`src/workspaces/site-planner/lib/mapChromeStack.js`** — the ONE map-overlay stacking model (an open panel outranks map chrome — Leaflet controls, scale bar) plus the available-room panel height
  - _exports_: `COMPS_TOGGLE_CLEARANCE_PX`, `EDGE_TAB_TOP_PX`, `LEAFLET_CONTROL_Z`, `MAP_CHROME_Z`, `MAP_OVERLAY_BAR_H_PX`, `MAP_OVERLAY_CHIP_H_PX`, `MAP_OVERLAY_TOP_PX`, `panelMaxHeight`, `SCALE_BAR_CLEARANCE_PX`, `TOOLS_TAB_RESERVE_PX`, `TOOLS_TAB_WIDTH_PX`, `TOP_RIGHT_ROW_NARROW_RIGHT_PX`, `TOP_RIGHT_ROW_RESERVE_PX`, `ZOOM_CONTROL_CLEARANCE_PX`, `zoomStackBottomPx`
...
- **`src/workspaces/site-planner/lib/metesAndBounds.js`** — Pure polyline offset / buffer / ring-overlap primitives, shared by easements, road corridors, pipeline corridors and the KMZ export (the deed PARSER that used to live here moved to deedParse.js so it could be deferred)
  - _exports_: `bufferPolyline`, `offsetPolyline`, `offsetPolylineMiterLimit`, `ringsOverlap`
...
- **`src/workspaces/site-planner/lib/nameGroupIntegrity.js`** — TODO — describe
  - _exports_: `auditRows`, `columnGroupKeyOf`, `groupKeyMismatch`, `jsonbGroupKeyOf`, `nameMismatch`, `scheduleNameDrift`, `scheduleNameStaleAgainstLive`, `unstampedRow`
...
- **`src/workspaces/site-planner/lib/overlayCrop.js`** — Pure non-destructive crop geometry for a placed site-plan overlay (B712595): clamp/normalize a crop rect to the sparse "no key = full image" convention, the SVG clipPath screen rect, and the panel's feet-based edge-trim fields
  - _exports_: `clampCropRect`, `clampPolyPoints`, `clipPathValueForCrop`, `constrainOctant`, `cropClipRectScreen`, `cropClipShapeScreen`, `cropEditBlock`, `cropFromTrimFeet`, `cropKind`, `cropTrimFeet`, `effectiveCropRect`, `hasCrop`, `isFullCrop`, `isUsablePoly`, `isValidCropShape`, `MAX_POLY_VERTICES`, `MIN_CROP_PX`, `MIN_POLY_BBOX_PX`, `MIN_POLY_VERTICES`, `nearestOnSegment`, `normalizeCrop`, `normalizeCropShape`, `normalizePolyCrop`, `polygonAreaPx`, `polyPointsToRect`, `recropForRaster`, `rectToPolyPoints`
...
- **`src/workspaces/site-planner/lib/parcelDisplay.js`** — Shared parcel-outline display layers for map and planner: styleable esri vector layer, image-export layer for query-disabled TxGIO, Drive-snapshot geoJSON layer, add/remove cursors
  - _exports_: `ADD_CURSOR`, `makeParcelAdaptiveLayer`, `makeParcelDisplayLayer`, `makeParcelImageLayer`, `makeParcelLayer`, `makeSnapshotLayer`, `PARCEL_MINZOOM`, `PARCEL_VECTOR_MINZOOM`, `parcelDisplayIsImageOnly`, `parcelDisplayRegimeForZoom`, `parcelUrlSupportsImageExport`, `REMOVE_CURSOR`
- **`src/workspaces/site-planner/lib/parcelDisplayZoom.js`** — TODO — describe
  - _exports_: `MAPSERVER_LAYER_RE`, `PARCEL_MINZOOM`, `PARCEL_VECTOR_MINZOOM`, `parcelDisplayRegimeForZoom`, `parcelUrlSupportsImageExport`
...
- **`src/workspaces/site-planner/lib/parcelQuery.js`** — Shared parcel ID/address lookup: SQL-injection-safe where-clause builder with county scoping and primary-CAD to statewide-TxGIO outage fallback plus circuit-breaker health recording
  - _exports_: `buildParcelWhere`, `idAttrFor`, `isDefaultLookupUrl`, `lookupParcels`, `okField`, `resolveSearchField`
...
- **`src/workspaces/site-planner/lib/parking.js`** — Pure parking-layout math: rows-to-depth, split into double-loaded modules, explode into stall-row/aisle bands, curb-adjacency test
  - _exports_: `edgeAbutsPaving`, `explodeParkingBands`, `freeParkStack`, `parkDepthForRows`, `parkFlipIsNoOp`, `parkRowsForDepth`, `PAVED_NEIGHBOR_TYPES`, `relayoutFreeStack`, `splitParkingPieces`
...
- **`src/workspaces/site-planner/lib/planStyle.js`** — Shared element style tokens (fills/strokes/weight/pattern per surface type), style resolver, paint z-order, element feet ring outline
  - _exports_: `byZ`, `elRingFeet`, `elStyle`, `elToRingFeet`, `getAccountStyleDefaults`, `getPreviewStyleDefaults`, `parcelDefaultStyle`, `setAccountStyleDefaults`, `SETBACK_LINE`, `setbackChipStyle`, `setbackDashArray`, `setbackLineStyle`, `setPreviewStyleDefaults`, `standardScope`, `toHex6`, `TYPE`, `typeStyle`, `zOrder`
...
- **`src/workspaces/site-planner/lib/printFrameDrag.js`** — TODO — describe
  - _exports_: `FRAME_GRIPS`, `frameGripAnchor`, `MIN_FRAME_FT`, `orientationForAspect`, `resizeFrame`
...
- **`src/workspaces/site-planner/lib/printSheet.js`** — Pure single-SVG print sheet composer: page geometry, buildings table, metrics band, title block, export filename builder
  - _exports_: `buildPrintSheetSvg`, `buildStormwaterSvg`, `formatDateStamp`, `metricsRowsFor`, `pageSize`, `pageSizeForFit`, `PAPER_SIZES`, `printSheetLayout`, `sanitizeFilename`, `sheetFileName`, `stormwaterBandH`
...
- **`src/workspaces/site-planner/lib/rasterIdentifyMap.js`** — leaflet glue for the raster identify: map hover/click wiring, the transient readout DOM, and the direct-then-cache-proxy transport that gets past a no-CORS agency host.
  - _exports_: `attachRasterIdentify`, `makeIdentifyFetch`
...
- **`src/workspaces/site-planner/lib/roadGeometry.js`** — Pure centerline road geometry: tessellate clicked alignment into arc fillets/smooth splines/sharp corners, min radius of curvature
  - _exports_: `canRemoveRoadVertex`, `cardinalTeePoint`, `concatRoads`, `cornerApproachShortfall`, `cornerShares`, `curbStrokePx`, `dedupeRoadVertices`, `DEFAULT_ARC_RADIUS`, `DEFAULT_TESS_DEG`, `findRoadConnect`, `fitRoadCorners`, `fixRoadRadii`, `insertRoadVertex`, `legShares`, `minRadiusOfCurvature`, `nearestRectEdge`, `nodeJunction`, `PAD_EDGE_COLLINEAR_TOL_DEG`, `planRoadConnect`, `polygonContainsPoint`, `polygonDepthBehind`, `polygonEdgeRunFrom`, `polygonEdges`, `polylineLength`, `projectToPolyline`, `projectToRoadCenterline`, `rectContainsPoint`, `rectEdges`, `removeRoadVertex`, `repairBakedRadii`, `ROAD_SIMPLIFY_TOL_FT`, `ROAD_VERTEX_COLLAPSE_FT`, `roadBearingDeg`, `roadCenterline`, `roadCenterlineTagged`, `roadCornerRadii`, `roadEdgeCrossing`, `roadMinRadius`, `roadRadiusConflicts`, `roadsMergeCompatible`, `simplifyRoadVertices`, `slideTeeNode`, `TEE_CARDINAL_STEP_DEG`, `teeGeometry`, `teeNodeIndex`, `weldCoverPolygon`
- **`src/workspaces/site-planner/lib/roadJunctions.js`** — TODO — describe
  - _exports_: `armStraightRun`, `BUILDING_CLEAR_FT`, `buildingRunLimit`, `DRIVE_RETURN_SEED`, `DRIVE_TEE_SNAP_FT`, `driveJunctionsOf`, `roadRunFrom`, `roadTangentNoise`, `teeJunctionsOf`, `teeNodeIndicesOf`, `VERTEX_NOISE_FT`
- **`src/workspaces/site-planner/lib/roadNetwork.js`** — Dissolves connected road strips + curb-return wedges into ONE pavement region per cluster (clipper union, orientation-normalised, morphologically closed), and trims curb stripes at junctions. The topology replacement for the old per-junction cover patches.
  - _exports_: `clipPolylineOutside`, `clusterIds`, `collapseRingSpikes`, `dissolveRings`, `polygonOutlineCutSegments`, `polylinesPathD`, `rectOutlineCutSegments`, `regionEdgeSegments`, `regionPathD`, `RING_MICRO_AREA_FLOOR_SQFT`, `RING_SPIKE_DRIFT_FT`, `RING_SPIKE_LEN_FT`, `RING_SPIKE_PASS_DRIFT_FT`, `RING_SPIKE_TURN_DEG`, `ringDrift`, `ringSpikes`, `ringTurnAngles`, `ROAD_JOIN_MITER_LIMIT`, `roadSurfaceRing`
...
- **`src/workspaces/site-planner/lib/sharedWithTeam.js`** — What a site row's shared-with indicator shows: the TEAM name a site is shared with (never a roster of people) — B845088, retires sharedWithMonogram.js
  - _exports_: `entityInitials`, `sharedWithDisplay`
...
- **`src/workspaces/site-planner/lib/sheetFurniture.js`** — Map sheet furniture: graphic scale bar and two-tone north arrow, output-unit sized with no-occlude corner placement, screen + export
  - _exports_: `calibBadgePlacement`, `canvasPillBottom`, `furnitureMetrics`, `MAP_CHROME_REF_S`, `mapChromeCardStyle`, `northArrowPlate`, `pickScaleBar`, `r2`, `scaleBarPlate`, `screenFurniturePlates`
...
- **`src/workspaces/site-planner/lib/siteGeometry.js`** — Shared pure geometry primitives behind a site's drawn area math: element footprints, curb bands, road strip/pavement area, junction detection and the per-plan roundabout resolution
  - _exports_: `carStalls`, `CURB`, `CURB_6`, `CURB_TYPES`, `curbAreaOf`, `curbEdgesOf`, `curbHost`, `curbWidthOf`, `driveJunctionCurbStripes`, `elCorners`, `estStalls`, `estTrailers`, `extendInsetTowardSideTangent`, `insetArcTowardCenter`, `isCenterlineRoad`, `outwardCurbEdge`, `polyArea`, `ringOf`, `roadCurbLines`, `roadCurbWidth`, `roadDefaultRadius`, `roadDenseCenterline`, `roadImperviousArea`, `roadJunctionVerticesOf`, `roadStripArea`, `roadStripRing`, `rot2`, `roundaboutsForSite`, `sidewalkBetween`, `SQFT_PER_ACRE`, `STRIPE_OVERLAP_FT`, `TEE_COINCIDE_FT`, `TEE_COINCIDE_MAX_FT`, `teeCoincideFt`, `teeTargetOf`, `teeTargetPointOf`, `trailerStalls`
...
- **`src/workspaces/site-planner/lib/sourceHealth.js`** — Per-source circuit breaker for county parcel servers: track consecutive failures, open/cooldown/half-open, filter healthy candidates, and decide the honest statewide-backup badge
  - _exports_: `filterHealthyCandidates`, `isSourceOpen`, `isStatewideBackup`, `recordSourceResult`, `resetSourceHealth`, `SOURCE_COOLDOWN_MS`, `SOURCE_FAIL_THRESHOLD`, `SOURCE_SLOW_MS`, `sourceCooldownMs`, `suppressRedundantStatewide`
...
- **`src/workspaces/site-planner/lib/userPrefsStore.js`** — TODO — describe
  - _exports_: `_normalizePrefs`, `EMPTY_PREFS`, `getStandardPref`, `loadPrefsRaw`, `readMirror`, `savePrefsRaw`, `setSitesPanelPref`, `setStandardPref`, `writeMirror`
...
- **`src/workspaces/site-planner/lib/zOrder.js`** — explicit z_index utilities (B671): assign/sort/renormalize the within-type-layer stacking tiebreak that replaced implicit array position
  - _exports_: `byZAsc`, `ensureZ`, `migrateBandForce`, `needsZ`, `nextZ`, `normalizeZ`, `sortByZ`, `withMissingZ`, `Z_GAP`
...
- **`public/sequence/index.html`** — Self-contained Sequence/Schedule iframe app: spreadsheet with an Excel-like formula engine, dependency-graph task scheduling, Gantt + PDF export, Supabase cloud-save/history, suggestions review queue, site-linked
  - _exports_: `_cpMeasure`, `_deepClone`, `_formulaBuiltinMap`, `_mEq`, `_mIdArr`, `_mintTaskSid`, `_mIsObj`, `_mKeyOf`, `_mStripRev`, `_pfCostRef`, `_pfDateRef`, `_pfNumOrBlank`, `_taskFingerprint`, `_taskKey`, `_taskSidMap`, `activeEffectiveCols`, `addBD`, `addCalendarMonths`, `addD`, `addMonths`, `AddNoteComposer`, `addWorkdays`, `adjacentMeetingDate`, `agendaDeadline`, `agendaLeadSummary`, `ancestorIdsOf`, `App`, `applyMeetingBinding`, `approxTextPx`, `AutomationPanel`, `avatarColor`, `balOf`, `barLabelText`, `buildFormulaRowColumns`, `buildGanttSVG`, `buildHolidaySet`, `buildPDFHtml`, `cadenceSummary`, `calcEnd`, `CaretPopover`, `cascadeDates`, `celebrateTaskComplete`, `Cell`, `changedProjectIds`, `cleanEmailBody`, `colArray`, `colLabel`, `collectCountable`, `collectNonBlank`, `collectNums`, `collectNumsKind`, `collectRefs`, `colLettersToNum`, `colNumToLetters`, `compareValues`, `compareValuesSafe`, `computeDisplayHealth`, `computeFormulaValues`, `computeRolledHealth`, `ConditionGroup`, `ConditionRow`, `constrainedStartFrom`, `ContactPicker`, `ContactsPanel`, `copyMeetingBodyWithNewId`, `coreChanged`, `countChangedTaskRows`, `crossScheduleLabel`, `cumArgs`, `datedif`, `decomposeForDualWrite`, `defaultStartFor`, `DeleteChildrenModal`, `depAnchors`, `DepCell`, `depCellFullTitle`, `DesktopSplitView`, `detectCascadeDrift`, `dif`, `difBD`, `dropDuplicateLagPatch`, `dropFormulaSheetRefs`, `durTooltip`, `effectiveColsOf`, `ErrorBoundary`, `errVal`, `escapeHtml`, `escapeRegExp`, `estLabelWidth`, `evalBinary`, `evalCall`, `evalConditionGroup`, `evalFieldCondition`, `evalHealthCondition`, `evalHealthRules`, `evalNode`, `evalRule`, `evaluateFormula`, `extractColorTag`, `extractRefs`, `extraHolidaySet`, `fd`, `fdLocal`, `ferr`, `Field`, `findMeetingBodyElsewhere`, `findSignChange`, `finishCallResult`, `fitCaptionFs`, `flatOrderWithLevel`, `flattenArgs`, `fmtBarDate`, `fmtD`, `fmtPreds`, `fmtTaskDuration`, `formatDateToken`, `formatNumberSection`, `formatNumberToken`, `FormatPanel`, `formatSheetQualifier`, `formatValue`, `formatValueColor`, `formulaCalendar`, `formulaColsOf`, `formulaColToColDef`, `FormulaColumnEditor`, `formulaColumnNames`, `FormulaError`, `formulaToday`, `fvOf`, `ganttNameWeight`, `GanttView`, `getHealthRules`, `getInitials`, `GridColumnsChooser`, `GridHeaderMenu`, `GridView`, `groupRowsByProject`, `HealthColHeader`, `HealthColHeaderIcon`, `HealthDropMenu`, `HealthPicker`, `HistoryPanel`, `importMeetingBodies`, `importMeetingBody`, `indentSelection`, `InlineDate`, `ipmtOf`, `irrOf`, `isBlank`, `isBuiltinFormulaName`, `isCallInvariant`, `isDate`, `isErrVal`, `isFormulaError`, `isLeap`, `isOrgOwned`, `isoToSerial`, `isoWeekNum`, `isRangeArg`, `isRowInvariant`, `isSiteOwned`, `isValidColor`, `isValidFontSize`, `isWorkingSerial`, `joinWithAnd`, `layoutExhibitCols`, `loadExportPrefs`, `loadTabView`, `loadViewPrefs`, `logMergeToastEvent`, `looksLikeDateFormat`, `looseEqual`, `makeDate`, `MasterView`, `matchesCriteria`, `matchIndex`, `matchRefToken`, `mbBodiesSameCadence`, `mbFmt`, `mbMonthsPhrase`, `MBMultiPick`, `mbOrdinal`, `mbPositionsPhrase`, `mbRecurrenceSignature`, `mbRuleSignature`, `mbRuleSummary`, `measuredColWidth`, `meetingBindingTrace`, `meetingBoundInfo`, `MeetingCalendarsModal`, `meetingCostDays`, `meetingDatesInRange`, `meetingDoubleLagFlag`, `meetingFloatBD`, `MeetingImportPicker`, `mergeCloudDoc`, `mergeLocationPhrase`, `migrateCfRulesToHealthRules`, `migrateRule`, `migrateScheduleOwnership`, `mirrOf`, `mkt`, `moveSelectionToDestination`, `MoveToModal`, `nameCollision`, `need`, `networkDays`, `newMeetingBody`, `nextEligibleMeeting`, `normalizeName`, `normalizeOwnerLists`, `normalizeScheduleOwnership`, `normPreds`, `noteISO`, `NoteRow`, `NotesColHeader`, `noteShortMD`, `NotesModal`, `notifyRowsDeleted`, `nperOf`, `nthWeekday`, `nthWeekdayOfMonth`, `num1`, `numberFormatColorFor`, `numToGeneralStr`, `occurrencesInMonth`, `opsForField`, `otherScheduleMeetingBodies`, `outdentSelection`, `ownerDisplayParts`, `ownerJoin`, `ownerKeyOf`, `ownerListOf`, `ownerOf`, `parse`, `parseCriteriaOperand`, `parseDurationInput`, `parseFlexDate`, `parseFormula`, `parseFormulaUncached`, `parseLooseDate`, `parsePreds`, `parseRefText`, `partitionSchedules`, `pd`, `PDFExportModal`, `persistViewState`, `PhoneSplitView`, `pickOrdinal`, `placeGanttLabel`, `planFormulaColumns`, `pmtOf`, `positionMissingReport`, `PredEditor`, `predLagPlainEnglish`, `ProjContextMenu`, `ProjDropdown`, `promoteChildrenAndDelete`, `pruneOrphanScheduleRefs`, `pruneScheduleRefs`, `pvOf`, `raiseIfErr`, `rangeArray`, `rangeGrid`, `rateOf`, `readGridCell`, `readGridCfg`, `rebuildHEALTH`, `rebuildMeetingBodyIndex`, `recascadeWithDrift`, `recomposeFromRows`, `recomputeAfterStructureChange`, `recomputeSchedule`, `reconcileNeedsAttention`, `refToText`, `RenameModal`, `renumberTasks`, `resolveDuration`, `resolveLabelAlign`, `resolveNamedRange`, `resolveSheetGrid`, `resolveTaskSpan`, `resolveViewState`, `rewriteFormulaForCopy`, `rewriteFormulaForSheetRename`, `rewriteFormulaForStructuralShift`, `rollForwardToWorkday`, `rollupParentDates`, `roundAwayFromZero`, `Row`, `RuleColorPicker`, `sameFamily`, `saveExportPrefs`, `saveTabView`, `saveViewPrefs`, `scheduleLabelParts`, `scheduleList`, `scheduleRowPrefix`, `schedulesForOwner`, `sectionForSign`, `SegBtn`, `serialToISO`, `serialToYMD`, `SettingsPanel`, `sgqAvatarColor`, `SgqCascadePicker`, `SgqChangeRow`, `sgqChildrenOf`, `sgqConfClass`, `SgqCrumb`, `sgqCrumbOf`, `SgqDot`, `sgqFmtDate`, `sgqInitials`, `sheetQualifierAt`, `showToast`, `solveRoot`, `sortByVisualOrder`, `splitFormatSections`, `SplitView`, `startForEnd`, `StatusHeaderDropdown`, `StatusPicker`, `stepFontSize`, `stepVisibleColIdx`, `stripAncestorPredecessors`, `stripFormatLiterals`, `stripTaskViewFields`, `stripViewState`, `subBD`, `SuccessorPromptModal`, `SuggestionsView`, `suggestScheduleName`, `SwatchGrid`, `TaskContextMenu`, `taskDurUnit`, `taskDurValue`, `textFormat`, `toBool`, `toDateSerial`, `Toggle`, `tokenize`, `toNumber`, `toShortDate`, `toStr`, `touchesSchedule`, `TYPE_RANK`, `validateNewSchedule`, `validatePredEdit`, `valueFamily`, `weekdayOf`, `weekNum`, `wildcardToRegExp`, `withLastActiveSite`, `workdaysBetween`, `writeGridCfg`, `writeLabelAlign`, `xirrOf`, `xnpvPairs`, `yearFrac`, `ymdToSerial`
...
- **`src/workspaces/scheduler/lib/navState.js`** — Pure null-safe parse/sanitize of the embedded scheduler's postMessage nav-state and derive current/site-linked project for the breadcrumb
  - _exports_: `dashboardNavActions`, `deriveCurrentProject`, `findAllBySiteId`, `findBySiteId`, `isGridMismatched`, `isPickShowing`, `needsScheduleCarryIn`, `newProjectAction`, `parseNavState`, `resolveReportRowNavigation`, `sanitizeProjects`, `shouldAdoptLinkedSiteIntoRoute`, `shouldNeutralizeToReports`, `shouldShowLinkPanel`
