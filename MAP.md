...
> **Generated 2026-07-29 @ `2a8de41` by `scripts/build-map.mjs` — do not hand-edit the inventory.**
...
- **`src/workspaces/site-planner/lib/exportLabelScale.js`** — The scale the LABEL tier reasons at on an export pass: the sheet's own px-per-foot (framed extent vs plan box), so declutter/LOD/collision decisions and label sizes are a function of the plan and the paper, never of the live zoom.
