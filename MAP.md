...
> **Generated 2026-09-01 @ `1c0a8e60` by `scripts/build-map.mjs` — do not hand-edit the inventory.**
...
- **`src/shared/comps/lib/compSheetColumns.js`** — pure column model for the comp entry SHEET: column defs grouped PROPERTY/DEAL/PRICE/RENT/CONCESSIONS/DERIVED/PARTIES, per-cell get/set (polymorphic Price/Size/Unit/triangle Price-NOI-Cap route by comp type), cellState (editable/fixed/na/derived), fill-down, Excel-style paste-spill, visibleColumnIndices (hide a column the current rows don't use)
...
- **`src/workspaces/site-planner/lib/geocode.js`** — Shared address geocoder (Esri World primary, Nominatim fallback), forward (geocodeAddress) and reverse (reverseGeocodeLatLon), with an honest hit/not-found/service-down return contract, used by the map, the planner, and the comp entry sheet's Location cell
