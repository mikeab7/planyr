/* B915 / B916 / B917 — reaching markups you can click, and the honesty cues for locked ones.
 *
 ...
 *   • B915 — an UNFILLED closed markup no longer grabs its interior; a click falls THROUGH to what's
 *            under it, and the markup is still reachable near its stroke.
 *   • B916 — repeat-clicking a spot where two markups overlap CYCLES the selection between them.
 *   • B917 — a LOCKED markup shows an honest "unlock to move/reshape" hint, a 🔒 cue, and a non-move
...
test.describe("markup reachability + locked honesty (logged out)", () => {
  test("B915 — an unfilled markup no longer swallows interior clicks; a covered element is reachable through it", async ({ page }) => {
    const errors = [];
  ...
  test("B916 — repeat-clicking overlapping markups cycles the selection between them", async ({ page }) => {
    const errors = [];
    page.on("pageerror", (e) => errors.push(String(e)));
    await startBlank(page);
    const box = await canvas(page).boundingBox();
  ...
  test("B917 — a locked markup shows an honest hint + 🔒 cue + a non-move cursor", async ({ page }) => {
    const errors = [];
    page.on("pageerror", (e) => errors.push(String(e)));
    await startBlank(page);
    const box = await canvas(page).boundingBox();
