...
- **Shared coordinate spine.** One real-world coordinate system underpins everything:
  ...
  **real EPSG:2278 ↔ WGS84 projection** (`projectToGrid`/`gridToProject`, Lambert
  ...
  coverage engine** (B274), which reprojects each GIS service's published extent to
...
- **Layer coverage engine + coverage-aware picker (B274/B275).** Each layer is tagged
  ...
  street imagery" + gated as "needs setup" (B276/B277); jurisdiction vector services now
  retry transient 5xx with backoff (B278). `lib/coverage.js`.
