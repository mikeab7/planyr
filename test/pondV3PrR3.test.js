...
describe("NEW-19 — one flood-facts truth: header state == the facts the numbers use", () => {
  ...
  it("the header reads 'not checked' ONLY when floodChecked is false (never over definite remembered numbers)", () => {
    /* RETARGETED 2026-08-06 (NEW-4), not relaxed: the owner made the facts pass manual, so the line
     * gained a STALE branch between "not checked" and the age. The invariant this test exists for is
     * unchanged and is still asserted in full — "not checked" is reachable ONLY through
     * !drainage.floodChecked, and it still comes BEFORE any age. What is new is that a stale check
     * REPLACES the age rather than joining it: "3d ago" is true and useless once the answer no
     * longer describes the drawing. */
    expect(src).toContain('!drainage.floodChecked ? "Flood data: not checked" : drainage.freshness?.state === "stale" ? "Flood data: re-check" : floodAgeMs != null ? `Flood data ${formatAge(floodAgeMs)}` : "Flood data: checked"');
  });
