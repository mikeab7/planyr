/* OCR stress test (B349) — adversarial pass over the scanned-sheet path, mirroring the B348
...
describe("B349 stress — wordsToItems rejects every malformed box (no NaN ever leaks)", () => {
...
describe("B349 stress — extractWords tolerates any Tesseract shape", () => {
...
describe("B349 stress — createOcrRunner fails soft on every engine failure", () => {
...
describe("B349 stress — OCR → readSheetMeta → grouping integration on messy scans", () => {
...
describe("B349 stress — randomized fuzz (invariants hold for any junk OCR output)", () => {
  it("300 random word sets never throw, never leak a non-finite item, and read a finite confidence", () => {
    const rnd = mulberry32(0xB349);
