...
### B527 — Claude↔Planyr connector: chat with Claude on your own account + propose Scheduler changes `[Scheduler / integration]` (feature)  *(owner-requested 2026-06-27: "integrate my Claude projects… chat with Claude within the app and maybe it adjusts my schedule"; chose connector-first, schedule-only, suggest-and-approve)*
`[x]` **Code tranche shipped (branch `claude/claude-app-integration-d1s30x`).** A minimal **MCP server** (`server/scheduler/` + same-origin endpoint `functions/api/mcp/[[path]].js`) lets the Claude Michael already pays for (claude.ai chats/Projects, or Claude Desktop) **read the live Scheduler and propose changes** — proposals land as `pending` rows in the existing `planar_suggestions` table and surface in the Scheduler's **Review panel** for one-click Approve/Dismiss. Claude never edits the schedule directly (suggest-only). Reuses the Scheduler's own Supabase (anon key, already public; ref `ksetjztkplttbcehyicv`) and its built-in suggestion/approve pipeline (`SuggestionsView`, `SGQ_ALLOWED_PATCH`). Tools: `get_schedule`, `propose_task_change`, `propose_new_task`; field allow-list mirrors the Review panel so a proposal can't carry a rejectable field. 28 unit tests (`test/scheduler.test.js`), lint/test/build green.
- **Dormant until provisioned** (honest 503/401, never open): needs `SCHEDULER_SUPABASE_URL` / `SCHEDULER_SUPABASE_ANON_KEY` / `SCHEDULER_CONNECTOR_TOKEN` in Cloudflare Pages env + a one-time `public/sequence/db/suggestions_rls.sql` run on the Scheduler project. Owner steps in `OWNER-TODO.md`; end-to-end + OAuth-for-claude.ai-web check in `VERIFICATION.md` **V153**.
- **Phase 2 (deferred, owner's choice):** the in-app chat box inside Planyr — same suggestion tools but talks to Claude through the metered API (own key, per-message cost), following the `server/filing/` + `functions/api/file.js` pattern. Revisit after the connector is in use.
