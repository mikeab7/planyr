...
### B410 — Auto-filing: per-page discipline + multi-discipline split (read a whole bound set, file each block right) `[Doc Review / auto-filing]` (feature)  *(owner-requested 2026-06-23: "test all the Drive files for sheet-title / file-name recognition, and if a drawing has multiple disciplines split it up properly"; minted **B410** = highest B# (B409) + 1)*
`[ ]` **Intelligence + read-path SHIPPED on branch `claude/planner-drive-file-testing-r0vh2a` — 1251 tests · lint 0 · build green; validated against 7 real Drive PDFs (harness in `scratchpad/`).** ONE product decision left for the owner (below) before the multi-discipline *actuation* (creating the separate filed sets) lands.
- **What shipped:**
  - `src/shared/files/disciplineSplit.js` (pure, 13 unit tests) — **prefix-first** per-page discipline (a sheet code's letter, e.g. `A5.00`→Architectural, `M-1`→Mechanical, is the engineer's own filing code → it overrides a keyword tally that stray cross-references skew) + **sticky smoothing** (cover/notes/unreadable pages join their block; a lone codeless misread is pulled to two agreeing neighbours) → discipline **segments** + merged **sets**, each set qualifying as standalone only with ≥2 pages OR its own sheet code (so a lone cross-ref page never becomes a junk 1-page filing — the "misfile worse than unfiled" rule).
  - `titleBlockParse.disciplineFromSheetNumber()` exported as the single source for prefix→discipline.
  - Read path now reads **every** page (`pdf.extractAllPagesText`) and the file's discipline is a **majority vote** (`localRead.js`) — fixes the old "read first 2 pages → label the whole file" misfile. Decision gains `multiDiscipline` + `sets` (per-discipline page ranges); both drawers surface "Also contains Structural (4p)…".
- **Real-file findings (Drive corpus, all <10 MB — see blocker):** make-ready package was mis-filed **Fire Sprinkler**, now **Architectural** (correct dominant); Mesa Mechanical B1 was mis-filed **Architectural**, now **Mechanical**; **Bergstrom Arch IFP correctly splits into Architectural 7p + Structural 4p**; single-discipline sets (surveys, fire) unchanged. Two honest gaps: (a) a **scanned/image-only** set (Mesa Electrical B1) has no text → reads nothing (needs the OCR path wired into *filing*, not just the stitcher); (b) **date recognition** sometimes grabs an old base date over the issue/revision date.
- **⛔ OWNER DECISION (actuation):** when a file is multi-discipline, how should the split physically manifest? (1) **Split the bytes** into one PDF per discipline, each filed in its own folder — cleanest, but needs a PDF-writer dependency (`pdf-lib`); (2) file the one combined PDF under each detected discipline (no new dep, but every folder holds the whole file); (3) file under the dominant only + one-click "also file under X (pages …)". Until decided, files file under the corrected dominant discipline with the others flagged.
- **⛔ BLOCKER (validation):** the Drive connector caps downloads at **10 MB**, so the big real combined sets (GPL Civil IFP 125 MB, Jacintoport MEP 13.6 MB, the full Mesa COH sets) couldn't be pulled here to validate the split on the richest cases. Need either a higher-limit access path or owner-side spot-checks.
