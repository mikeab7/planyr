...
### B626 — Metes-and-bounds parser produced a degenerate "there-and-back" sliver for one Grand Port deed `[Site Planner]` (bug)  *(surfaced 2026-07-02 running the B625 alignment on the real Grand Port data; minted **B626** = B625 + 1)*
`[?]` On the real Grand Port project (`sites` id `smqfy2r7pdec`, Chambers County), the plotted deed labelled **"94.91 acres"** (markup `e79374qioqog`) is a **0.38-acre, 119-ft-wide sliver** — the traverse runs up one side and back down nearly the same line instead of closing a ~95-acre polygon. The sibling **"Tract 1 – 94.53 Acres"** deed from (presumably) the same document parsed into a proper ~95-ac ring, so the failure is specific to that one description's wording — `parseTracts`/`coursesOf`/`governingDist` likely latched onto the wrong distance or dropped a run of calls. **Blocked on input (only the owner has it):** the plotted markup keeps only the *parsed* `calls`, not the source text, so I can't reproduce the mis-parse. **To fix:** need the original "94.91 acres" legal description (the .docx/PDF or pasted text) → add it as a fixture to `test/metesAndBoundsDeeds.test.js`, find why it slivered, harden the parser, re-verify it plots a closed ~95-ac ring. (Not an alignment bug — the B625 "Align to parcel" fix is unrelated and already shipped.)
