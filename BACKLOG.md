...
### B689 — Site Planner error-boundaried on EVERY load ("Cannot read properties of undefined (reading 'length')") — a null/husk parcel in a persisted record poisoned the site list `[Site Planner / model]` (bug — CRITICAL, owner-reported crash) #site-planner #persistence  *(owner chat 2026-07-07 — incognito tab on planyr.io: "Site Planyr hit an error and couldn't load… Try again/Reload/Ctrl-Shift-R all send me right back." **B689** = highest B# across both files (B688) + 1. Branch `claude/site-planner-incognito-error-49h3as`. DEDUPE-FIRST — grepped Open/⏳ Verify/Done for `length`/`crash on load`/`error boundary`/`husk`/`null parcel`: B54/B209 are different classes (fetch-wipe, silent save); no prior item covers a malformed collection entry crashing the load render. Net-new.)*
