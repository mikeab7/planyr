...
- **REMAINING — the wiring + a now-FORCED architecture decision (⚠ key finding 2026-06-24):** caching esri-leaflet's server-rendered `dynamic`/`esriImage` exports is constrained by CORS. Those government hosts (FEMA, NWI, TxRRC, COH utilities) render via a **CORS-exempt `<img>`** *because they don't send cross-origin headers* (see `layers.js:370`). So **the app-level blob approach (A) is INFEASIBLE for them** — a browser `fetch()` of a non-CORS host is blocked, and an opaque response can't be read into IndexedDB as a usable image. The IndexedDB foundation (`gisImageCache.js`) therefore fits only **same-origin / CORS-clean** resources (e.g. the `/api/mapillary` proxy), NOT the headline gov imagery.
  - **The only mechanism that can cache the cross-origin gov imagery = a Service Worker + Cache API** (it can store the OPAQUE export responses and replay them to the CORS-exempt `<img>`). Recommended design to contain live-site risk: **scope the SW fetch handler to the known GIS export hosts ONLY** (an allowlist) and pass every app/asset request straight through untouched — so it can never serve a stale app bundle. Caveats to weigh: opaque responses pad storage quota heavily (~7 MB each in some browsers), and can't be inspected (age must be tracked in a side index).
  - Decision pending owner sign-off (a SW on the live site). Until then: `gisImageCache.js` stands as the tested substrate for any same-origin/CORS-clean caching.
