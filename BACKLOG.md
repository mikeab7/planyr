...
- **Follow-ons (small, filed not lost):** physically move a demoted (superseded) file's Drive object to `02. Archive` when it's replaced (display already correct); ~~migrate PRE-tree uploads' bytes into the tree~~ **DONE — B660** (the owner asked for it same-day); drop-onto-a-selected-folder manual filing.
