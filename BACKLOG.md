...
### B438 — GIS layer IMAGERY caching (stale-while-revalidate) `[Site Planner / GIS]` (feature) — roadmap Track-1 #1; IN PROGRESS  *(2026-06-24)*
`[ ]` **Goal:** county/agency layer *pictures* (FEMA flood, wetlands, utilities) paint INSTANTLY from a saved copy when reopening a previously-viewed site, survive a county server outage, and always show a visible "as of Xm ago" age (screening-only — never a stale picture mistaken for live). The headline value is the imagery itself, per-map-view (panning to a brand-new area still needs a live pull).
- **DONE this session — the cache foundation:** `lib/gisImageCache.js` — an IndexedDB-backed blob SWR cache (roomy in-browser store; pictures are too big for the localStorage `gisCache`). Injectable async store + clock so the pure SWR/eviction logic unit-tests with no IndexedDB/network; ~50 MB budget with oldest-first eviction; reuses `gisCache`'s `formatAge`/`isStale` so the age/stale UI path is identical; degrades to live-only when IndexedDB is unavailable. 12 unit tests (`test/gisImageCache.test.js`). Not wired to layers yet.
- **REMAINING — the wiring + a now-FORCED architecture decision (⚠ key finding 2026-06-24):** caching esri-leaflet's server-rendered `dynamic`/`esriImage` exports is constrained by CORS. Those government hosts (FEMA, NWI, TxRRC, COH utilities) render via a **CORS-exempt `<img>`** *because they don't send cross-origin headers* (see `layers.js:370`). So **the app-level blob approach (A) is INFEASIBLE for them** — a browser `fetch()` of a non-CORS host is blocked, and an opaque response can't be read into IndexedDB as a usable image. The IndexedDB foundation (`gisImageCache.js`) therefore fits only **same-origin / CORS-clean** resources (e.g. the `/api/mapillary` proxy), NOT the headline gov imagery.
  - **The only mechanism that can cache the cross-origin gov imagery = a Service Worker + Cache API** (it stores the OPAQUE export responses and replays them to the CORS-exempt `<img>`).
  - **OWNER CHOSE the host-scoped Service Worker (2026-06-24) — BUILT + VERIFIED on the branch, pending ship to production:**
    - `public/gis-sw.js` — caches ONLY cross-origin ArcGIS imagery/tile requests (`/MapServer/export`, `/ImageServer/exportImage`, `/{MapServer,ImageServer}/tile/{z}/{y}/{x}`, generic `{z}/{y}/{x}`); SWR (instant cached paint + background revalidate; on network fail serves the cached copy = outage survival); caches OK **and OPAQUE** responses; versioned cache (`planyr-gis-v1`) cleaned on activate; oldest-first trim to `MAX_ENTRIES` (120). **Every same-origin request is passed straight through (no `respondWith`)** → can never serve a stale app bundle.
    - `lib/gisSwRules.js` (pure canonical matcher + trim, inlined into the SW) + `test/gisSwRules.test.js` (10 tests incl. the "never same-origin app assets" safety lock); `lib/registerGisSw.js` (fail-safe, secure-context-only, after-load) wired in `src/main.jsx`.
    - **Headless-verified `ui-audit/verify-b438-gis-sw.mjs` (5/5):** registers + activates + controls the page; caches the cross-origin tiles (the app's real basemap — 37 cached); NO app/same-origin leak; app shell still renders. (Sandbox can't reach the real gov hosts → tile response mocked at the network layer; match rules unit-tested.)
  - **Follow-ups (not blockers):** a precise per-layer "as of Xm ago" age badge for SW-cached gov layers (opaque responses carry no readable age → needs a small side index; SWR keeps a shown copy ≤1 view-load stale meanwhile). The IndexedDB `gisImageCache.js` remains the tested substrate for same-origin/CORS-clean caching if wanted later.
- **Data footprint:** ~150 KB per layer-view; a typical site with several layers ≈ 1–1.5 MB; a handful of sites ≈ a few–~10 MB; capped ~50 MB with auto-cleanup (note the opaque-padding caveat above for the SW path).
