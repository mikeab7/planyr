...
`[?]` The owner dropped a real 3-page GPL topo slice ("Pages_from_GPL_Civil_Compressed_topo.pdf"). **B412 (shipped) now reads their bare title-block codes (C-2/C-3/C-4) so the three group as one "Topographic Survey · C-2–C-4 · 3 sheets" set** — but they still won't AUTO-stitch, and that's data-limited, not a bug: each sheet says **"NOT TO SCALE" + "FOR REFERENCE ONLY"**, has a **tiny text layer (9 items)**, and carries **NO "MATCH LINE … SEE SHEET" text** — the entire signal `autoPlaceGroup` (B337) needs. The drawing/contours/match-lines live in the raster image, not the text. So precise automatic alignment requires reading the IMAGE. Options for the owner:
- **(A) Image-based auto-stitch (the real fix, B340's "geometric edge-line stitch fallback").** Render adjacent sheets, correlate the linework across the shared edge to find the offset, place them. Bigger CV build; verify headlessly via Playwright (screenshot the composite). The genuinely-correct automatic answer.
- **(B) Sequence pre-seed.** Since they now group, auto-arrange the set in sheet order (edge-to-edge) as the manual-Align STARTING layout (honestly tagged "not aligned"), so the owner drags-to-finish from a sensible arrangement instead of scattered sheets. Ships quickly; not precise.
