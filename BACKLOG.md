...
### B591 — Freshly drawn polyline (or any markup) vanishes mid-session: a per-tab id counter let a new item reuse a tombstoned id, which `mergeSiteContent` then stripped `[Site Planner / Persistence]` (bug) — DATA-LOSS, critical  *(owner-reported 2026-06-30 on planyr.io — "I drew a polyline and it disappeared… could it be a save/overwrite issue?"; arrived as "NEW-1"; minted **B591** = highest real B# across both files (B590) + 1; SAME symptom family as B134 but a DISTINCT, newly-found root cause — see the fold note; branch `claude/polyline-save-overwrite-u3l49r`)*
...
- **Verified:** `npm run lint` **0 errors** · full suite **2072 green** (+7) · `npm run build` green (lazy split intact; `src/shared/ids.js` folds into the planner chunk). The signed-in, two-tab live reproduction on planyr.io (sandbox is logged-out, cross-tab needs auth) = **V183**.
- **Dedup / fold vs B134:** same "work disappears" symptom FAMILY, but B134's open candidates were #3 (debounce-flush) and #4 (silent save error) — this is a **distinct sixth root cause** (id reuse → tombstone strip). Kept as its own item with its own test path; B134 amended to cross-reference. Ships with **B592** (the detection half).

### B592 — The "changes aren't being saved" notice fired AFTER the loss; the on-device read-back verify was count-only, blind to a same-count drop `[Site Planner / Persistence]` (bug)  *(owner-reported 2026-06-30 — "the notification popped up too late"; arrived as "NEW-2"; minted **B592** = B591 + 1; same branch; ships with B591; the open half of B134's #4 + B125's loud-failure scope)*
`[x]` **Fixed this session — membership-anchored read-back + a cross-tab merge tripwire. *Moves to BACKLOG-DONE on merge.***
- **Why the notice lagged (found in the same audit):** the immediate B458 mirror verifies the on-device write by **count** (`drawnCount got < want`). A same-count swap — one item dropped while another lands, exactly the B591 tombstone/id-collision fold — leaves `got==want`, so the count check is **blind** to it; the loss only surfaced later when the thinned cloud push tripped a CAS conflict / the user noticed. So the indicator was a lagging symptom, not an edge-of-loss alarm.
- **Fix:**
  ...
  - **Cross-tab merge tripwire:** the `storage` handler now asserts the union never drops a live id without a tombstone explaining it; if it ever does (a future regression of the B591 class), it goes LOUD to telemetry (`merge-dropped-live`) instead of silently shrinking the canvas. Reuses the B468 telemetry pattern.
...
- **Verified:** covered by the B591 gate (lint 0 · 2072 tests green · build green). The live signed-in confirmation that a genuinely non-landing save now flips the badge promptly is folded into **V183**.
...
> **Amendment 2026-06-30 — a DISTINCT sixth root cause in this same symptom family was found + fixed: B591/B592.** An owner report ("I drew a polyline and it disappeared") triggered a fresh adversarial audit that pinned a cause NOT in the #1–#5 list: drawn-element ids came from a per-tab counter that `ensureIdAbove` re-seeded only from parcels+els, so a freshly drawn markup could reuse a **tombstoned** id (after a reopen) or **collide** across two tabs — and `mergeSiteContent`'s tombstone filter then stripped the live item mid-session (then persisted the loss). Fixed by salting `uid()` (new `src/shared/ids.js`) + seeding `ensureIdAbove` from all collections+tombstones (**B591**), and the count-only read-back that let it pass silently was made membership-anchored (**B592**). This does not close #3/#4 (still the open debounce-flush / silent-error cases) but removes the most-likely real-world trigger of "work vanished on its own." Live two-tab signed-in repro = V183.
