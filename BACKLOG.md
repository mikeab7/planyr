...
- **`[ ]` Stage B (deferred, low priority) — the live sites map STAYS on localStorage, on purpose.** It's small post-Shipment-1, and its two-window guard (B127/B314) depends on localStorage's *synchronous cross-tab* visibility + the `storage`-event merge in `SitePlanner.jsx`. Moving it means rebuilding that on a **BroadcastChannel** (reuse `src/shared/presence/` — `multiTab.js`/`editorLock.js`; the editor lock already serializes editing) + preserving the quota-degrade path, verified with a real TWO-WINDOW + signed-in test. Not worth the risk now that the cap is non-binding — **owner agreed 2026-06-25** ("if it doesn't really matter, we don't need to"). Cloud (Supabase) stays the single source of truth.
