...
- Verify: live
- Recurrence: 2026-07-05 — owner repro (chat "NEW-5", DEDUPE-FIRST fold — same error string, same first-paint null-`projects` read): create a project via **Start blank → Schedule tab** → "This view failed to load — Cannot read properties of null (reading 'projects')" renders **behind the connect-a-schedule modal**. This is the **fresh-project / no-schedule-linked** path, deterministic (not only the cold-load race) — expected is a clean empty state only, so the fix must gate first paint for the null-data state, not just shorten the race.
- **Blocker update 2026-07-05:** the "sandbox can't boot the embed" half of the below is STALE in the current environment — the React/Babel/Supabase CDNs are reachable through the egress proxy, so a headless repro of the fresh-project path is now possible.
