...
### B637 — Scheduler embed: first COLD boot throws `Cannot read properties of null (reading 'projects')` + the loader overlay exceeds the 6s backstop (self-recovers) `[Scheduler / robustness]` (bug — first-paint hardening)  *(Cowork live verification 2026-07-03, from V153; minted **B637** = highest real B# across both files (B636) + 1 — B635 is a concurrent session's Gantt-labels item, B636 is the eager-race follow-on filed this session)*
