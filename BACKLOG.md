...
### B692 — Sharing a project silently REVERTS: any ordinary save from the owner's open tab overwrote `sites.team_id` back to null, locking the collaborator out `[Site Planner / Review / teams · RLS]` (bug — data-integrity class) #site-planner #persistence #auth  *(owner live repro 2026-07-08 with the Cowork session: right-clicked Goose Creek → Share to michael.butler@hillwood.com → the site APPEARED on the test account, then broke — red cloud, elements gone after reload, no conflict toasts. Cowork's DB forensics found team "HIP Houston" + both members present but 0 sites linked, and filed it as "Share never sets team_id" (their NEW-1). Minted **B692** = B691 + 1.)*
`[x]` **IMPLEMENTED + unit-verified this session; parks in ⏳ Verify (Verify: live — the share round-trip is two-account/auth-only) → V232.** Sharing now survives every save: the sharing pointer can only be changed by the explicit Share/unshare action, never by a content autosave.
- Verify: live
...
- **⏳ V232 (live, two accounts — both already exist):** see VERIFICATION.md.
- DEDUPE-FIRST: net-new (no prior B# covers the share flow or team_id write-isolation); B486 (Done) is the non-owner rehome guard this bug walked around by BEING the owner; B455/B460 conflict lineage unrelated. The same owner brief's NEW-3 = **B690** (already fixed + merged 2026-07-08) and NEW-4 = **B672 (×2)** (already fixed + merged 2026-07-08) — the live session ran on build 40884e6, which predates both fixes; no new numbers minted.
