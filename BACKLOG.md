...
### B620 — Inline line labels (text riding along a line) as an alternative to callouts `[Site Planner]` (feature)  *(owner-dropped 2026-07-02 as "NEW-6"; owner flagged NOT urgent — OK to file and sequence last; minted **B620** = B619 + 1; same branch)*
`[ ]` Let a line/polyline carry an optional label rendered **along the line itself** (e.g. "18″ SANITARY SEWER" sitting on the line) so a simple line annotation needs no separate callout. SVG `<text>` rotated to the segment angle, at the segment midpoint (longest segment for a polyline), auto-flipped so it never reads upside-down, offset slightly above the centerline, with a white casing/halo (reuse the pond-label dark-text + white-casing treatment). Edit via a new "Inline label" field in the line's Properties; store as a property so it moves/rotates with the line. Unlike selection chrome, this label DOES appear in PNG/PDF exports.
