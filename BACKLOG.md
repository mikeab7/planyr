...
### B596 — Centerline road model + multi-point drawing + legacy migration `[Site Planner / drawing]` (feature)  *(owner-dropped 2026-06-30 as "NEW-1"; minted **B596** = highest real B# across both files (B595) + 1; branch `claude/centerline-road-model-hxi45g`)*
...
- **Dedup:** the existing rectangle-road items (**B61, B70, B102, B110, B121, B146**) all concern the rotated-rect road and keep their curb / travel-width / dimension behavior — curved multi-point geometry is **new**, no conflict. The **B70 curb contract is preserved** (width = travel width; 6″ curb each side; 24′ travel = 25′ wide) and the **B146 travel-width dimension callout + click-to-edit** still work (see B598).
- **Verified:** lint 0 · **2128 tests** green · build green · headless `ui-audit/verify-centerline-road.mjs` 7/7 (migration render + UI draw). Auth-only cloud round-trip = **V186**.

### B597 — Road curve geometry: per-vertex sharp/arc/smooth + feasibility-clamped radius `[Site Planner / drawing]` (feature)  *(owner-dropped 2026-06-30 as "NEW-2"; minted **B597** = B596 + 1; depends on B596)*
...
- `roadCenterline(pts, vtx, { defaultRadius })` → a **tessellated dense polyline** (the rendered centerline) that B598 offsets. Per interior vertex `treatment`:
...
- Treatment is **per-vertex and convertible after drawing** (pen-tool style): click a road vertex on the canvas to select it, then flip **sharp ↔ arc ↔ smooth** in the Element panel (Arc seeds the class default radius, editable). No global tool mode, no spacing threshold — dense clicks + the clamp produce smoothness naturally. `minRadiusOfCurvature` (circumradius of consecutive triples) backs the B599 civil check, uniformly for arc and traced/smooth runs.
- **Tests (21):** clamp never overruns a segment; arc tangent at both ends; spline passes through its points; sharp == polyline; tessellation density scales with `tessDeg`; min-radius recovers a known circle + finds the tightest along a mixed alignment; + the B599 class-threshold math.

### B598 — Road surface + curb + dimension render from the centerline (reuse offset; no JSTS) `[Site Planner / drawing]` (feature)  *(owner-dropped 2026-06-30 as "NEW-3"; minted **B598** = B597 + 1; depends on B596/B597)*
...
### B599 — Road class + civil design parameters: defaults + non-blocking min-radius warning `[Site Planner / Setup — Roads]` (feature)  *(owner-dropped 2026-06-30 as "NEW-4"; minted **B599** = B598 + 1; depends on B597)*
