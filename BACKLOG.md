...
### B550 — Scheduler: recursive tree-walks can infinite-loop (hang the tab) on a parentId cycle `[Scheduler / robustness]` (bug — hardening)  *(found 2026-06-27 by the round-10 bug-hunt, scheduler-logic lens; minted **B550** = highest real B# (B549) + 1)*
`[ ]` `public/sequence/index.html` has ~9 recursive parent→child tree-walks with NO cycle detection: `sortByVisualOrder` (load path, called by `normalizeIds`), `getSubtreeIds` (in `outdentTask` + `outdentTaskById`), and the `walk()` closures in the review-task builder, `deleteTask`, `indentTask`/`indentTaskById`, `insertTaskBelow`/`insertTaskAbove`, `collectSubtreeFor`. If the persisted schedule ever contains a `parentId` cycle (A→B→A — from data corruption, a manual edit, or a past bug), the matching walk infinite-loops and **hangs the browser tab** — potentially on every load if the saved data is corrupt, making the schedule inaccessible.
- **Why filed not shipped this lap (defensive + high-churn + regression risk):** there is **no confirmed live trigger** — the current UI logic prevents *creating* a cycle, so this guards against unconfirmed corruption. The complete fix touches the Scheduler's core tree logic across ~9 functions in the **vanilla-JS Babel blob that is under active concurrent development** (B495 etc.), where a wrong edit could break the LIVE scheduler — worse than the defended-against hang. That earns a careful, focused pass (with the esbuild-parse check + ideally a scheduler test), not a same-PR add alongside unrelated fixes.
- **Preferred fix (one place protects all):** add a cycle-sanitize pass at the data-load boundary in `normalizeIds` — walk each task's parent chain with a `seen` Set and, on a detected cycle, null the `parentId` that closes the loop (the task degrades to a root). Then NO downstream walk can ever see a cycle. **Fallback:** add a `seen`-Set guard to each recursive walk (mechanically safe — on valid data every node is visited once, so the guard never short-circuits a legitimate visit; it only stops a cycle). Verify the esbuild parse + that valid schedules are unchanged.
