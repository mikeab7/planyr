...
- **REMAINING blockers before it can ship (each real):**
  ...
  3. **Pixel-precise seam alignment (the hard CV tail).** The match-line is a DASHED line INSET from the page edge (above the title block), and different sheets inset differently — so `autoStitch`'s drawing-area-edge placement only gives correct TOPOLOGY, not a pixel-aligned seam. A correct stitch needs match-line detection (dashed-line find, ~13% row fill defeats a naive max-ink scan) + a parallel-offset solve (correlate crossing linework). Until this lands, placement must stay `aligned:false` → manual Align.
- **Honest note for the owner:** these are "FOR REFERENCE ONLY / NOT TO SCALE" compressed scans; for measured stitching the vector CAD set (or the original 36×48 sheets) would auto-stitch far more reliably. Phase-1 foundation is committed so the next focused session continues from a tested base.
