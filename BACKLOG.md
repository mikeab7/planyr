...
- **REMAINING — the wiring (the larger, riskier part) + an architecture decision:** caching esri-leaflet's server-rendered `dynamic`/`esriImage` exports. Two approaches:
  - **(A) App-level (recommended):** for a cached layer, fetch its `export?...&f=image` URL ourselves → `gisImageCache.swr` → render the blob via `L.imageOverlay(objectURL, bounds)`, refresh on `moveend`, stamp age via the existing `onStatus`. No service worker (lower live-site risk), full control of the per-layer age/stale badge. Cost: reimplements esri-leaflet's dynamic/image render path for those layers (must keep the CORS-clean export fetch + avoid visual regressions vs the current working layers).
  - **(B) Service Worker:** intercept esri-leaflet's own export requests transparently (less layer-code change, also covers tiles), BUT adds a SW to a site that has none — lifecycle/asset-staleness risk on the live app, must scope to GIS hosts only, harder to verify in-sandbox.
  - Then: `esriFeature` vectors via SWR too; age/stale surfaced in `LayerPanel` (already renders `ls.ts`); headless verify a cached layer paints on reload + shows age + refreshes; ship + verify live.
- **Data footprint:** ~150 KB per layer-view; a typical site with several layers ≈ 1–1.5 MB; a handful of sites ≈ a few–~10 MB; capped ~50 MB with auto-cleanup.
