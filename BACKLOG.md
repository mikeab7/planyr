...
- **What the owner gets:** draw a building and it now shows its **structural column grid** + **dock doors** automatically, the way spec industrial is actually framed. Off each dock face is a **speed bay** (the staging strip — default **60′**, the one number the owner cares most about). Behind it the **interior bays are a FIXED typical size** (default **56′** along the docks · **50′** deep — never stretched to an odd number); only the **end bays (length)** and the **rear / centre bay (depth)** flex to close the building (where offices + odd geometry live). **Length and depth typical sizes are independent** (real buildings run e.g. **56′ × 50′**, exactly the owner's case). Cross-dock mirrors a speed bay to **both** long walls with the flex landing in the centre. *(Model corrected per the owner 2026-06-30: flex lives ONLY at the end/rear bays, not by stretching every interior bay.)*
- **Geometry (new pure lib `lib/buildingGrid.js`, dependency-free):** `computeBuildingGrid({length, depth, dock, grid})` → building-LOCAL column-line offsets (`lengthLines` / `depthLines`, each tagged `speed` | `flex` | `std`), bay lists, and a panel summary. `divideSpan` lays FIXED interior bays at the module + flex bays (ends/rear/centre) that absorb the residual; the interior count is chosen so each flex bay stays near the module (no slivers, nothing oversized), and a flex bay landing on the module reads as a normal bay. `resolveGridSettings(el, settings)` resolves the plan defaults under per-building overrides (the existing `…Override` pattern). 32 unit tests (`test/buildingGrid.test.js`): interior-bays-stay-the-module, end/rear/centre flex, speed-bay placement, cross-dock symmetry + too-shallow guard, independent length vs depth modules, dock-`none`, degenerate depth, out-of-band-target safety, empty footprint.
