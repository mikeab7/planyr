...
### B565 — Schedule: replace the floppy "Save" button with the unified cloud save-status badge (auto-save parity with Site Planner) `[Scheduler / Persistence]` (feature)  *(owner-dropped 2026-06-29 as "NEW-1"; minted **B565** = highest real B# across both files (B564) + 1 — renumbered from a provisional B562 that collided with main's `importLegacyIntoCloud` B562; branch `claude/scheduling-auto-save-parity-dlg65g`)*
