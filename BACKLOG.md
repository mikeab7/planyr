...
### B645 — Per-project standard folder tree, user-editable in-app, with continuous one-way sync to Google Drive `[Doc Review / Library / storage · drive-integration / persistence]` (feature — umbrella, multi-phase)  *(owner brief 2026-07-04, arrived as "NEW-1"; minted **B645** = highest real B# across both files (B644) + 1; branch `claude/planyr-folder-drive-sync-0k8h0t`. Deduped against the Drive / storage-adapter / auto-filing / file-index cluster — this FOLDS INTO the existing storage adapter + Supabase file-index rather than adding a parallel one; see the **Dedup** note.)*
...
> **✅ CODE SHIPPED this session (branch `claude/planyr-folder-drive-sync-0k8h0t`) — lint 0 · 2,477 tests (46 new) · build green · logged-out E2E smoke passing. REMAINING = the signed-in + live-Drive click-through only → `VERIFICATION.md` V205 (scaffold/template/independence) + V206 (sync-in-place + delete-safety), and the owner runs `project_folders.sql` once (OWNER-TODO).** What shipped, end-to-end:
...
### B629 — Drive-backed county PARCEL snapshot cache so outages stop breaking the map `[Site Planner / GIS]` (feature — multi-phase)  *(owner-requested 2026-07-03 — "I keep having this [Chambers GIS down] issue … cache a copy of the entire Chambers County CAD and just update it … same for Fort Bend … we've got a Google Drive with 32 gigs"; scope refined to "all except Harris" → Chambers + Waller + Fort Bend. Minted **B629** = highest real B# across both files (B628) + 1; plan `/root/.claude/plans/okay-um-i-mean-elegant-haven.md`; branch `claude/chambers-county-gis-down-ttqfnv`.)*
