...
- **Follow-ons (small, filed not lost):** physically move a demoted (superseded) file's Drive object to `02. Archive` when it's replaced (display already correct); optionally migrate PRE-tree uploads' bytes from the flat `project-<id>/<discipline>` Drive path into the tree (display already correct — placement is derived); drop-onto-a-selected-folder manual filing.
