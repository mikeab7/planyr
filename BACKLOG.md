...
- **Verified:** `npm run lint` **0 errors** · full suite **1874 green** (16 new) · `npm run build` green (SelectionChrome is its own shared lazy chunk; planner/review chunks intact) · headless **PASSED logged-out**: `ui-audit/verify-multiselect.mjs` (Site Planner 9/9: marquee tool + lit feedback + neutral chrome + multi-move + multi-delete + single-undo + Ctrl-click toggle) and `ui-audit/verify-multiselect-docreview.mjs` (Document Review 8/8 over the fixture PDF: rail tool + lit + marquee select + multi-delete). The signed-in confirm that a multi-move/delete on a CLOUD-loaded site/review survives reload (rides the unchanged autosave; sandbox is logged-out) is **V176**. Single-select keeps its existing resize/rotate handles (richer than grips); only multi-select + marquee use the neutral chrome (deliberate).
