...
`[ ]` A split parcel and its children occupy the SAME ground, so any active set containing a parent AND a child double-counts that acreage. Repro: start with Parcel 3 (16.40 ac) → "Split a parcel" cuts it into two children (2.52 ac + 13.87 ac); today all three — parent + both children — stay active → Yield/Analysis sums 2.52 + 13.87 + 16.40 ≈ 31.8 ac against a real ~16.4. Manually deactivating one child doesn't fix it: parent + remaining child still overlap (~30 ac).
- Verify: live
- Origin: filed 2026-07-04 from chat
- **Invariant to enforce (must always hold):** the active set that feeds Yield/Analysis is spatially NON-OVERLAPPING.
- **Expected:**
  - **Split is a replacement, not an addition** — one operation creates + activates the two children AND deactivates the parent. Correct acreage by default, no manual cleanup.
  - **Retain the parent** in the list marked "split / superseded" (greyed, non-counting) with the two children nested under it, so the original real parcel stays visible.
  - **Name children by parent lineage:** Parcel 3 → 3A / 3B (not fresh 1 / 2 IDs). Support repeat splits (3A → 3A1 / 3A2 or similar).
  - **Mutual-exclusion guard:** a parent and its split children can NEVER both be active — toggling a parent active auto-deactivates its children and vice-versa, so the overlap can't be reintroduced by hand.
- Pairs with **B652** (the general overlap-detection safety net). TOMBSTONE-DELETES / persistence: any new lineage field (`parentId` / `supersededBy`) must round-trip through the Site Model + cloud merge without resurrecting a superseded parent as active.

### B652 — Warn when active parcels overlap in Yield/Analysis `[Site Planner / Analysis]` (feature) #site-planner #yield #ui  *(owner brief 2026-07-04, arrived as "NEW-2"; **B652** = B651 + 1. Safety net beyond B651's split fix; DEDUPE-FIRST — net-new, no prior overlap-detection item.)*
`[ ]` Safety net beyond splits. Analysis/Yield detects when any two active parcels have overlapping geometry (pairwise intersection above a small area tolerance) and surfaces a NON-BLOCKING warning banner — e.g. "Active parcels overlap — acreage may be double-counted" — naming the offending parcel IDs. Catches manual mistakes too (activating a parent + child, or two hand-drawn parcels that overlap), not just the B651 split case.
- Verify: sandbox
- Origin: filed 2026-07-04 from chat
...
### B644 — Scheduler embed: first COLD boot throws `Cannot read properties of null (reading 'projects')` + the loader overlay exceeds the 6s backstop (self-recovers) `[Scheduler / robustness]` (bug — first-paint hardening)  *(Cowork live verification 2026-07-03, from V153; minted **B644** = highest real B# across both files (B643) + 1 — B635–B642 are concurrent sessions' items, B643 is the eager-race follow-on filed this session)*
`[ ]` On planyr.io, the FIRST cold Site→Schedule switch of a session took ~10.5s with the "Assembling schedule…" overlay up (exceeding the documented 6s loader backstop) AND the embed threw a one-time `⚠️ This view failed to load — Cannot read properties of null (reading 'projects')`. A direct re-nav then loaded cleanly (Gantt rendered, no error) — a **transient cold-load race that self-recovered**, not a hard failure, but a real first-paint defect.
- **Why filed, not fixed this session (not a blind edit):** the error is a null `X.projects` read during first paint before the async `hs-v1` load resolves, but `public/sequence/index.html` has **139** `.projects` accesses and is under active concurrent development (merge-conflict + anti-drift-mirror risk); pinpointing the exact cold-paint site needs a **visible-window repro** — the sandbox can't boot the embed (its React/Babel + Supabase CDNs are egress-blocked). A guess-and-guard here risks a new bug or a lockstep drift. Related to V151 (MasterView/Dashboard null-projects robustness).
- **Fix when picked up (focused pass, visible window):** gate the top-level scheduler render (+ MasterView/Dashboard) on a non-null `data`/projects list so first paint shows the loader, not a crash; confirm the cold-boot overlay stays within the 6s backstop (relates to B494/V153); keep any `src/` engine mirror in lockstep (anti-drift). Live re-check → **V153** (cold path).
...
`[ ]` **Phase 1 MACHINERY shipped + unit-tested this session; NOT user-live yet** — it stays inert (no behaviour change) until the nightly builder populates Drive, which needs an owner secrets step (see OWNER-TODO) + the Chambers/Waller source confirmed on the first CI run. Then a lot draws + selects from Planyr's saved copy even with the county server 100% down.
...
- **REMAINING to go live (Phase 1b):** (1) OWNER adds `GOOGLE_CLIENT_ID/SECRET/REFRESH_TOKEN` (+ `PLANYR_STORAGE_BACKEND=drive`) as **GitHub Actions secrets** — same values as Cloudflare, ⛔ do NOT re-mint the refresh token (OWNER-TODO). (2) Confirm the Chambers/Waller **TNRIS StratMap** source URL in `build-parcel-snapshot.mjs` — the `feature.tnris.org` mirror is egress-blocked in the sandbox so unverified here; the first CI run either succeeds or opens a `@claude` issue (keep-last-good means the app is unaffected either way). (3) Trigger the workflow once to seed the first snapshots. (4) Live in-browser click-through = **V199**.
...
### B484 — Renderer freezes (~30 s main-thread stalls) during PDF title-block reading and heavy map/parcel ops `[Doc Review + Site Planner / perf]` (task — perf) #doc-review #site-planner #perf  *(Cowork signed-in verification 2026-06-26, "NEW-5"; minted **B484**)*
...
_(none yet — new `Verify: live` items land here after implementation.)_
