...
- **PROGRESS (multi-PR sweep — stays Open until all parts land):**
  ...
  - ⏳ **(5A)** token repointing — landing incrementally inside the PRs that already touch each region (pond-area cream → B655 card PR; control-def hex → 5B extraction; toast/banner semantic pairs → dedicated 5B commit).
  - ⏳ **(5B)** shared control primitives (`src/shared/ui/controls.jsx`) + staged migration Library → Auth/Team/DocReview/Stitcher → Site Planner. *(not started)*
