...
### B569 — Multi-select in the default pointer: Ctrl/⌘ + Shift-click modifiers + multi-selection operations `[Site Planner + Document Review / shared selection]` (feature)  *(owner-dropped 2026-06-30 as "NEW-1"; minted **B569** = highest real B# across both files (B568) + 1; branch `claude/multi-select-default-pointer-aycvyr`)*
...
- **Verified:** `npm run lint` **0 errors** · full suite **1874 green** (16 new) · `npm run build` green (SelectionChrome is its own shared lazy chunk; planner/review chunks intact) · headless **PASSED logged-out, 20/20**: `ui-audit/verify-multiselect.mjs` (Site Planner **12/12**: marquee tool + lit feedback + neutral chrome + multi-move + multi-delete + single-undo + Ctrl-click toggle + the lone-measure regression) and `ui-audit/verify-multiselect-docreview.mjs` (Document Review **8/8** over the fixture PDF: rail tool + lit + marquee select + multi-delete). The signed-in confirm that a multi-move/delete on a CLOUD-loaded site/review survives reload (rides the unchanged autosave; sandbox is logged-out) is **V176**. Single-select keeps its existing resize/rotate handles (richer than grips); only multi-select + marquee use the neutral chrome (deliberate).
- **Dedup:** distinct from B443/B420 (parcel Shift-merge — left divergent, as the brief sanctions) and B423/B431 (the shared markup ENGINE — this adds the shared SELECTION layer it never had). Depends-on/ships-with **B570** (the marquee tool).

### B570 — Marquee (box-select) tool: drag a rubber-band box on empty canvas to multi-select `[Site Planner + Document Review / shared selection]` (feature)  *(owner-dropped 2026-06-30 as "NEW-2"; minted **B570** = B569 + 1; same branch)*
`[x]` **Shipped this session alongside B569. *Moves to BACKLOG-DONE on merge.***
...
- **Verified:** part of the same two headless harnesses as B569 (Site Planner + Document Review), both PASSED logged-out; lint/test/build green; signed-in cloud-persistence confirm = V176.
