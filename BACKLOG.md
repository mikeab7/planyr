...
- **Verified (fully, here — no signed-in path owed; the grid is pure canvas, identical logged-in or out):** `npm run lint` **0 errors** · full suite **1878 green** (+27 new) · `npm run build` green · headless `ui-audit/verify-b568-b569-grid.mjs` **8/8**: grid + speed line(s) + door leaves render on a single-load AND a cross-dock building (1 vs 2 speed lines = mirror symmetry; 24 vs 48 door leaves), and the grid correctly vanishes when zoomed out below the gate. Nothing pending in `VERIFICATION.md`.
