...
- **`[ ]` Shipment 2 (in progress, owner-directed "do the best long-term solution now") — move the on-device cache OFF the 5 MB localStorage onto IndexedDB** (gigabytes) via an in-memory **sync façade** + a non-destructive one-time migration, so the cap is gone entirely and the local cache is future-proof. Cloud (Supabase) stays the single source of truth; Drive stays the home for big files. localStorage retained only as a slim sync fallback mirror.
