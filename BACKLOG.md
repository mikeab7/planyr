...
### B686 — The Site Planner inspector / left panel doesn't theme to dark (hardcoded cream) `[Site Planner / UI]` (bug) #site-planner #ui  *(found 2026-07-06 during the B657-5B convergence screenshot pass; minted **B685**, RENUMBERED to **B686** on the merge-in of PR #532 — which independently minted B685 for the Library any-file-type work and shipped it referenced in its title, commits, and `VERIFICATION.md` V227, so that B685 keeps the number and this reference-free Open item moves.)*
`[ ]` In dark mode the top chrome + tabs go dark (correct), but the Site Planner's **left inspector column and its cards stay warm cream** — the surfaces are hardcoded light hex, not theme tokens: the panel column `#efeadf` / `#efe9dd` (`SitePlanner.jsx` ~:7751/:9334), the readout boxes `#f8f6f0` (~:10032/:10254/:11313), the `Section` border `#ece6d9` (~:12024), and the YieldPanel local palette (`panelBg:"#FBF8F2", border:"#E7DFD2", cardBg:"#F2ECE1"` ~:12143). So a dark-theme user gets a bright cream inspector against dark chrome — the "chrome themes WITH the app" KEY DECISION, unmet for this surface.
- Verify: live (theme-dependent rendering — check both themes in the real app)
- Origin: found 2026-07-06 from the B657-5B pass
- Scope note: this is a deliberate-looking "drafting paper" cream that was never tokenized; fixing it = repoint those surfaces to `--surface-*`/`--border-default` (and give the YieldPanel `Y.*` palette a dark variant) so the whole inspector themes. Non-trivial (many hardcoded spots + must clear the contrast audit in dark), so filed rather than rushed. **Do NOT half-fix** (e.g. theming a Section border while its panel stays cream creates a dark-border-on-cream mismatch — exactly the trap that surfaced this).
