...
- **What the owner gets:** draw a building and it now shows its **structural column grid** + **dock doors** automatically, the way spec industrial is actually framed. Off each dock face is a **speed bay** (the staging strip — default **60′**, the one number the owner cares most about). Behind it the **typical bays flex within an industry band (50–58′)** toward a target to *close the grid cleanly*; only a genuine leftover spills into the **end / rear bays** (where offices + odd geometry live). **Length and depth targets are independent** (real buildings run e.g. **56′ × 50′**, exactly the owner's case). Cross-dock mirrors a speed bay to **both** long walls with the leftover landing in the centre.
- **Geometry (new pure lib `lib/buildingGrid.js`, dependency-free):** `computeBuildingGrid({length, depth, dock, grid})` → building-LOCAL column-line offsets (`lengthLines` / `depthLines`, each tagged `speed` | `flex` | `std`), bay lists, and a panel summary. `divideSpan` is the flex-to-band core (primary = uniform in-band nearest target → no residual; fallback = standard module + residual end/rear/centre bay). `resolveGridSettings(el, settings)` resolves the plan defaults under per-building overrides (the existing `…Override` pattern). 27 unit tests (`test/buildingGrid.test.js`): speed-bay placement, cross-dock symmetry + too-shallow guard, flex-to-band selection each direction, independent length vs depth targets, residual ends/rear/centre, dock-`none`, degenerate depth, empty footprint.
