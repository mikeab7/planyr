...
### B481 — Phone header clips its controls instead of scrolling sideways `[Shared header / mobile]` (bug)  *(surfaced 2026-06-27 driving the Cowork batch-4 V11 phone-layout item under REAL mobile emulation, which the Cowork `resize_window` tool couldn't produce; minted **B481** = highest real B# across both files (B480) + 1)*
`[x]` **Filed AND fixed + headless-verified the SAME session per STANDING RULE #1 (branch `claude/resume-planner-persistence-36zqh1`).** *NOTE — push-to-branch only this session; no PR/merge to `main` opened. Kept in Open until merged.*
...
- **Dedup:** net-new; the shared header's mobile responsiveness. Distinct from B113 (planner-body phone mode, already live) and from B475/B476 (header *data*-source / save-messaging bugs, not layout).
...
### B479 — Persistence "state-of-the-art" perf refactors (the deferred tail of the B477 review) `[Site Planner / Persistence]` (enhancement/perf)  *(spun out of the B477 adversarial review, 2026-06-26; renumbered **B478→B479** on merge — a concurrent `main` (#368) took B478 for the resume-into-planner fix; these are the findings the verifiers marked fixNow=false — real but larger/riskier than a same-session patch, NOT data-loss. B477 shipped all the data-loss + honesty fixes.)*
