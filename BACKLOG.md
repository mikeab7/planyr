...
### B473 — DATA LOSS: a NEW site's edits can vanish on reload when signed in (root cause un-pinned; mitigated + instrumented) `[Site Planner / Persistence]` (bug, DATA-LOSS)  *(owner repro 2026-06-25 on planyr.io, signed in; minted **B473** = highest real B# across both files + 1 — renumbered from a provisional B472 that a concurrent `main` (#355, parking-explode) shipped while this was in flight)*
`[ ]` **Owner repro: brand-new site, single tab, signed in, placed new items → GONE after reload.** Decisively NOT the multi-tab lock (single new tab), NOT a CAS conflict (no other session), NOT the version-history count-dedup (new items change counts). **Headless logged-out repro `ui-audit/verify-new-site-save.mjs` (10/10) PROVES the pure-code path is sound** — create-via-"Start blank" → place items → reload → intact + re-opened. So the loss is **signed-in / cloud-specific** and was NOT pinpointable by static reading (two Explore agents + a full trace all concluded "should work": the URL↔active-project sync syncs a new site into the hash, `bootActiveId`/`applyUser` resume it, and `pullCloud`/`mergePulledSites` preserve a local-only site). I refuse a third wrong-cause guess.
- **Shipped MITIGATION (silent loss → impossible + self-reporting) — the B473 safety net:** the autosave now **reads back** every on-device write; if it didn't persist it fires a LOUD red *"could not be saved on this device"* banner + a `save-verify-failed` telemetry event (no more silent loss). An explicit **"Save now"** menu action force-saves + verifies + shows a provable *"Saved ✓ N items"* (or a loud failure). Reuses B468 telemetry + B458 immediate-mirror. (`SitePlanner.jsx` writeMirror verify + `saveNow` + `localSaveFailed`/`saveNowMsg` banners; `drawnCount`.) lint 0 · 1528 tests · build green · `verify-new-site-save.mjs` 10/10 + `verify-readonly-takeover.mjs` 7/7 (no regression).
- **OPEN — blocked on ONE live signed-in occurrence to pin the exact cause** (the sandbox CORS-blocks sign-in, and the bug is signed-in-only). Next step from a real failure: read `window.pfTelemetry.recent()` + the `public.client_errors` rows + whether the new site's record sits in `planarfit:sites:cloud:<uid>` vs the legacy `planarfit:sites:v1` — which cleanly separates never-saved / saved-to-wrong-store-pre-auth / saved-but-orphaned-on-resume / cloud-insert-rejected. Then fix precisely. (Suspects, each with file: SitePlanner.jsx autosave gate; storage.js `sitesKey()` activeUser timing; SitePlannerApp `bootActiveId`/`applyUser` resume vs URL project; cloudSync.js `cloudUpsert` first-insert.)
