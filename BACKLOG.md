...
- **Owner decision 2026-07-06:** full scope — do BOTH 5A (repoint hardcoded hex → shared tokens) and 5B (extract shared control primitives so every React-shell tab's controls are one system). The **Scheduler embedded app** (`public/sequence/index.html`, iframe) is explicitly **out of scope** — walled module, keeps its purple accent + internals.
- **PROGRESS (multi-PR sweep — stays Open until all parts land):**
  - ✅ **(1)** "Boundary tool (right rail)" → "Parcel tool (right rail)" (empty-state step 3 + Parcel-panel empty state); geometry-sense "boundary" left intact. *(shipped — wording PR)*
  - ✅ **(2)** converged the pond a11y label "Detention Basin" → "Detention Pond" (matches the 4 visible uses + `planStyle.js`); the Yield short-form "Detention" kept as a category shorthand. *(shipped)*
  - ✅ **(3)** assessed — the Map (Site) / Dashboard (Schedule) home-crumb labels are **intentional per B204** and already render through the one shared `ProjectBreadcrumb` chrome; the "Concept" trailing crumb is a Site-only plan concept so its absence off-Site is correct. No code change; working-as-intended. *(If the owner later wants a single home word across tabs, that's a one-line `homeLabel` change.)*
  - ✅ **(4)** File menu "Export JSON" → "Export project file" (+ "Import JSON…" → "Import project file…" for parity; titles updated). *(shipped)*
  - ⏳ **(5A)** token repointing — landing incrementally: ✅ Section border `#ece6d9` → `var(--border-default)` (folded into the shared Section) + ✅ ToolRail hardcodes `#EF9F27`/`#231a0e` → `var(--accent-review)`/`var(--on-accent-review)`. Remaining: the toast/banner semantic light/dark pairs (they already theme — lower priority) fold into the per-surface 5B migrations.
  - ⏳ **(5B)** shared control primitives — ✅ **foundation shipped:** `src/shared/ui/controls.jsx` (Button/ToggleChip/IconButton/Field/Section/MenuItem + the one radius/pad/type scale; token-driven; threaded `accent` prop keeps each module's hue) + the contrast-audit pair (`--on-accent`/`--accent`, passes AA both themes) + first adopter (Library facet chips → `ToggleChip`). **Remaining migrations (own PRs, screenshot-verified): Auth/Team → DocReview/Stitcher → Site Planner** — each changes control sizes on its surface, so paused for an owner look at the converged anatomy before rolling app-wide.
