...
### B542 — Building dimensions mislabeled + mis-bound: "Width/Depth" → dock-relative "Length/Depth" `[Site Planner / massing panel]` (bug)  *(owner-dropped 2026-06-27 as "NEW-1"; minted **B542** = highest real B# across both files (B541) + 1 — verified collision-free vs origin/main, which also tops out at B541)*
`[ ]` The **Selected · Building** panel labels the two plan dimensions **"Width"** and **"Depth"** bound to the raw footprint axes (`el.w`→Width, `el.h`→Depth, `SitePlanner.jsx` ~L8223). For the live example (`el.w 328 / el.h 1159`) that prints **Depth 1159** — physically impossible for a single-load warehouse, because "Depth" is bound to the LONG / dock-parallel edge. Industrial convention: **Depth** = perpendicular to the dock wall (dock face → rear; dock-wall → dock-wall for cross-dock); **Length** = parallel to the dock wall (doors array along it). "Width" isn't a warehouse term and must retire. This building should then read **Length 1159 / Depth 328**.
- **NOT just a relabel — the binding is transposed.** Fix so **Depth drives the dock-perpendicular edge** and **Length the dock-parallel edge**, derived from the dock face (`dockSidesFor`/`footprintDepth` in `lib/dockZones.js`), never a hardcoded X/Y axis — so it stays correct when docks move walls (cross-dock → depth = distance between the two dock walls).
- **Dedup / scope (audited this session):** `footprintDepth(el)` is ALREADY dock-relative (shipped under B417/B418) and the **canvas** red depth dimension already uses it (`SitePlanner.jsx` ~L9720) — the PANEL is the one surface that never adopted it, so this is a remaining gap, not a re-file. Plan: add the dock-parallel counterpart `footprintLength(el)` + a shared `footprintAxes(el)` (which raw axis is depth vs length) and repoint the panel's two fields + their edit handlers. The **dock-doors summary** (~L8440) reads `max(w,h)` (correct-by-coincidence = the long/length wall) → repoint to `footprintLength` for explicitness; dock-door RENDERING (~L9663) already arrays doors along the long wall — correct. **No persistence migration:** buildings store only `w`/`h` (no `width`/`depth`/`length` field), so this is display + edit-binding only; stored geometry is untouched and reinterpreted via the dock axis at render (lossless). Rotation independence preserved (Length/Depth are local-frame; `rot` only orients the box). Labels stand alone (no sub-caption). **"Rear-load" does not exist in the code** (only Single-load/Cross-dock/No docks) — nothing to remove.

### B543 — Reorganize the Selected · Building panel into 4 concept groups; promote Loading; compact dock features `[Site Planner / massing panel]` (feature)  *(owner-dropped 2026-06-27 as "NEW-2"; minted **B543**; sibling of B542 — same panel)*
`[ ]` The building inspector (`SitePlanner.jsx` ~L8169–8446) is a flat list interleaving four unrelated concepts across five control styles, with the consequential **Docks** loading-type control buried mid-list (salience ≠ importance). Regroup — density + ordering only, drop NOTHING — into four weight/size-labeled groups (house rule: hierarchy by weight/size, never fading):
  - **Footprint** — Length, Depth (the corrected dock-relative pair from B542; bare labels, no sub-captions).
  - **Loading** — **Docks** config promoted as the section anchor, with the dock-feature controls (Dock zones, Car parking, Bump-outs, the Add-layer choosers, per-zone depths) grouped beneath it.
  - **Structure** — Clear height, Slab (each keeps its auto ⇄ manual override affordance).
  - **Placement** — Rotation (moved to the END; a map-orientation nicety, not a building spec).
- **Compact the dock-feature rows** (today: label + sub-label + two large +/− buttons each — very tall) into single-line rows to recover vertical space; standardize control affordances. Full field inventory confirmed against the real component — the Footprint-area + Dock-doors readout + Pin/Delete stay at the bottom.
- **Docks options:** Single-load / Cross-dock / No docks — **"Rear-load" is already absent** (never in the code), nothing to remove there; "No docks" is a legitimate spec-building state and stays.
- **Dedup:** net-new; no existing Open/Done item covers a building-panel reorg (checked both files + origin/main ≤ B541). Sibling to B542; ship together.

### B540 — Address geocoder conflates "service unreachable" with "address not found" `[Site Planner / geocode]` (bug — honesty/UX)  *(found 2026-06-27 by the round-5 bug-hunt, async-errors lens; minted **B540** = highest real B# (B539, round-5 supabase fix) + 1)*
`[ ]` `geocode.js`'s `geocodeAddress` catches every Esri/Nominatim fetch failure and returns `null` — identical to a genuine "no candidates" result. So when the network is down / a host is blocked, the user is told the address doesn't exist (and may give up on a valid address). Violates the CLAUDE.md "honest error surfacing" rule.
- **Why filed not shipped this lap (contract change across callers):** the clean fix changes the return shape to `{lat,lon,label} | {error} | null`, which means **both** call sites must be updated to distinguish the two cases — `MapFinder.jsx` (~L902, the "Find a site" box) and `SitePlanner.jsx` (~L4818, the ＋ Add-parcel "add by address"). A caller that currently does `if (hit) { …hit.lat… }` would treat `{error}` as a hit and read `undefined` coords, so this needs careful per-caller handling + its own headless verification of the new "service unavailable" message. Small but deserves a focused pass, not a same-PR drive-by with two crash fixes.
- **Plan:** return `{error: "…unreachable"}` only when a fetch throws or returns non-OK on BOTH services; keep `null` for "both responded, zero candidates". Update both callers to show a distinct "geocoder unreachable — try again" vs "address not found". Update `test/geocode.test.js` to assert the error is surfaced.

### B541 — Cloud-sync badge popover closes itself when the sync state advances (saving → synced) `[Shared / UI]` (bug — UX)  *(found 2026-06-27 by the round-5 bug-hunt, react-correctness lens; minted **B541**)*
`[ ]` `CloudSyncBadge.jsx` (~L174) keys its internal error-boundary on `String(state)`, so the Badge remounts on EVERY sync-state transition. If the user has the badge's detail popover open while a save completes (`saving` → `synced`), the remount slams the popover shut — jarring, and it hides the very detail they opened.
- **Why filed not shipped this lap (error-boundary semantics):** the boundary key is load-bearing for crash recovery (remounting on an actual error clears stale error state). The fix must keep remount-on-error while NOT remounting on benign transitions — e.g. key on a collapsed `state==='error'||'offline'||'readonly' ? state : 'ok'`. That changes when the crash boundary resets, so it wants its own careful reasoning + a headless check that (a) the popover survives a save cycle and (b) the boundary still recovers from a thrown error. Not a same-PR add with unrelated crash fixes.
- **Plan:** collapse the boundary key to problem-states-only; verify popover persistence across a saving→synced cycle and that an injected render error still recovers.

### B528 — Doc Review: overlapping same-id cloud saves can false-trip the self-conflict guard → autosave lockout `[Doc Review / persistence]` (bug — concurrency)  *(found 2026-06-27 by the round-4 bug-hunt, edge/persistence lens; minted **B528** = highest real B# (B527, the round-4 drawer-theme fix) + 1)*
`[ ]` `usePersistence.js`'s `writeNow` can fire again (debounce flush, beforeunload/visibility, unmount, resume) **before the previous cloud write for the same review id resolves**. Two in-flight upserts to the same row race the optimistic-concurrency `version` compare-and-swap: the second reads the pre-first `version`, so when the first lands the second looks like a *foreign* edit and the badge flips to a false "conflict / saved elsewhere", which then **blocks subsequent autosaves** until reload. No data is lost (last write wins on the row) but the user sees a scary state and silent autosave stops.
- **Why deferred (real blocker — load-bearing guard):** the fix is to serialize writes per id (an in-flight promise chain / single-flight latch so a new `writeNow` awaits or supersedes the pending one, and the `version` it submits is the one the pending write will produce). That touches the exact compare-and-swap anti-clobber path that protects against genuine cross-device clobbers — getting it subtly wrong could DISABLE clobber protection. Needs its own careful pass with a focused concurrency test (simulate two overlapping `writeNow`s, assert one network write coalesces and the badge never false-conflicts), not a same-PR drive-by.
- **Plan when picked up:** add a per-id `inFlight` map in `reviewStore`; `writeNow` chains onto any pending write for that id (coalesce trailing edits into one follow-up write); thread the server-returned `version` forward so the chained write submits the correct expected version. Unit-test the coalescing + the version handoff. Pairs with B529 (same class in the Site Planner).

### B529 — Site Planner: overlapping same-id cloud saves can false-trip the self-conflict guard `[Site Planner / persistence]` (bug — concurrency)  *(found 2026-06-27 by the round-4 bug-hunt; minted **B529**; same root cause as B528, different module)*
`[ ]` The Site Planner's cloud save path (`SitePlanner.jsx` ~L1486, `cloudUpsert` + the save-status state machine) has the same overlapping-write race as B528: a rapid second save for the same site id can submit a stale `version`, so the first write looks like a foreign edit → false "saved elsewhere" badge and a stalled autosave. Same no-data-loss-but-scary-and-sticky symptom.
- **Why deferred:** same blocker as B528 — the `version` compare-and-swap is the real cross-device clobber guard; serializing it wrong weakens that protection. Do it carefully alongside B528 (ideally lift the single-flight latch into a shared helper both `reviewStore` and the planner's `cloudSync` use, so the two modules can't drift).
- **Plan:** shared per-id single-flight write helper in `cloudSync`/`reviewStore`; coalesce trailing edits; forward the server `version`; concurrency tests on both call sites.

### B530 — AuthPanel modal: no Escape-to-close and no dialog role `[Site Planner / a11y]` (bug — accessibility)  *(found 2026-06-27 by the round-4 bug-hunt, a11y lens; minted **B530**)*
`[ ]` `AuthPanel.jsx` renders a centered modal (sign-in / Profile / Settings) over a scrim, but there's no `Escape` keydown handler to close it and the container has no `role="dialog"`/`aria-modal`. A keyboard user can't dismiss it without finding the × by tab, and screen readers don't announce it as a dialog. Small, cleanly-actionable: add a window `keydown` Escape listener (mounted while open) that calls the existing close handler, and `role="dialog" aria-modal="true" aria-label="Account"` on the panel. Verify headless that Escape closes it. Grouped with B531/B532 as a focused a11y lap.

### B531 — Reviews/library rows + Section headers aren't keyboard-reachable `[Doc Review + Site Planner / a11y]` (bug — accessibility)  *(found 2026-06-27 by the round-4 bug-hunt, a11y lens; minted **B531**)*
`[ ]` Several interactive rows are click-only `<div>`s with no `tabIndex`/`role="button"`/`onKeyDown`: `ReviewsBar.jsx` review rows and the collapsible Section header in `SitePlanner.jsx` (~L9781). A keyboard/screen-reader user can't focus or activate them. Cleanly-actionable: make each a real button or add `role="button" tabIndex={0}` + an Enter/Space `onKeyDown` mirroring the `onClick`. Verify focusability headless. Grouped with B530/B532.

### B532 — Site review modal: no Escape-to-close `[Site Planner / a11y]` (bug — accessibility)  *(found 2026-06-27 by the round-4 bug-hunt, a11y lens; minted **B532**)*
`[ ]` `SiteReviewModal.jsx` is a scrim modal with no `Escape` handler (× / scrim-click only) and no `role="dialog"`. Same small fix as B530 (Escape listener while open + dialog role/aria-label). Grouped with B530/B531 as the a11y lap.
