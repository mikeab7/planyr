...
### B488 — Phantom read-only lock on a lone tab (stale Web Lock from a prior closed tab) `[Site Planner / editor lock]` (bug, minor)  *(Cowork signed-in audit 2026-06-26 batch 5, "minor observation"; minted **B488** = highest B# (B487) + 1)*
`[ ]` **Repro:** during a signed-in session, close all extra tabs on a plan except one → the surviving lone tab shows the **"open in another tab — read-only"** banner even though no other tab actually holds the plan. Benign (clears on navigation or a Take-over), but a single-tab user should never see a phantom lockout.
- **Cause (hypothesis, from B455 Web Locks + B466 handoff bus):** the browser's Web Locks are same-origin/same-browser but tab-local; if a tab that held the lock exits ABRUPTLY (crash / hard close / dev-tools stop) it may leave the Lock in an "unreleased" or "waiting" state on the origin briefly, or the same-browser BroadcastChannel yield never fires. The surviving tab's next queued request would then be granted the lock automatically — but the code above may still be observing the pre-close read-only state until an explicit re-probe.
- **Fix direction (small, when picked up):** on `visibilitychange`/focus, if the tab is the only visible one AND its lock role is `readOnly`, re-probe the lock (`request … ifAvailable`) — if it can be acquired, hand the tab up to `active` without waiting for a Take-over click. Alternatively, a heartbeat + expiry on the BroadcastChannel bus (a peer that stops broadcasting for N seconds is presumed gone → release its claim). Deferred until it recurs so we can be sure of the actual trigger.
