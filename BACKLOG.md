...
### B689 — Aerial basemap control: honest disabled state without a placement; folded into the shared panel as a Basemap group (Off / Aerial / USGS) `[Site Planner]` (bug) #site-planner #ui #gis  *(owner brief 2026-07-07, arrived as "NEW-1"; minted **B689** = B688 + 1. Branch `claude/basemap-boundary-layers-dntiit`. DEDUPE-FIRST: B654 (References-panel CAPTURED aerial + opacity/lock) is a different control; B171 (imagery-source eval) stays GATED — nothing new evaluated, this reuses the existing B220 registry; B65/B182/B220 are the backfill/retina history this preserves. Net-new.)*
`[x]` **IMPLEMENTED + headless-verified this session; parks (Verify: live — the USGS source's live imagery + the retina maxNative−1 ceiling behavior are GIS-endpoint / zoom-dependent classes) → V232.**
- Verify: live
...
- **Verified here (sandbox, seeded cache):** hover → "Harris County" tooltip; click → popover with name + jurisdiction wording + source; labels render in the label pane with collision-drop (overlapping seeds → biggest wins); labels clear past the gate and on toggle-off; zero page errors. Live density/collision behavior on real 254-county data = V234.
> **Dedup:** presentation layer over B690's data; reuses the B176 disclaimer wording verbatim in spirit (per-source phrasing), never a new jurisdictional claim.
...
### B672 — Element-level sync, phase 3/5: realtime read path + rejoin refetch (read cutover; blob frozen) `[Site Planner]` (feature) #site-planner #persistence  *(owner brief 2026-07-06, arrived as "NEW-3"; minted **B672** — see B671's renumber note. Branch `claude/site-elements-schema-sync-4p2x5b`.)*
