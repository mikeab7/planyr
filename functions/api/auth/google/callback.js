...
const page = (title, bodyHtml) =>
  new Response(
    ...
    `<title>${title}</title><body style="font-family:system-ui,sans-serif;max-width:640px;margin:40px auto;padding:0 16px;color:#2c2a26;line-height:1.5">${bodyHtml}</body>`,
...
export async function onRequestGet(context) {
  ...
  if (err) return page("Consent cancelled", `<h2>Consent was cancelled</h2><p>Google returned: <code>${err}</code>. You can close this tab and try again.</p>`);
  ...
  if (!r.ok) return page("Token exchange failed", `<h2>Couldn't complete sign-in</h2><p>${r.error}</p>`);
  if (!r.refreshToken) {
  ...
  return page("Drive connected — copy your refresh token", `
    ...
    <textarea readonly rows="4" style="width:100%;font-family:ui-monospace,monospace;font-size:13px;padding:8px;border:1px solid #d8d3c7;border-radius:8px" onclick="this.select()">${r.refreshToken}</textarea>
