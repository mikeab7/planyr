...
### V110 — Schedule toolbar lifted into the unified header, driving the iframe over the bridge (B386) ✅ (self-verified headless — fully done)
- **Harness:** `ui-audit/verify-b386-toolbar-lift.mjs` (chromium-1228, `--ignore-certificate-errors`, built app via `vite preview` on `:4173`). Switches to Schedule, waits for the iframe + first `planar:toolbar-state`, then drives the LIFTED controls in the shell header and observes BOTH the shell and the iframe.
...
- **Supersedes V106** (`verify-schedule-toolbar.mjs`, B381) — that guarded the toolbar INSIDE the iframe; B386 moved it to the shell, so V106's harness was retired and replaced by this one.

### V109 — Optional center toolbar zone in AppHeader Row 2 (B385) ✅ (self-verified headless — fully done)
- **Harness:** `ui-audit/verify-b385-toolbar-center.mjs` + `ui-audit/header-center-harness.{html,jsx}` (chromium-1228, `--ignore-certificate-errors`, `vite` dev on `:5173`). Mounts the REAL `AppHeader` twice inside a real `ThemeProvider` — once WITHOUT `toolbarCenter` (the unchanged 2-zone layout) and once WITH it (the new 3-zone layout).
...
### V108 — "Add parcel" front-door in the Parcel left-hand panel (B383) ✅ (self-verified headless — fully done)
- **Harness:** `ui-audit/verify-b383-add-parcel.mjs` (chromium-1228, `--ignore-certificate-errors`, logged-out against the built `dist/` on `:4173`). Two scenarios, both seeding one locked parcel so the Parcel panel auto-opens: **A** a georeferenced site (`origin` set → Identify enabled), **B** a site with `origin: null` (no GIS frame).
- **Result:** PASS 2026-06-22 — **14/14**. Scenario A: the **＋ Add parcel** button is present and reads as a **primary action** (accent background `rgb(194, 65, 12)` = `--canvas-selection`, not a plain chip); clicking it opens the flyout with **both** methods ("Identify from county GIS" + "Draw a new boundary"); clicking **Identify** arms identify mode (the "Identifying — click a spot" status row appears) and clicking that row stops it; re-opening and clicking **Draw a new boundary** arms the parcel draw tool (the "Click to drop boundary points" boundary hint shows); the old standalone "🔍 Identify parcel" toggle is consolidated away (0 occurrences). Scenario B: the menu's Identify path shows the disabled "needs a georeferenced plan" copy while **Draw a new boundary** stays enabled and works (boundary hint shows) — the no-GIS-frame fallback. **0 app console/page errors** in both (30 env GIS-CORS probe lines from the sandbox proxy ignored, as documented). Screenshots `ui-audit/screens/b383-add-parcel-menu.png` (flyout open), `b383-add-parcel-geo.png`, `b383-add-parcel-nogeo.png`.
- **Pre-merge gates:** lint 0 errors · **1201 tests** · build green; `SitePlannerApp` lazy chunk intact. Reuses the existing `addIdentifiedParcel` / `setParcels` / `identifyMode` / `selectTool("parcel")` paths — no new add pipeline.
